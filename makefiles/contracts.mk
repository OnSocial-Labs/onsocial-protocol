...
test-integration-contract-%-no-run: build-docker-contracts ensure-scripts-executable
	...
	@if [ "$*" = "core-onsocial" ]; then \
		...
		$(MAKE) test-integration-contract-vesting-onsocial-no-run; \
...
.PHONY: test-integration-contract-boost-onsocial-no-run
test-integration-contract-boost-onsocial-no-run: build-docker-contracts ensure-scripts-executable
	$(call docker_run_contracts,set -euo pipefail; \
		cd /code/contracts/boost-onsocial; \
		cargo build --release --target wasm32-unknown-unknown; \
		cd /code; \
		cargo test -p onsocial-integration-tests --release --color always --no-run)
	@echo "✅ Integration tests compiled for contract boost-onsocial (no-run)"

.PHONY: test-integration-contract-social-spend-onsocial-no-run
test-integration-contract-social-spend-onsocial-no-run: build-docker-contracts ensure-scripts-executable
	$(call docker_run_contracts,set -euo pipefail; \
		cd /code/contracts/social-spend-onsocial; \
		cargo build --release --target wasm32-unknown-unknown; \
		cd /code; \
		cargo test -p onsocial-integration-tests --release --color always --no-run)
	@echo "✅ Integration tests compiled for contract social-spend-onsocial (no-run)"
...
.PHONY: test-integration-contract-vesting-onsocial-no-run
test-integration-contract-vesting-onsocial-no-run: build-docker-contracts ensure-scripts-executable
	$(call docker_run_contracts,set -euo pipefail; \
		cd /code/contracts/vesting-onsocial; \
		cargo build --release --target wasm32-unknown-unknown; \
		cd /code; \
		cargo test -p onsocial-integration-tests --release --color always --no-run)
	@echo "✅ Integration tests compiled for contract vesting-onsocial (no-run)"
