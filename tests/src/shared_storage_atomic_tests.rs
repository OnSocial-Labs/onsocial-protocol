...
fn find_event_by_operation<'a>(
    events: &'a [StorageEvent],
    operation: &str,
) -> Option<&'a StorageEvent> {
...
async fn test_share_storage_updates_pool_used_bytes() -> Result<()> {
    ...
    let shared_bytes_before = pool_before
        ...
        .unwrap_or(0);
    println!(
        "   Pool before: used_bytes={}, shared_bytes={}",
        used_bytes_before, shared_bytes_before
    );
    ...
    let shared_bytes_after = pool_after
        ...
        .unwrap_or(0);
    println!(
        "   Pool after: used_bytes={}, shared_bytes={}",
        used_bytes_after, shared_bytes_after
    );
...
async fn test_return_storage_credits_pool_used_bytes() -> Result<()> {
    ...
    let shared_bytes_before = pool_before
        ...
        .unwrap_or(0);
    println!(
        "   Pool before return: used_bytes={}, shared_bytes={}",
        used_bytes_before, shared_bytes_before
    );
    ...
    let shared_bytes_after = pool_after
        ...
        .unwrap_or(0);
    println!(
        "   Pool after return: used_bytes={}, shared_bytes={}",
        used_bytes_after, shared_bytes_after
    );
    ...
    if overhead_credited > 0 {
        println!(
            "   ✓ used_bytes decreased by {} bytes (overhead credited)",
            overhead_credited
        );
        ...
        println!(
            "   ⚠ used_bytes did not decrease: before={}, after={}",
            used_bytes_before, used_bytes_after
        );
    }
...
async fn test_share_storage_rejects_duplicate_allocation() -> Result<()> {
    ...
    let failures: Vec<String> = second_share
        .failures()
        .iter()
        .map(|f| format!("{:?}", f))
        .collect();
    ...
    assert!(
        error_text.contains("already")
            || error_text.contains("allocation")
            || error_text.contains("shared"),
        "Error should mention existing allocation: {}",
        error_text
    );
...
async fn test_share_storage_rejects_below_minimum_bytes() -> Result<()> {
    ...
    let failures: Vec<String> = share_result
        .failures()
        .iter()
        .map(|f| format!("{:?}", f))
        .collect();
    let error_text = failures.join(" ");
    assert!(
        error_text.contains("2000")
            || error_text.contains("minimum")
            || error_text.contains("at least"),
        "Error should mention minimum bytes: {}",
        error_text
    );
    ...
    for log in &logs {
        if log.starts_with(EVENT_JSON_PREFIX) && log.contains("return_storage") {
                ...
                .and_then(|arr| arr.first())
            {
                ...
                assert!(
                    data.get("max_bytes").is_some(),
                    "Event should have max_bytes"
                );
                assert!(
                    data.get("used_bytes").is_some(),
                    "Event should have used_bytes"
                );
...
async fn test_share_storage_verifies_capacity_after_overhead() -> Result<()> {
    ...
    if let Some(pool) = pool_info {
        ...
        println!(
            "   Pool: balance={} yocto, used_bytes={}",
            storage_balance, used_bytes
        );
...
async fn test_share_storage_increases_contract_storage_usage() -> Result<()> {
    ...
    println!(
        "   Contract storage_usage BEFORE share: {} bytes",
    ...
    println!(
        "   Contract storage_usage AFTER share: {} bytes",
        storage_usage_after
    );
    ...
    println!(
        "   ✓ Contract storage_usage increased by {} bytes",
        storage_delta
    );
    ...
    if used_bytes >= storage_delta {
        println!(
            "   ✓ Pool used_bytes ({}) >= storage_delta ({})",
            used_bytes, storage_delta
        );
    } else {
        println!(
            "   ⚠ Pool used_bytes ({}) < storage_delta ({})",
...
async fn test_shared_storage_lifecycle_write_delete_returns_to_pool() -> Result<()> {
    ...
    let bob_shared_used_after_write = bob_storage_after_write
        ...
        .unwrap_or(0);
    println!(
        "   Bob's shared.used_bytes after write: {}",
        bob_shared_used_after_write
    );
    assert!(
        bob_shared_used_after_write > 0,
        "Bob should have used some shared bytes"
    );
    ...
    let bob_shared_used_after_delete = bob_storage_after_delete
        ...
        .unwrap_or(0);
    println!(
        "   Bob's shared.used_bytes after delete: {}",
    ...
    println!(
        "   ✓ Bob's shared.used_bytes decreased by {} bytes",
        bob_bytes_freed
    );
    ...
    println!(
        "   ✓ Pool used_bytes decreased by {} bytes",
        pool_bytes_freed
    );

    // The bytes freed should be approximately equal
    println!(
        "   Bob freed: {} bytes, Pool recovered: {} bytes",
        bob_bytes_freed, pool_bytes_freed
    );
...
async fn test_shared_storage_multiple_users_write_delete_lifecycle() -> Result<()> {
    ...
    let pool_used_after_writes = pool_after_writes
        ...
        .unwrap_or(0);
    println!(
        "   Pool used_bytes after writes: {}",
        pool_used_after_writes
    );
    ...
    println!(
        "   Bob's shared.used_bytes final: {}",
        bob_shared_used_final
    );
    println!(
        "   Carol's shared.used_bytes final: {}",
        carol_shared_used_final
    );
...
async fn test_share_storage_event_matches_view_data() -> Result<()> {
    ...
    let used_bytes_before = pool_before
        ...
        .unwrap_or(0);
    println!(
        "   Pool before: shared_bytes={}, used_bytes={}",
        shared_bytes_before, used_bytes_before
    );
    ...
    assert!(
        share_event.is_some(),
        "share_storage event should be emitted"
    );
    ...
    let used_bytes_after = pool_after
        ...
        .unwrap_or(0);
    println!(
        "   Pool after (view): shared_bytes={}, used_bytes={}",
        shared_bytes_after, used_bytes_after
    );
    ...
    let event_new_shared = event
        .fields
        .get("new_shared_bytes")
    ...
    println!(
        "   ✓ Event new_shared_bytes matches after state: {}",
        event_new_shared
    );

    // Verify new_used_bytes in event matches after state
    let event_new_used = event
        .fields
        .get("new_used_bytes")
        .and_then(|s| s.parse::<u64>().ok())
        .unwrap_or(0);
    ...
    println!(
        "   ✓ Event new_used_bytes matches after state: {}",
        event_new_used
    );

    // Verify pool_available_bytes is present
    let event_available = event
        .fields
        .get("pool_available_bytes")
        .and_then(|s| s.parse::<u64>().ok());
    assert!(
        event_available.is_some(),
        "Event should have pool_available_bytes"
    );
    println!(
        "   ✓ Event pool_available_bytes: {}",
        event_available.unwrap()
    );
...
async fn test_return_storage_event_matches_view_data() -> Result<()> {
    ...
    let used_bytes_before = pool_before
        ...
        .unwrap_or(0);
    println!(
        "   Pool before return: shared_bytes={}, used_bytes={}",
        shared_bytes_before, used_bytes_before
    );
    ...
    let bob_used_before = bob_storage_before
        ...
        .unwrap_or(0);
    println!(
        "   Bob's shared.used_bytes before return: {}",
        bob_used_before
    );
    ...
    assert!(
        ...
        "return_storage event should be emitted"
    );
    ...
    println!(
        "   ✓ Event used_bytes matches Bob's usage: {}",
        event_used_bytes
    );
...
async fn test_share_storage_rollback_on_overhead_capacity_failure() -> Result<()> {
    ...
    let pool_info = get_shared_pool(&contract, alice.id().as_str())
        .await?
        .expect("Pool should exist");
    let available_bytes = pool_info
        .get("available_bytes")
        .and_then(|v| v.as_u64())
        .unwrap_or(0);
    ...
    println!(
        "\n   Step 2: Alice shares {} bytes with Bob...",
        first_share_bytes
    );
    ...
    let pool_before = get_shared_pool(&contract, alice.id().as_str())
        .await?
        .expect("Pool should exist");
    let shared_bytes_before = pool_before
        .get("shared_bytes")
    ...
    let available_before = pool_before
        ...
        .unwrap_or(0);
    println!(
        "   Pool before: shared={}, used={}, available={}",
    ...
    let carol_has_shared_before = carol_storage_before
        ...
        .is_some();
    println!(
        "   Carol has shared_storage before: {}",
        carol_has_shared_before
    );
    assert!(
        !carol_has_shared_before,
        "Carol should NOT have shared storage yet"
    );
    ...
    let pool_after = get_shared_pool(&contract, alice.id().as_str())
        .await?
        .expect("Pool should exist");
    let shared_bytes_after = pool_after
        .get("shared_bytes")
...
async fn test_share_storage_nonexistent_pool_distinct_error() -> Result<()> {
    ...
    assert!(
        pool_before.is_none(),
        "Alice should NOT have a pool initially"
    );
...
async fn test_return_storage_validates_before_mutation() -> Result<()> {
    ...
    let pool_before = get_shared_pool(&contract, alice.id().as_str())
        .await?
        .expect("Pool should exist");
    let pool_shared_before = pool_before
        .get("shared_bytes")
        .and_then(|v| v.as_u64())
        .unwrap_or(0);
    let pool_used_before = pool_before
        .get("used_bytes")
        .and_then(|v| v.as_u64())
        .unwrap_or(0);
    println!(
        "   Pool before: shared_bytes={}, used_bytes={}",
        pool_shared_before, pool_used_before
    );
    ...
    if bob_used > 0 && bob_balance == 0 {
        ...
        let pool_after = get_shared_pool(&contract, alice.id().as_str())
            .await?
            .expect("Pool should exist");
        let pool_shared_after = pool_after
            .get("shared_bytes")
            .and_then(|v| v.as_u64())
            .unwrap_or(0);
        let pool_used_after = pool_after
            .get("used_bytes")
            .and_then(|v| v.as_u64())
            .unwrap_or(0);
        ...
        let bob_storage_after: serde_json::Value = contract
            ...
            .await?
            .json()?;
        let bob_still_has_shared = bob_storage_after.get("shared_storage").is_some();
        assert!(
            bob_still_has_shared,
            "Bob should still have shared storage after failed return"
        );
        println!("   ✓ Bob still has shared storage allocation");
    } else {
        // If return succeeded (Bob has balance or no data), that's also valid
        println!(
            "   ⓘ Return succeeded (Bob has balance or no data). Test scenario not applicable."
        );
    }
...
async fn test_share_storage_self_share_rejected() -> Result<()> {
    ...
    assert!(
        ...
        "Error should mention self-share: {}",
        error_text
    );
...
async fn test_share_storage_rejects_invalid_target_id() -> Result<()> {
    ...
    assert!(
        !share_result.is_success(),
        "share_storage with invalid target_id should fail"
    );
...
async fn test_shared_pool_deposit_rejects_invalid_pool_id() -> Result<()> {
    ...
    assert!(
        !deposit_result.is_success(),
        "shared_pool_deposit with invalid pool_id should fail"
    );
    println!("   ✓ Deposit with invalid pool_id rejected");

    // Verify error message mentions invalid pool_id
    let failures: Vec<String> = deposit_result
        .failures()
        .iter()
        .map(|f| format!("{:?}", f))
        .collect();
    ...
    assert!(
        ...
        "Error should mention invalid pool_id: {}",
        error_text
    );
...
async fn test_group_sponsor_quota_set_rejects_invalid_target_id() -> Result<()> {
    ...
    // Should fail
    assert!(
        !quota_set.is_success(),
        "group_sponsor_quota_set with invalid target_id should fail"
    );
    println!("   ✓ Quota set with invalid target_id rejected");

    // Verify error message mentions invalid target_id
    let failures: Vec<String> = quota_set
        .failures()
        .iter()
        .map(|f| format!("{:?}", f))
        .collect();
...
async fn test_return_shared_storage_rejects_when_no_allocation() -> Result<()> {
    ...
    assert!(
        !has_shared,
        "Alice should NOT have shared storage initially"
    );
    ...
    assert!(
        error_text.contains("No shared storage allocation")
            || error_text.contains("no shared storage"),
        "Error should mention 'No shared storage allocation to return', got: {}",
        error_text
    );
...
async fn test_return_shared_storage_rejects_when_pool_deleted() -> Result<()> {
    ...
    if !withdraw_result.is_success() {
        println!(
            "   ⓘ Withdrawal failed (pool has outstanding allocations) - this is correct behavior"
        );
    ...
    assert!(
        !return_result.is_success(),
        "return_shared_storage should fail when pool deleted"
    );
    println!("   ✓ Return correctly rejected");
    ...
    assert!(
        error_text.contains("does not exist") || error_text.contains("not exist"),
        "Error should mention pool doesn't exist: {}",
        error_text
    );
