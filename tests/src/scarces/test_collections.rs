...
async fn test_create_collection_invalid_id_fails() -> Result<()> {
    ...
    assert!(
        result.is_failure(),
        "collection_id with ':' should be rejected"
    );
    ...
    assert!(
        result.is_failure(),
        "reserved collection_id 's' should be rejected"
    );
...
async fn test_mint_from_collection() -> Result<()> {
    ...
    assert!(
        tokens[0].token_id.starts_with("mintcol:"),
        "token should start with collection prefix"
    );
...
async fn test_mint_from_collection_non_creator_fails() -> Result<()> {
    ...
    let result =
        mint_from_collection(&contract, &stranger, "authcol", 1, None, DEPOSIT_LARGE).await?;
    assert!(
        result.is_failure(),
        "non-creator should not be able to mint_from_collection"
    );
...
async fn test_airdrop_from_collection() -> Result<()> {
    ...
    let progress = get_collection_progress(&contract, "airdrop")
        .await?
        .unwrap();
...
async fn test_purchase_from_collection_insufficient_deposit_fails() -> Result<()> {
    ...
    let result = purchase_from_collection(&contract, &buyer, "expcol", 1, price, ONE_YOCTO).await?;
...
async fn test_pause_non_creator_fails() -> Result<()> {
    ...
    assert!(
        result.is_failure(),
        "non-creator should not pause collection"
    );
...
async fn test_delete_collection_with_minted_tokens_fails() -> Result<()> {
    ...
    assert!(
        result.is_failure(),
        "cannot delete collection with minted tokens"
    );
