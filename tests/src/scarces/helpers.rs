...
// Admin View Helpers
//
// All admin views are exposed via the consolidated `get_contract_info()` view
// method on the contract. The helpers below extract individual fields from
// that response so existing test code keeps working unchanged.
// =============================================================================

#[derive(Debug, Clone, Deserialize)]
struct ContractInfoView {
    owner: String,
    version: String,
    fee_recipient: String,
    fee_config: FeeConfig,
    approved_nft_contracts: Vec<String>,
}

async fn get_contract_info(contract: &Contract) -> Result<ContractInfoView> {
    let result = contract.view("get_contract_info").await?;
    let info: ContractInfoView = serde_json::from_slice(&result.result)?;
    Ok(info)
}

pub async fn get_fee_config(contract: &Contract) -> Result<FeeConfig> {
    Ok(get_contract_info(contract).await?.fee_config)
}

pub async fn get_fee_recipient(contract: &Contract) -> Result<String> {
    Ok(get_contract_info(contract).await?.fee_recipient)
}

pub async fn get_owner(contract: &Contract) -> Result<String> {
    Ok(get_contract_info(contract).await?.owner)
}

pub async fn get_version(contract: &Contract) -> Result<String> {
    Ok(get_contract_info(contract).await?.version)
}

pub async fn get_approved_nft_contracts(contract: &Contract) -> Result<Vec<String>> {
    Ok(get_contract_info(contract).await?.approved_nft_contracts)
}
