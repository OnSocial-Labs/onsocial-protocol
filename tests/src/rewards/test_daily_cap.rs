...
async fn test_daily_cap_limits_credit() -> Result<()> {
    ...
    assert_eq!(
        claimable, DEFAULT_MAX_DAILY,
        "claimable should be exactly max_daily"
    );
...
async fn test_daily_cap_partial_credit() -> Result<()> {
    ...
    credit_reward(&rewards, &owner, user.id().as_str(), 90 * ONE_SOCIAL, None)
        .await?
        .into_result()?;
...
async fn test_daily_cap_different_users_independent() -> Result<()> {
    let (owner, _ft, rewards) = full_setup(&create_sandbox().await?).await?;
    let user1 = owner
        .create_subaccount("ind1")
        .initial_balance(near_workspaces::types::NearToken::from_near(5))
        .transact()
        .await?
        .into_result()?;
    let user2 = owner
        .create_subaccount("ind2")
        .initial_balance(near_workspaces::types::NearToken::from_near(5))
        .transact()
        .await?
        .into_result()?;
    ...
    let result =
        credit_reward(&rewards, &owner, user2.id().as_str(), 50 * ONE_SOCIAL, None).await?;
    assert!(
        result.is_success(),
        "different user cap should be independent"
    );
...
async fn test_daily_cap_after_max_daily_increase() -> Result<()> {
    ...
    assert!(
        result.is_success(),
        "should allow more after max_daily increase"
    );
