...
async fn test_claim_transfers_tokens() -> Result<()> {
    ...
    assert_eq!(
        user_ft_balance,
        10 * ONE_SOCIAL,
        "user should receive SOCIAL tokens"
    );
...
async fn test_claim_auto_registers_user_with_ft() -> Result<()> {
    ...
    assert!(
        ...
        "user should not be registered before claim"
    );
    ...
    assert!(
        result.is_success(),
        "claim should succeed for unregistered user"
    );

    // Confirm user IS now registered with FT
    let balance_after = ft
        .view("storage_balance_of")
        .args_json(serde_json::json!({ "account_id": user.id().to_string() }))
        .await?
        .json::<Option<serde_json::Value>>()?;
    assert!(
        ...
        "user should be registered after claim"
    );
...
async fn test_claim_already_registered_user() -> Result<()> {
    let (owner, ft, rewards) = full_setup(&create_sandbox().await?).await?;
    let user = owner
        .create_subaccount("preregistered")
        .initial_balance(near_workspaces::types::NearToken::from_near(5))
        .transact()
        .await?
        .into_result()?;
    ...
    // Credit and claim
    credit_reward(&rewards, &owner, user.id().as_str(), 8 * ONE_SOCIAL, None)
        .await?
        .into_result()?;
    let result = claim_rewards(&rewards, &user).await?;
    assert!(
        result.is_success(),
        "claim should succeed for pre-registered user"
    );
...
async fn test_claim_multiple_credits_then_single_claim() -> Result<()> {
    ...
    assert_eq!(
        user_ft_balance,
        10 * ONE_SOCIAL,
        "should claim accumulated total"
    );
...
async fn test_claim_twice_second_fails() -> Result<()> {
    ...
    assert!(
        result.is_failure(),
        "second claim with 0 balance should fail"
    );
