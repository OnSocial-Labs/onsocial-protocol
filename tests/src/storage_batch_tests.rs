...
fn user_profile_with_deposit(user_id: &str, name: &str, deposit_amount: u128) -> serde_json::Value {
    ...
    let _ = user_id;
    data.insert(
        "storage/deposit".to_string(),
        json!({"amount": deposit_amount.to_string()}),
    );
    data.insert("profile/name".to_string(), json!(name));
    json!({
        "request": {
...
async fn test_batch_storage_deposit_with_data_real_balance() -> anyhow::Result<()> {
    ...
    if let Some(balance) = storage_balance.get("balance").and_then(|b| b.as_u64()) {
        ...
        assert!(
            balance_near <= 1.1,
            "Storage balance should not exceed attached amount (got {} NEAR)",
            balance_near
        );
    }
...
async fn test_storage_deposit_missing_amount() -> anyhow::Result<()> {
    ...
    assert!(
        ...
        "Should contain 'amount required': {}",
        failure_msg
    );
...
async fn test_storage_refund_goes_to_user() -> anyhow::Result<()> {
    ...
    assert!(
        spent_near < 1.0,
        "User should have been refunded ~1.5 NEAR, but spent {} NEAR",
        spent_near
    );
...
async fn test_shared_pool_deposit_uses_attached_balance() -> anyhow::Result<()> {
    ...
    assert!(
        result.is_success(),
        "Shared pool deposit should succeed: {:?}",
        result.failures()
    );
    ...
    assert!(
        spent_near < 1.5,
        "User should have been refunded ~1 NEAR, but spent {} NEAR",
        spent_near
    );
    println!("   ✓ Unused balance was refunded correctly");
...
async fn test_shared_pool_deposit_cant_exceed_attached() -> anyhow::Result<()> {
    ...
    assert!(
        result.is_failure(),
        "Should have failed when trying to deposit more than attached!"
    );
...
async fn test_batch_exceeds_attached_fails_atomically() -> anyhow::Result<()> {
    ...
    assert!(
        result.is_failure(),
        "Should have failed when batch exceeds attached!"
    );
...
async fn test_shared_pool_deposit_unauthorized_cross_account() -> anyhow::Result<()> {
    ...
    assert!(
        result.is_failure(),
        "Cross-account shared pool deposit should fail"
    );
    let failure_msg = format!("{:?}", result.failures());
    assert!(
        failure_msg.contains("Permission denied") || failure_msg.contains("Unauthorized"),
        "Should be permission/authorization error: {}",
        failure_msg
    );
...
async fn test_shared_pool_deposit_zero_amount() -> anyhow::Result<()> {
    ...
    assert!(
        result.is_failure(),
        "Zero amount shared pool deposit should fail"
    );
    let failure_msg = format!("{:?}", result.failures());
    assert!(
        failure_msg.contains("Minimum pool deposit"),
        "Should contain 'Minimum pool deposit': {}",
        failure_msg
    );
    println!("   ✓ Zero amount correctly rejected");
...
async fn test_shared_pool_deposit_missing_pool_id() -> anyhow::Result<()> {
    ...
    assert!(
        ...
        "Should contain 'pool_id required': {}",
        failure_msg
    );
...
async fn test_shared_pool_deposit_invalid_pool_id() -> anyhow::Result<()> {
    ...
    assert!(
        ...
        "Should contain 'Invalid pool_id': {}",
        failure_msg
    );
...
async fn test_shared_pool_deposit_accumulates_balance() -> anyhow::Result<()> {
    ...
    assert_eq!(
        balance1,
        deposit1.as_yoctonear(),
        "First deposit should be in pool"
    );
    ...
    println!(
        "   ✓ Second deposit: pool balance = {} yoctoNEAR (expected {})",
        balance2, expected
    );
...
async fn test_shared_pool_deposit_emits_correct_event() -> anyhow::Result<()> {
    ...
    assert!(
        event_log.contains("pool_id"),
        "Event should contain pool_id"
    );
    assert!(event_log.contains("amount"), "Event should contain amount");
    assert!(
        event_log.contains("previous_pool_balance"),
        "Event should contain previous_pool_balance"
    );
    assert!(
        event_log.contains("new_pool_balance"),
        "Event should contain new_pool_balance"
    );
    assert!(
        event_log.contains(&deposit_amount.as_yoctonear().to_string()),
        "Event should contain deposit amount"
    );
    println!("   ✓ Event contains all required fields");

    // Verify previous_pool_balance was 0 for first deposit
    assert!(
        event_log.contains("\"previous_pool_balance\":\"0\""),
        "First deposit should have previous_pool_balance=0: {}",
        event_log
    );
...
async fn test_storage_withdraw_goes_to_signer() -> anyhow::Result<()> {
    ...
    let user_after = user.view_account().await?.balance;
    let gained = user_after
        .as_yoctonear()
        .saturating_sub(user_before.as_yoctonear());
...
async fn test_withdraw_more_than_available_fails() -> anyhow::Result<()> {
    ...
    assert!(
        result.is_failure(),
        "Should fail when withdrawing more than available!"
    );
...
async fn test_withdraw_zero_amount_rejected() -> anyhow::Result<()> {
    ...
    assert!(
        ...
        "Error should mention zero amount rejection: {}",
        failure_str
    );
...
async fn test_withdraw_unregistered_account_rejected() -> anyhow::Result<()> {
    ...
    assert!(
        result.is_failure(),
        "Withdraw from unregistered account should fail!"
    );

    let failure_str = format!("{:?}", result.failures());
    assert!(
        failure_str.contains("Account not registered") || failure_str.contains("not registered"),
        "Error should mention account not registered: {}",
        failure_str
    );
...
async fn test_withdraw_all_when_zero_balance_rejected() -> anyhow::Result<()> {
    ...
    assert!(
        result.is_failure(),
        "Withdraw all with zero balance should be rejected!"
    );

    let failure_str = format!("{:?}", result.failures());
    assert!(
        failure_str.contains("Nothing to withdraw"),
        "Error should say 'Nothing to withdraw': {}",
        failure_str
    );
...
async fn test_storage_withdraw_event_emission() -> anyhow::Result<()> {
    ...
    let storage_before: serde_json::Value = contract
        ...
        .await?
        .json()?;
    let balance_before = storage_before
        .get("balance")
        .and_then(|v| v.as_str())
        .and_then(|s| s.parse::<u128>().ok())
        .unwrap_or(0);
    ...
    assert!(
        ...
        "Should emit storage_withdraw event"
    );
    let event_json = withdraw_event
        .unwrap()
    ...
    assert_eq!(
        ...
        Some("storage_withdraw")
    );

    // Verify required fields exist
    assert!(
        event_data.get("amount").is_some(),
        "Event should have 'amount' field"
    );
    assert!(
        event_data.get("previous_balance").is_some(),
        "Event should have 'previous_balance' field"
    );
    assert!(
        event_data.get("new_balance").is_some(),
        "Event should have 'new_balance' field"
    );
    assert!(
        event_data.get("available_balance").is_some(),
        "Event should have 'available_balance' field"
    );

    // Verify amount matches
    let event_amount = event_data
        .get("amount")
        .and_then(|v| v.as_str())
        .and_then(|s| s.parse::<u128>().ok())
        .unwrap_or(0);
    assert_eq!(
        ...
        "Event amount should match withdraw amount"
    );
    ...
    println!(
        "     - previous_balance: {}",
        event_data.get("previous_balance").unwrap()
    );
    println!(
        "     - new_balance: {}",
        event_data.get("new_balance").unwrap()
    );
    println!(
        "     - available_balance: {}",
        event_data.get("available_balance").unwrap()
    );
...
async fn test_deposit_write_withdraw_remaining() -> anyhow::Result<()> {
    ...
    let used_bytes = storage_after_write
        .get("used_bytes")
        .and_then(|v| v.as_u64())
        .unwrap_or(0);
    ...
    if result.is_success() {
        ...
        let gained = user_balance_after
            .as_yoctonear()
            .saturating_sub(user_balance_before.as_yoctonear());
        let gained_near = gained as f64 / 1e24;
...
async fn test_withdraw_remaining_balance_exact() -> anyhow::Result<()> {
    ...
    let event_amount: u128 = event_data
        .get("amount")
        .and_then(|v| v.as_str())
        .and_then(|s| s.parse().ok())
        .expect("Event should have amount");

    let previous_balance: u128 = event_data
        .get("previous_balance")
        .and_then(|v| v.as_str())
        .and_then(|s| s.parse().ok())
        .expect("Event should have previous_balance");

    let new_balance: u128 = event_data
        .get("new_balance")
        .and_then(|v| v.as_str())
        .and_then(|s| s.parse().ok())
    ...
    println!("   Event data:");
    println!(
        "     - previous_balance: {} yoctoNEAR ({} NEAR)",
        previous_balance,
        previous_balance as f64 / 1e24
    );
    println!(
        "     - amount withdrawn: {} yoctoNEAR ({} NEAR)",
        event_amount,
        event_amount as f64 / 1e24
    );
    println!(
        "     - new_balance:      {} yoctoNEAR ({} NEAR)",
        new_balance,
    ...
    println!(
        "   ✓ Remaining balance = {} yoctoNEAR (0.7 NEAR)",
        new_balance
    );
...
async fn test_withdraw_state_transition() -> anyhow::Result<()> {
    ...
    let gained_1 = user_after_1
        .as_yoctonear()
        .saturating_sub(user_before_1.as_yoctonear());
    ...
    assert!(
        gained_near_1 > 0.2,
        "Should gain substantial amount (got {} NEAR)",
        gained_near_1
    );
    assert!(
        gained_near_1 < 0.35,
        "Should not gain more than withdrawn (got {} NEAR)",
        gained_near_1
    );
    ...
    assert!(
        result.is_success(),
        "Second withdraw should succeed: {:?}",
        result.failures()
    );

    let user_after_2 = user.view_account().await?.balance;
    let gained_2 = user_after_2
        .as_yoctonear()
        .saturating_sub(user_before_2.as_yoctonear());
    ...
    let total_gained =
        (user_after_2.as_yoctonear() as f64 - user_before_1.as_yoctonear() as f64) / 1e24;
    println!(
        "\n   Total gained from both withdrawals: {:.4} NEAR",
        total_gained
    );
...
async fn test_shared_pool_only_owner_can_deposit() -> anyhow::Result<()> {
    ...
    assert!(
        result.is_failure(),
        "Bob should not be able to deposit to Alice's pool!"
    );
    ...
    assert!(
        result.is_success(),
        "Alice should be able to deposit to her own pool: {:?}",
        result.failures()
    );
...
async fn test_multiple_deposits_in_sequence() -> anyhow::Result<()> {
    ...
    if let Some(balance_str) = storage_balance.get("balance").and_then(|b| b.as_str()) {
        ...
        println!(
            "   ✓ Total balance correctly accumulated: {} yoctoNEAR",
            balance
        );
    }
...
async fn test_universal_storage_user_writes_without_deposit() -> anyhow::Result<()> {
    ...
    let result = user
        .call(contract.id(), "execute")
        .args_json(user_profile_data(
            user.id().as_str(),
            "Alice",
            Some("Testing universal storage - no deposit needed!"),
        ))
        // NO DEPOSIT ATTACHED!
        .gas(Gas::from_tgas(100))
        .transact()
    ...
    assert!(
        result.is_success(),
        "Write without deposit should succeed: {:?}",
        result.failures()
    );
    ...
    let is_sponsored = storage_balance
...
async fn test_universal_storage_disabled_requires_deposit() -> anyhow::Result<()> {
    ...
    assert!(
        result.is_failure(),
        "Should fail without deposit when pool empty"
    );
...
async fn test_universal_storage_pool_empty_fallback() -> anyhow::Result<()> {
    ...
    assert!(
        result.is_success(),
        "Should succeed with user deposit: {:?}",
        result.failures()
    );
...
async fn test_universal_storage_user_with_deposit_uses_personal() -> anyhow::Result<()> {
    ...
    let personal_balance = storage_balance
        .get("balance")
        .map(|v| {
    ...
    assert!(
        personal_balance > 0,
        "User should have personal balance (preserved)"
    );
    assert!(
        is_sponsored,
        "User should be platform_sponsored (platform pays first)"
    );
    println!(
        "   ✓ User has personal balance: {} yoctoNEAR (preserved for later)",
        personal_balance
    );
    ...
    for i in 1..=5 {
        ...
        let result = user
            ...
            .args_json(user_profile_data(
                user.id().as_str(),
                &format!("User{}", i),
                Some(&format!("I am user number {} using platform storage", i)),
            ))
            // NO DEPOSIT!
            .gas(Gas::from_tgas(100))
            .transact()
...
async fn test_universal_storage_sponsored_user_can_still_deposit() -> anyhow::Result<()> {
    ...
    let is_sponsored = storage_before
        .get("platform_sponsored")
        .and_then(|v| v.as_bool())
        .unwrap_or(false);
    ...
    // Balance can be returned as number or string depending on serialization
    let personal_balance = storage_after
        .get("balance")
        .map(|v| {
            if let Some(n) = v.as_u64() {
                n as u128
    ...
    let still_sponsored = storage_after
        .get("platform_sponsored")
        .and_then(|v| v.as_bool())
        .unwrap_or(false);
    ...
    println!(
        "   ✓ User has personal balance: {} yoctoNEAR",
        personal_balance
    );
...
async fn test_sponsored_user_delete_frees_pool_capacity() -> anyhow::Result<()> {
    ...
    let pool_after_write: serde_json::Value = contract
        ...
        .await?
        .json()?;
    let used_after_write = pool_after_write
        .get("used_bytes")
        .and_then(|v| v.as_u64())
        .unwrap_or(0);
    println!(
        "   ✓ Pool used_bytes after write: {} (increased by {})",
        used_after_write,
        used_after_write - used_before
    );
    assert!(
        used_after_write > used_before,
        "Pool should have increased usage"
    );
    ...
    let pool_after_delete: serde_json::Value = contract
        ...
        .await?
        .json()?;
    let used_after_delete = pool_after_delete
        .get("used_bytes")
        .and_then(|v| v.as_u64())
        .unwrap_or(0);
    println!(
        "   ✓ Pool used_bytes after delete: {} (freed {} bytes)",
        used_after_delete,
        used_after_write.saturating_sub(used_after_delete)
    );

    // Pool usage should decrease (capacity recycled)
    assert!(
        used_after_delete < used_after_write,
...
async fn test_sponsored_user_cannot_withdraw_platform_near() -> anyhow::Result<()> {
    ...
    println!(
        "   User personal storage balance: {} yoctoNEAR",
        personal_balance
    );
    assert_eq!(
        personal_balance, 0,
        "Sponsored user should have 0 personal balance"
    );
...
async fn test_sponsored_user_updates_existing_data() -> anyhow::Result<()> {
    ...
    let pool_after_create: serde_json::Value = contract
        ...
        .await?
        .json()?;
    let used_after_create = pool_after_create
        .get("used_bytes")
        .and_then(|v| v.as_u64())
        .unwrap_or(0);
    ...
    let pool_after_grow: serde_json::Value = contract
        ...
        .await?
        .json()?;
    let used_after_grow = pool_after_grow
        .get("used_bytes")
        .and_then(|v| v.as_u64())
        .unwrap_or(0);
    println!(
        "   ✓ Pool used_bytes after grow: {} (delta: +{})",
        used_after_grow,
        used_after_grow.saturating_sub(used_after_create)
    );

    assert!(
        used_after_grow > used_after_create,
    ...
    let pool_after_shrink: serde_json::Value = contract
        ...
        .await?
        .json()?;
    let used_after_shrink = pool_after_shrink
        .get("used_bytes")
        .and_then(|v| v.as_u64())
        .unwrap_or(0);
    println!(
        "   ✓ Pool used_bytes after shrink: {} (freed: {} bytes)",
        used_after_shrink,
        used_after_grow.saturating_sub(used_after_shrink)
    );

    assert!(
        used_after_shrink < used_after_grow,
...
async fn test_pool_exhausts_mid_batch_fallback() -> anyhow::Result<()> {
    ...
    assert!(
        result.is_success(),
        "Large write should succeed with fallback: {:?}",
        result.failures()
    );
    ...
    let bio_written = entry_value_str(&data, &bio_key)
        .map(|s| s.len())
        .unwrap_or(0);

    assert!(bio_written > 0, "Large bio should be written");
    println!(
        "   ✓ Large data ({} chars) written successfully",
        bio_written
    );
...
async fn test_mixed_sponsored_and_personal_operations() -> anyhow::Result<()> {
    ...
    let _ = sponsored_user
        .call(contract.id(), "execute")
        .args_json(user_profile_data(
            sponsored_user.id().as_str(),
            "SponsoredUser",
            Some("I use platform storage"),
        ))
        .gas(Gas::from_tgas(100))
        .transact()
        .await?;
    ...
    let is_sponsored = sponsored_storage
        .get("platform_sponsored")
        .and_then(|v| v.as_bool())
        .unwrap_or(false);
    ...
    assert!(
        ...
        "User 2 deposit failed: {:?}",
        deposit_result.failures()
    );
    ...
    // Then write
    let write_result = depositing_user
        .call(contract.id(), "execute")
        .args_json(user_profile_data(
            depositing_user.id().as_str(),
            "DepositingUser",
            Some("I deposited but platform pays first"),
        ))
        .gas(Gas::from_tgas(100))
        .transact()
        .await?;
    ...
    let has_balance = depositing_storage
        .get("balance")
        .map(|v| {
            if let Some(n) = v.as_u64() {
                n > 0
    ...
    let is_also_sponsored = depositing_storage
        .get("platform_sponsored")
        .and_then(|v| v.as_bool())
        .unwrap_or(false);
    ...
    assert!(
        has_balance,
        "User 2 should have personal balance, got: {:?}",
        depositing_storage
    );
    assert!(
        is_also_sponsored,
        "User 2 should ALSO be platform_sponsored (platform-first)"
    );
...
async fn test_pool_capacity_exact_match() -> anyhow::Result<()> {
    ...
    while last_available > 1000 {
        // Stop when less than 1KB available
        let user = sandbox.dev_create_account().await?;
        ...
        if users_created % 10 == 0 {
            println!(
                "   Created {} users, {} bytes remaining",
                users_created, last_available
            );
        }
    }

    println!(
        "   ✓ Created {} users, {} bytes remaining in pool",
        users_created, last_available
    );
...
async fn test_pool_exhausts_mid_batch_falls_back_to_personal() -> anyhow::Result<()> {
    ...
    let available_before = pool_before
        ...
        .unwrap_or(0);
    println!(
        "   ✓ Platform pool funded: {} bytes available",
        available_before
    );
    ...
    let storage_after_deposit: serde_json::Value = contract
        ...
        .json()?;
    println!(
        "   ✓ User has personal balance: {:?}",
        storage_after_deposit.get("balance")
    );
    ...
    assert!(
        result.is_success(),
        "Transaction should succeed with graceful fallback to personal balance: {:?}",
        result.failures()
    );
    ...
    let final_used = storage_final
        .get("used_bytes")
        .and_then(|v| v.as_u64())
        .unwrap_or(0);
    let is_sponsored = storage_final
        .get("platform_sponsored")
        .and_then(|v| v.as_bool())
        .unwrap_or(true);
...
async fn test_pool_exhausts_no_personal_balance_fails() -> anyhow::Result<()> {
    ...
    assert!(
        result.is_failure(),
        "Transaction should fail when pool exhausted and no personal balance"
    );
...
async fn test_pool_exhausts_attached_deposit_saves() -> anyhow::Result<()> {
    ...
    assert!(
        result.is_success(),
        "Transaction should succeed with attached deposit fallback"
    );
    ...
    let pool_info: serde_json::Value = contract.view("get_platform_pool").await?.json()?;
    let pool_used = pool_info
        .get("used_bytes")
        .and_then(|v| v.as_u64())
        .unwrap_or(0);
    println!(
        "   ✓ Platform pool used_bytes: {} (unchanged, too small)",
        pool_used
    );
    ...
    let personal_balance = user_storage
        .get("balance")
        .and_then(|v| {
    ...
    let used_bytes = user_storage
        .get("used_bytes")
        .and_then(|v| v.as_u64())
        .unwrap_or(0);

    let is_platform_sponsored = user_storage
        .get("platform_sponsored")
        .and_then(|v| v.as_bool())
        .unwrap_or(false);
    ...
    println!(
        "   ✓ User personal balance: {} yoctoNEAR (~{} NEAR)",
        personal_balance,
        personal_balance / 1_000_000_000_000_000_000_000_000
    );
...
async fn test_full_priority_chain_multi_source_batch() -> anyhow::Result<()> {
    ...
    let pool_info: serde_json::Value = contract.view("get_platform_pool").await?.json()?;
    let platform_pool_bytes = pool_info
        .get("total_bytes")
        .and_then(|v| v.as_u64())
        .unwrap_or(0);
    ...
    assert!(
        result.is_success(),
        "Batch should succeed with multi-source funding: {:?}",
        result.failures()
    );
    println!("   ✓ Batch operation succeeded!");
    ...
    let final_balance = final_storage
        .get("balance")
        .and_then(|v| {
            v.as_f64()
                .map(|f| f as u128)
    ...
    assert!(
        final_balance > personal_deposit.as_yoctonear() / 2,
    ...
    let final_pool: serde_json::Value = contract.view("get_platform_pool").await?.json()?;
    let pool_used = final_pool
        .get("used_bytes")
        .and_then(|v| v.as_u64())
        .unwrap_or(0);
...
async fn test_rigorous_byte_accounting_across_sources() -> anyhow::Result<()> {
    ...
    let pool_initial: serde_json::Value = contract.view("get_platform_pool").await?.json()?;
    let pool_total_bytes = pool_initial
        .get("total_bytes")
        .and_then(|v| v.as_u64())
        .unwrap_or(0);
    let pool_used_initial = pool_initial
        .get("used_bytes")
        .and_then(|v| v.as_u64())
        .unwrap_or(0);
    ...
    println!(
        "   Platform Pool: {} total bytes, {} used",
        pool_total_bytes, pool_used_initial
    );
    ...
    let pool_after_1: serde_json::Value = contract.view("get_platform_pool").await?.json()?;
    let pool_used_after_1 = pool_after_1
        .get("used_bytes")
        .and_then(|v| v.as_u64())
        .unwrap_or(0);
    ...
    let user_storage_1: serde_json::Value = contract
        ...
        .await?
        .json()?;
    let user_used_1 = user_storage_1
        .get("used_bytes")
        .and_then(|v| v.as_u64())
        .unwrap_or(0);
    ...
    assert!(
        pool_delta_1 >= 100,
        "Pool should have increased by at least 100 bytes, got {}",
        pool_delta_1
    );
    ...
    let pool_after_2: serde_json::Value = contract.view("get_platform_pool").await?.json()?;
    let pool_used_after_2 = pool_after_2
        .get("used_bytes")
        .and_then(|v| v.as_u64())
        .unwrap_or(0);
    ...
    assert!(
        pool_delta_2 >= 200,
        "Pool should have increased by at least 200 bytes, got {}",
        pool_delta_2
    );
    assert!(
        pool_total_delta >= 300,
        "Total pool increase should be >= 300 bytes, got {}",
        pool_total_delta
    );
    ...
    let pool_after_3: serde_json::Value = contract.view("get_platform_pool").await?.json()?;
    let pool_used_after_3 = pool_after_3
        .get("used_bytes")
        .and_then(|v| v.as_u64())
        .unwrap_or(0);
    ...
    assert!(
        pool_delta_3 >= 350,
        "Pool should have increased by ~400 bytes (500-100), got {}",
        pool_delta_3
    );
    ...
    let pool_after_4: serde_json::Value = contract.view("get_platform_pool").await?.json()?;
    let pool_used_after_4 = pool_after_4
        .get("used_bytes")
        .and_then(|v| v.as_u64())
        .unwrap_or(0);
    ...
    assert!(
        pool_delta_4 <= -400,
        "Pool should have decreased by ~450 bytes (500-50), got {}",
        pool_delta_4
    );
    ...
    let pool_after_5: serde_json::Value = contract.view("get_platform_pool").await?.json()?;
    let pool_used_after_5 = pool_after_5
        .get("used_bytes")
        .and_then(|v| v.as_u64())
        .unwrap_or(0);
    ...
    assert!(
        pool_delta_5 <= -180,
        "Pool should have decreased by ~200 bytes on delete, got {}",
        pool_delta_5
    );
    ...
    let pool_final: serde_json::Value = contract.view("get_platform_pool").await?.json()?;
    ...
    println!("\n📊 FINAL STATE:");
    println!(
        "   Pool used_bytes: {} (started at {})",
        pool_used_final, pool_used_initial
    );
    ...
    println!(
        "   Net change from initial: {:+} bytes (user account metadata overhead)",
        net_change
    );
...
async fn test_platform_pool_deposit_basic() -> anyhow::Result<()> {
    ...
    println!(
        "   Donor initial balance: {} NEAR",
        donor_before.as_yoctonear() as f64 / 1e24
    );
    ...
    assert!(
        spent_near < 1.5,
        "Donor should have been refunded ~1 NEAR, but spent {} NEAR",
        spent_near
    );
...
async fn test_platform_pool_deposit_insufficient_attached() -> anyhow::Result<()> {
    ...
    assert!(
        result.is_failure(),
        "Should fail when trying to deposit more than attached!"
    );
    println!("   ✓ Correctly rejected: can't deposit more than attached");
...
async fn test_platform_pool_deposit_with_data_operations() -> anyhow::Result<()> {
    ...
    data.insert(
        "storage/deposit".to_string(),
        json!({"amount": "100000000000000000000000"}),
    );
    data.insert(
        "storage/platform_pool_deposit".to_string(),
        json!({"amount": donate_amount.as_yoctonear().to_string()}),
    );
    data.insert(format!("{}/profile/name", user_id), json!("Generous Donor"));
    data.insert(
        format!("{}/profile/bio", user_id),
        json!("I support the platform!"),
    );
    ...
    assert!(
        result.is_success(),
        "Batch with platform_pool_deposit should succeed: {:?}",
        result.failures()
    );
    println!("   ✓ Batch operation succeeded");
...
async fn test_platform_pool_deposit_zero_amount() -> anyhow::Result<()> {
    ...
    assert!(
        result.is_failure(),
        "Zero amount donation should fail: {:?}",
        result.outcomes()
    );
    let failure_msg = format!("{:?}", result.failures());
    assert!(
...
async fn test_platform_pool_deposit_exact_attached() -> anyhow::Result<()> {
    ...
    assert!(
        result.is_success(),
        "Exact amount donation should succeed: {:?}",
        result.failures()
    );
    ...
    println!(
        "   Donor spent: {} NEAR (1 NEAR donation + gas)",
        spent_near
    );

    // Should have spent ~1 NEAR (slightly more for gas)
    assert!(
        spent_near >= 1.0 && spent_near < 1.1,
        "Should have spent ~1 NEAR (donation) + small gas, but spent {} NEAR",
        spent_near
    );
...
async fn test_platform_pool_deposit_funds_new_users() -> anyhow::Result<()> {
    ...
    assert!(
        result.is_success(),
        "Platform pool donation should succeed: {:?}",
        result.failures()
    );
    ...
    assert!(
        ...
        "Share storage should succeed: {:?}",
        share_result.failures()
    );
    ...
    profile_data.insert(
        format!("{}/profile/bio", new_user_id),
        json!("I joined thanks to platform sponsors!"),
    );
...
async fn test_platform_pool_deposit_missing_amount_field() -> anyhow::Result<()> {
    ...
    assert!(
        failure_msg.contains("amount required"),
        "Should mention 'amount required': {}",
        failure_msg
    );
...
async fn test_platform_pool_deposit_cross_account_rejected() -> anyhow::Result<()> {
    ...
    assert!(
        result.is_failure(),
        "Cross-account platform_pool_deposit should be rejected"
    );
    let failure_msg = format!("{:?}", result.failures());
    assert!(
        failure_msg.to_lowercase().contains("permission")
            || failure_msg.to_lowercase().contains("denied"),
...
async fn test_platform_pool_deposit_unparseable_amount() -> anyhow::Result<()> {
    ...
    assert!(
        failure_msg.contains("amount required"),
        "Should mention 'amount required' (parse failure): {}",
        failure_msg
    );
...
async fn test_non_object_data_rejected() -> anyhow::Result<()> {
    ...
    assert!(
        !number_result.is_success(),
        "Number data should be rejected"
    );
...
async fn test_invalid_operation_key_rejected() -> anyhow::Result<()> {
    ...
    assert!(
        !result.is_success(),
        "Invalid operation key should be rejected"
    );

    let failure_msg = format!("{:?}", result.failures());
    assert!(
        failure_msg.contains("Invalid")
            || failure_msg.contains("invalid")
            || failure_msg.contains("operation"),
...
async fn test_set_for_unused_deposit_goes_to_signer() -> anyhow::Result<()> {
    ...
    let grant_result = alice
        ...
        .await?;
    assert!(
        grant_result.is_success(),
        "Grant should succeed: {:?}",
        grant_result.failures()
    );
    println!(
        "   ✓ Alice granted write permission to Bob for {}/delegated/",
        alice_id
    );
    ...
    let bob_balance_before = bob_storage_before
        ...
        .unwrap_or(0);
    println!(
        "   Bob's storage balance before: {} yoctoNEAR",
        bob_balance_before
    );
    ...
    let alice_balance_before = alice_storage_before
        ...
        .unwrap_or(0);
    println!(
        "   Alice's storage balance before: {} yoctoNEAR",
    ...
    assert!(
        ...
        "set_for should succeed: {:?}",
        set_for_result.failures()
    );
    ...
    println!(
        "   Bob's storage balance after: {} yoctoNEAR",
        bob_balance_after
    );
    println!(
        "   Alice's storage balance after: {} yoctoNEAR",
        alice_balance_after
    );
    ...
    println!(
        "   Bob's balance change: {} yoctoNEAR",
        bob_balance_after.saturating_sub(bob_balance_before)
    );
...
async fn test_permission_api_input_validation() -> anyhow::Result<()> {
    ...
    assert!(
        !missing_grantee.is_success(),
        "permission/grant without grantee should fail"
    );
    let err_str = format!("{:?}", missing_grantee.failures());
    assert!(
        err_str.contains("grantee required"),
        "Error should mention grantee required: {}",
        err_str
    );
    println!(
        "   ✓ Correctly rejected: {}",
        err_str.chars().take(80).collect::<String>()
    );
    ...
    assert!(
        !missing_path.is_success(),
        "permission/grant without path should fail"
    );
    let err_str = format!("{:?}", missing_path.failures());
    assert!(
        err_str.contains("path required"),
    ...
    assert!(
        !invalid_grantee.is_success(),
        "permission/grant with invalid grantee should fail"
    );
    let err_str = format!("{:?}", invalid_grantee.failures());
    assert!(
        err_str.contains("Invalid grantee"),
        "Error should mention invalid grantee: {}",
        err_str
    );
    println!(
        "   ✓ Correctly rejected: {}",
    ...
    assert!(
        !revoke_missing_grantee.is_success(),
        "permission/revoke without grantee should fail"
    );
    ...
    assert!(
        !revoke_missing_path.is_success(),
        "permission/revoke without path should fail"
    );
    ...
    assert!(
        !revoke_invalid_grantee.is_success(),
        "permission/revoke with invalid grantee should fail"
    );
    ...
    assert!(
        default_level.is_success(),
        "permission/grant without level should succeed: {:?}",
        default_level.failures()
    );
    ...
    assert!(
        has_write,
        "Bob should have WRITE permission (default level)"
    );
...
async fn test_permission_revoke_edge_cases() -> anyhow::Result<()> {
    ...
    println!(
        "   Revoke non-existent result: success={}",
        revoke_nonexistent.is_success()
    );
    ...
    assert!(
        !carol_revoke.is_success(),
        "Non-owner should not be able to revoke permission"
    );
    ...
    assert!(
        alice_revoke.is_success(),
        "Owner should be able to revoke: {:?}",
        alice_revoke.failures()
    );
...
async fn test_shared_storage_authorization() -> anyhow::Result<()> {
    ...
    let alice_deposit = alice
        ...
        .await?;
    assert!(
        alice_deposit.is_success(),
        "Alice pool deposit should succeed: {:?}",
        alice_deposit.failures()
    );
    ...
    assert!(
        !bob_shares_alice.is_success(),
        "Bob should NOT be able to share Alice's storage"
    );
    ...
    assert!(
        alice_shares.is_success(),
        "Alice should share storage with Bob: {:?}",
        alice_shares.failures()
    );
    ...
    for log in &logs {
        if log.starts_with(EVENT_JSON_PREFIX) && log.contains("share_storage") {
            ...
            assert!(
                log.contains("target_id"),
                "Event should contain target_id field"
            );
            ...
            {
                assert!(
                    data.get("target_id").is_some(),
                    "Event data should have target_id"
                );
    ...
    assert!(
        !charlie_returns_bob.is_success(),
        "Charlie should NOT return Bob's storage"
    );
    ...
    assert!(
        bob_returns.is_success(),
        "Bob should return his own storage: {:?}",
        bob_returns.failures()
    );
...
async fn test_share_storage_edge_cases() -> anyhow::Result<()> {
    ...
    println!(
        "   Share without pool: success={}",
        share_no_pool.is_success()
    );
    println!("   ✓ Handled gracefully (no panic)");
    ...
    if manager_deposit.is_success() {
        ...
        println!(
            "   Share excessive result: success={}",
            share_too_much.is_success()
        );
        println!("   ✓ Handled gracefully (no panic)");
        ...
        println!(
            "   Share to self result: success={}",
            share_to_self.is_success()
        );
        println!("   ✓ Handled gracefully (no panic)");
        ...
        println!(
            "   ⓘ Manager pool deposit failed: {:?}",
            manager_deposit.failures()
        );
    }
...
async fn test_return_shared_storage_edge_cases() -> anyhow::Result<()> {
    ...
    println!(
        "   Return when none shared: success={}",
        return_none.is_success()
    );
    println!("   ✓ Handled gracefully (no panic)");
    ...
    if manager_deposit.is_success() {
        ...
        if share_result.is_success() {
            ...
            println!(
                "   Double return result: success={}",
                double_return.is_success()
            );
            println!("   ✓ Handled gracefully (no panic)");
        } else {
...
async fn test_set_for_with_refund_unused_deposit_to_signer_wallet() -> anyhow::Result<()> {
    ...
    println!(
        "   Bob's wallet before: {} NEAR",
        bob_wallet_before.as_near()
    );
...
async fn test_set_for_cross_account_permission_edge_cases() -> anyhow::Result<()> {
    ...
    assert!(
        ...
        "set_for should fail without permission"
    );
    ...
    assert!(
        correct_path_result.is_success(),
        "set_for to correct path should succeed: {:?}",
        correct_path_result.failures()
    );
    ...
    assert!(
        !third_party_result.is_success(),
        "Charlie should not have permission"
    );
    ...
    assert!(
        !post_revoke_result.is_success(),
        "set_for should fail after revoke"
    );
...
async fn test_set_for_with_group_permissions() -> anyhow::Result<()> {
    ...
    let add_bob_result = alice
        ...
        .await?;
    assert!(
        add_bob_result.is_success(),
        "Add member should succeed: {:?}",
        add_bob_result.failures()
    );
    ...
    let bob_set_result = bob
        ...
        .await?;
    assert!(
        bob_set_result.is_success(),
        "Bob's set() to group should succeed: {:?}",
        bob_set_result.failures()
    );
    ...
    let bob_grant_result = bob
        ...
        .await?;
    assert!(
        bob_grant_result.is_success(),
        "Bob grant to Charlie should succeed"
    );
    ...
    let charlie_set_for_result = charlie
        ...
        .await?;
    assert!(
        charlie_set_for_result.is_success(),
        "Charlie set_for should succeed: {:?}",
        charlie_set_for_result.failures()
    );
    ...
    assert!(
        ...
        "Charlie should not be able to write to group"
    );
...
async fn test_batch_set_for_with_mixed_operations() -> anyhow::Result<()> {
    ...
    let revoke_broad_result = alice
        ...
        .await?;
    println!(
        "   Revoke broad permission result: success={}",
        revoke_broad_result.is_success()
    );
    ...
    assert_ne!(
        status_value, "online",
        "Atomic failure should not write any data"
    );
...
async fn test_set_for_storage_source_priority() -> anyhow::Result<()> {
    ...
    assert!(
        set_for_personal_result.is_success(),
        "set_for with personal deposit should succeed: {:?}",
        set_for_personal_result.failures()
    );
    ...
    let bob_balance = bob_storage
        ...
        .unwrap_or(0);
    println!(
        "   Bob's unused deposit saved to balance: {} yoctoNEAR",
        bob_balance
    );
    ...
    if platform_deposit_result.is_success() {
        ...
        assert!(
            set_for_pool_result.is_success(),
            "set_for with platform pool should succeed: {:?}",
            set_for_pool_result.failures()
        );
        ...
        if let Some(pool) = pool_info {
            println!(
                "   Platform pool balance: {} yoctoNEAR",
                pool.get("storage_balance")
                    .unwrap_or(&serde_json::Value::Null)
            );
        }
    } else {
        println!(
            "   ⓘ Platform pool deposit requires special permissions (expected in some configs)"
        );
    ...
    if shared_pool_deposit_result.is_success() {
        ...
        if share_result.is_success() {
            ...
            assert!(
                set_for_shared_result.is_success(),
                "set_for with shared storage should succeed: {:?}",
                set_for_shared_result.failures()
            );
...
async fn test_root_path_revoke_blocks_access() -> anyhow::Result<()> {
    ...
    let write1_result = bob
        ...
        .await?;
    assert!(
        write1_result.is_success(),
        "First write should succeed: {:?}",
        write1_result.failures()
    );
    ...
    let revoke_result = alice
        ...
        .await?;
    assert!(
        revoke_result.is_success(),
        "Revoke should succeed: {:?}",
        revoke_result.failures()
    );
...
async fn test_update_config_manager_only() -> anyhow::Result<()> {
    ...
    assert!(
        !alice_update_result.is_success(),
        "Non-manager should NOT be able to update config"
    );
    ...
    assert!(
        manager_update_result.is_success(),
        "Manager should be able to update config: {:?}",
        manager_update_result.failures()
    );
    ...
    assert_eq!(
        new_config["max_key_length"].as_u64().unwrap(),
        current_max_key_length + 50
    );
    assert_eq!(
        new_config["max_batch_size"].as_u64().unwrap(),
        current_max_batch_size + 25
    );
    assert_eq!(
        new_config["max_path_depth"].as_u64().unwrap(),
        current_max_path_depth + 2
    );
    ...
    assert!(
        !decrease_result.is_success(),
        "Config values should NOT be decreasable"
    );
...
async fn test_update_config_via_manager_contract() -> anyhow::Result<()> {
    ...
    assert!(
        res.is_success(),
        "Proxy should be able to update config via cross-contract call: {:?}",
        res.failures()
    );
    ...
    let new_config: serde_json::Value =
        core.view("get_config").args_json(json!({})).await?.json()?;
...
async fn test_update_manager_security() -> anyhow::Result<()> {
    ...
    assert!(
        !alice_update_result.is_success(),
        "Non-manager should NOT be able to update manager"
    );
    println!("   ✓ Alice (non-manager) correctly rejected");
    ...
    assert!(
        !no_deposit_result.is_success(),
        "update_manager without deposit should fail"
    );
    ...
    assert!(
        ...
        "Manager should be able to transfer ownership: {:?}",
    ...
    let has_update_manager_event = logs
        .iter()
        .any(|log| log.contains("CONTRACT_UPDATE") && log.contains("update_manager"));
    assert!(
        has_update_manager_event,
        "Expected CONTRACT_UPDATE/update_manager event in logs: {logs:?}"
    );
    ...
    assert!(
        !old_manager_result.is_success(),
        "Old manager should no longer have privileges"
    );
    ...
    assert!(
        alice_config_result.is_success(),
        "New manager should be able to update config: {:?}",
        alice_config_result.failures()
    );
...
async fn test_cancel_join_request_flow() -> anyhow::Result<()> {
    ...
    assert!(
        ...
        "Join request should NOT exist after cancellation"
    );
    ...
    assert_eq!(
        ...
        "Join request counter must not decrement twice"
    );
    ...
    let rejoin_result = bob
        ...
        .await?;
    assert!(
        rejoin_result.is_success(),
        "New join request should succeed: {:?}",
        rejoin_result.failures()
    );
...
async fn test_public_group_self_join_write_only() -> anyhow::Result<()> {
    ...
    let join_write = bob
        ...
        .await?;
    assert!(
        join_write.is_success(),
        "Self-join with WRITE should succeed: {:?}",
        join_write.failures()
    );
    ...
    assert!(
        is_member,
        "Bob should be a member after successful self-join"
    );
...
async fn test_permission_expiration() -> anyhow::Result<()> {
    ...
    let grant_expired = alice
        ...
        .await?;
    assert!(
        grant_expired.is_success(),
        "Grant should succeed: {:?}",
        grant_expired.failures()
    );
    ...
    assert!(
        !expired_write.is_success(),
        "Expired permission should NOT allow writing"
    );
    ...
    let grant_future = alice
        ...
        .await?;
    assert!(
        grant_future.is_success(),
        "Grant should succeed: {:?}",
        grant_future.failures()
    );
    ...
    assert!(
        valid_write.is_success(),
        "Valid permission should allow writing: {:?}",
        valid_write.failures()
    );
    ...
    let grant_permanent = alice
        ...
        .await?;
    assert!(
        grant_permanent.is_success(),
        "Grant should succeed: {:?}",
        grant_permanent.failures()
    );
    ...
    assert!(
        permanent_write.is_success(),
        "Permanent permission should allow writing: {:?}",
        permanent_write.failures()
    );
    ...
    assert!(
        bob_permanent_perm,
        "Permanent permission should return true"
    );
...
async fn test_get_api_comprehensive() -> anyhow::Result<()> {
    ...
    assert!(
        ...
        "Alice write should succeed: {:?}",
        alice_write.failures()
    );
    ...
    assert!(
        single_result[0].get("block_height").is_some(),
        "Should include block_height"
    );
    ...
    assert_eq!(
        multi_result[0].get("value").and_then(|v| v.as_str()),
        Some("Alice")
    );
    assert_eq!(
        multi_result[2].get("value").and_then(|v| v.as_str()),
        Some("dark")
    );
    println!(
        "   ✓ Multiple keys fetch works: {} keys returned",
        multi_result.len()
    );
    ...
    assert_eq!(
        cross_account[0].get("value").and_then(|v| v.as_str()),
        Some("Alice")
    );
    assert_eq!(
        cross_account[1].get("value").and_then(|v| v.as_str()),
        Some("Bob")
    );
    ...
    println!(
        "   ✓ Relative path query returned {} entries",
        relative_result.len()
    );
    ...
    assert_eq!(
        name_entry.get("value").and_then(|v| v.as_str()),
        Some("Alice")
    );
    ...
    assert!(
        ...
        "Empty keys should return empty result"
    );
...
async fn test_blacklisted_user_cannot_write_to_group() -> anyhow::Result<()> {
    ...
    let owner_deposit = owner
        ...
        .await?;
    assert!(
        owner_deposit.is_success(),
        "Storage deposit should succeed: {:?}",
        owner_deposit.failures()
    );
    ...
    let member_storage = member
        ...
        .await?;
    assert!(
        member_storage.is_success(),
        "Member storage deposit should succeed: {:?}",
        member_storage.failures()
    );
    ...
    assert!(
        !blocked_write.is_success(),
        "Blacklisted member should NOT be able to write"
    );
    ...
    assert!(
        !blocked_set_for.is_success(),
        "Blacklisted member should NOT be able to use set_for"
    );
    ...
    let regrant_perm = owner
        ...
        .await?;
    assert!(
        regrant_perm.is_success(),
        "Re-grant permission should succeed"
    );
    ...
    assert!(
        write_again.is_success(),
        "Unblacklisted member should be able to write: {:?}",
        write_again.failures()
    );
...
async fn test_permission_hierarchy_for_set_for() -> anyhow::Result<()> {
    ...
    assert!(
        deposit.is_success(),
        "Alice storage deposit should succeed: {:?}",
        deposit.failures()
    );
    println!("   ✓ Alice deposited storage");
    ...
    assert!(
        bob_write.is_success(),
        "Bob (WRITE) should be able to write: {:?}",
        bob_write.failures()
    );
    ...
    let carol_write = carol
        ...
        .await?;
    assert!(
        carol_write.is_success(),
        "Carol (MODERATE) should be able to write: {:?}",
        carol_write.failures()
    );
    ...
    let dan_write = dan
        ...
        .await?;
    assert!(
        dan_write.is_success(),
        "Dan (MANAGE) should be able to write: {:?}",
        dan_write.failures()
    );
    ...
    assert!(
        entry_exists(&bob_data, &bob_key),
        "Bob's entry should exist"
    );
    ...
    assert!(
        entry_exists(&carol_data, &carol_key),
        "Carol's entry should exist"
    );
    ...
    assert!(
        entry_exists(&dan_data, &dan_key),
        "Dan's entry should exist"
    );
    ...
    assert!(
        !carol_grant_attempt.is_success(),
        "Carol (MODERATE) should NOT be able to grant permissions"
    );
...
async fn test_reserved_ops_blocked_for_cross_account() -> anyhow::Result<()> {
    ...
    assert!(
        ...
        "Alice grant to Bob should succeed: {:?}",
        grant_res.failures()
    );
    ...
    let cross_grant_res = bob
        ...
        .await?;
    assert!(
        !cross_grant_res.is_success(),
        "Cross-account permission/grant should fail"
    );
    ...
    let cross_deposit_res = bob
        ...
        .await?;
    assert!(
        !cross_deposit_res.is_success(),
        "Cross-account storage/deposit should fail"
    );
    ...
    let cross_withdraw_res = bob
        ...
        .await?;
    assert!(
        !cross_withdraw_res.is_success(),
        "Cross-account storage/withdraw should fail"
    );
    ...
    let cross_revoke_res = bob
        ...
        .await?;
    assert!(
        !cross_revoke_res.is_success(),
        "Cross-account permission/revoke should fail"
    );
    ...
    let data_write_res = bob
        ...
        .await?;
    assert!(
        data_write_res.is_success(),
        "Cross-account data write with permission should succeed: {:?}",
        data_write_res.failures()
    );
...
async fn test_mixed_batch_with_reserved_ops_cross_account_fails() -> anyhow::Result<()> {
    ...
    assert!(
        !mixed_batch_res.is_success(),
        "Mixed batch with reserved op should fail atomically"
    );
    ...
    assert!(
        !entry_exists(&check, &bio_key),
        "Data should NOT exist after atomic failure"
    );
...
async fn test_zero_delta_replacement_no_pool_change() -> anyhow::Result<()> {
    ...
    let pool_after_write1: Option<serde_json::Value> =
        contract.view("get_platform_pool").await?.json()?;
    let used_bytes_after_write1 = pool_after_write1
        ...
        .unwrap_or(0);
    println!(
        "   Pool used_bytes after first write: {}",
        used_bytes_after_write1
    );
    ...
    let pool_after_write2: Option<serde_json::Value> =
        contract.view("get_platform_pool").await?.json()?;
    let used_bytes_after_write2 = pool_after_write2
        ...
        .unwrap_or(0);
    println!(
        "   Pool used_bytes after replacement: {}",
        used_bytes_after_write2
    );
...
async fn test_exact_pool_exhaustion_boundary() -> anyhow::Result<()> {
    ...
    let pool_before: Option<serde_json::Value> =
        contract.view("get_platform_pool").await?.json()?;
    let available_before = pool_before
        .as_ref()
    ...
    let pool_after1: Option<serde_json::Value> =
        contract.view("get_platform_pool").await?.json()?;
    let available_after1 = pool_after1
        ...
        .unwrap_or(0);
    println!(
        "   Pool available_bytes after first write: {}",
        available_after1
    );
    ...
    let pool_after2: Option<serde_json::Value> =
        contract.view("get_platform_pool").await?.json()?;
...
async fn test_sequential_writes_same_key_tracker_reset() -> anyhow::Result<()> {
    ...
    for (i, val) in values.iter().enumerate() {
        let write_res = alice
            ...
            .transact()
            .await?;
        assert!(write_res.is_success(), "Write {} should succeed", i + 1);
...
async fn test_mixed_delta_batch_grow_and_shrink_same_tx() -> anyhow::Result<()> {
    ...
    let pool_after_init: Option<serde_json::Value> =
        contract.view("get_platform_pool").await?.json()?;
    ...
    let mixed_res = alice
        ...
        .await?;
    assert!(
        mixed_res.is_success(),
        "Mixed delta batch should succeed: {:?}",
        mixed_res.failures()
    );

    let pool_after_mixed: Option<serde_json::Value> =
        contract.view("get_platform_pool").await?.json()?;
    ...
    println!(
        "   ✓ Both values correctly updated (key1={}B, key2={}B)",
        val1.len(),
        val2.len()
    );
...
async fn test_contract_status_state_machine() -> anyhow::Result<()> {
    ...
    let status: String = contract.view("get_contract_status").await?.json()?;
    ...
    let activate_no_deposit = contract
        ...
        .await?;
    assert!(
        !activate_no_deposit.is_success(),
        "activate_contract without 1 yocto should fail"
    );
    ...
    let double_activate = contract
        ...
        .await?;
    assert!(
        double_activate.is_success(),
        "Double activation should succeed but return false"
    );
    let double_result: bool = double_activate.json()?;
    assert!(
        !double_result,
        "Double activation should return false (no state change)"
    );
    ...
    let readonly_no_deposit = contract
        ...
        .await?;
    assert!(
        !readonly_no_deposit.is_success(),
        "enter_read_only without 1 yocto should fail"
    );
    ...
    let enter_readonly = contract
        ...
        .await?;
    assert!(
        enter_readonly.is_success(),
        "enter_read_only should succeed: {:?}",
        enter_readonly.failures()
    );
    ...
    let double_readonly = contract
        ...
        .await?;
    assert!(
        double_readonly.is_success(),
        "Double enter_read_only should succeed but return false"
    );
    let double_readonly_result: bool = double_readonly.json()?;
    assert!(
        !double_readonly_result,
        "Double enter_read_only should return false"
    );
    ...
    let resume_no_deposit = contract
        ...
        .await?;
    assert!(
        !resume_no_deposit.is_success(),
        "resume_live without 1 yocto should fail"
    );
    ...
    let resume_live = contract
        ...
        .await?;
    assert!(
        resume_live.is_success(),
        "resume_live should succeed: {:?}",
        resume_live.failures()
    );
...
async fn test_invalid_status_transitions_blocked() -> anyhow::Result<()> {
    ...
    let genesis_to_readonly = contract
        ...
        .await?;
    assert!(
        !genesis_to_readonly.is_success(),
        "Genesis → ReadOnly should be blocked"
    );
    let failure_msg = format!("{:?}", genesis_to_readonly.failures());
    assert!(
        failure_msg.contains("Live")
            || failure_msg.contains("transition")
            || failure_msg.contains("Invalid"),
    ...
    assert!(
        live_to_live.is_success(),
        "resume_live from Live should succeed but return false"
    );
    let live_result: bool = live_to_live.json()?;
    assert!(
        !live_result,
        "resume_live from Live should return false (no state change)"
    );
...
async fn test_writes_blocked_in_readonly_mode() -> anyhow::Result<()> {
    ...
    let write_readonly = alice
        ...
        .await?;
    assert!(
        !write_readonly.is_success(),
        "Write should FAIL in ReadOnly mode"
    );
    let failure_msg = format!("{:?}", write_readonly.failures());
    assert!(
        failure_msg.contains("read")
            || failure_msg.contains("Read")
            || failure_msg.contains("ContractReadOnly"),
    ...
    assert!(
        entry_exists(&read_result, &full_key),
        "Read should work in ReadOnly mode"
    );
    ...
    let write_resumed = alice
        ...
        .await?;
    assert!(
        write_resumed.is_success(),
        "Write should succeed after resume_live"
    );
...
async fn test_storage_tracker_full_lifecycle_delta_correctness() -> anyhow::Result<()> {
    ...
    println!(
        "   used_bytes after create: {} (delta: +{})",
        bytes_after_create, delta_create
    );
    ...
    println!(
        "   used_bytes after grow: {} (delta: {:+})",
        bytes_after_grow, delta_grow
    );
    ...
    println!(
        "   used_bytes after shrink: {} (delta: {:+})",
        bytes_after_shrink, delta_shrink
    );
    assert!(
        delta_shrink < 0,
        "Shrink should decrease bytes (bytes_released path)"
    );
    ...
    println!(
        "   used_bytes after delete: {} (delta: {:+})",
        bytes_after_delete, delta_delete
    );

    // Soft delete may keep tombstone bytes, but should be less than shrink state
    assert!(delta_delete <= 0, "Delete should not increase bytes");
    ...
    println!(
        "\n   Final overhead above baseline: {} bytes",
        final_overhead
    );
...
async fn test_batch_size_limit_enforcement_rejects_oversized_batch() -> anyhow::Result<()> {
    ...
    println!(
        "   Attempting batch with {} operations...",
        oversized_batch.len()
    );
    ...
    println!(
        "\n   Verifying batch at exact limit ({} operations)...",
        max_batch_size
    );
    ...
    println!(
        "   ✓ Batch at exact limit ({} operations) succeeded",
        max_batch_size
    );
...
async fn test_mixed_operation_types_in_single_batch() -> anyhow::Result<()> {
    ...
    assert!(
        entry_exists(&data, &name_key),
        "Profile name should be stored"
    );
    assert_eq!(
    ...
    assert!(
        entry_exists(&data, &bio_key),
        "Profile bio should be stored"
    );
    assert_eq!(
...
async fn test_batch_operation_failure_is_atomic() -> anyhow::Result<()> {
    ...
    assert!(!exists, "Failed batch should not write partial state");
...
/// 4. CONTRACT_UPDATE/update_config event emitted
#[tokio::test]
async fn test_patch_config_partial_updates() -> anyhow::Result<()> {
    println!("\n🔧 Test: update_config partial updates...\n");
    ...
    let initial_onboarding = initial_config["platform_onboarding_bytes"]
        .as_u64()
        .unwrap();

    // Step 1: Update only max_key_length (increase)
    println!("   Step 1: Update only max_key_length (increase)...");
    let result = contract
        .call("update_config")
        .args_json(json!({
            "update": {
    ...
    let logs = result.logs();
    assert!(
        has_contract_update_config_partial_event(&logs),
        "Expected CONTRACT_UPDATE/update_config event: {:?}",
        logs
    );
    println!("   ✓ update_config succeeded with event");
    ...
    assert_eq!(
        config_after["max_key_length"].as_u64().unwrap(),
        initial_max_key + 100
    );
    assert_eq!(
        config_after["platform_onboarding_bytes"].as_u64().unwrap(),
        initial_onboarding
    );
    ...
    assert!(
        !decrease_result.is_success(),
        "Decreasing safety limit should fail"
    );
    ...
    assert!(
        increase_allowance.is_success(),
        "Increasing platform_onboarding_bytes should succeed: {:?}",
        increase_allowance.failures()
    );
    ...
    let config_after_increase: serde_json::Value = contract
        ...
        .json()?;
    assert_eq!(
        config_after_increase["platform_onboarding_bytes"]
            .as_u64()
            .unwrap(),
    ...
    assert!(
        decrease_allowance.is_success(),
        "Decreasing platform_onboarding_bytes back to minimum should succeed: {:?}",
        decrease_allowance.failures()
    );
    ...
    let final_config: serde_json::Value = contract
        ...
        .json()?;
    assert_eq!(
        final_config["platform_onboarding_bytes"].as_u64().unwrap(),
        initial_onboarding
    );
    ...
    assert!(
        !below_min.is_success(),
        "Decreasing platform_onboarding_bytes below minimum should fail"
    );
    println!("   ✓ Decrease below minimum correctly rejected");

    println!("\n✅ Test passed: update_config partial updates");
    Ok(())
}

/// Test: update_config rejects zero safety limits (partial)
#[tokio::test]
async fn test_patch_config_rejects_zero_safety_limits() -> anyhow::Result<()> {
    ...
    assert!(
        !result.is_success(),
        "Zero max_batch_size should be rejected"
    );
...
async fn test_update_config_rejects_zero_safety_limits() -> anyhow::Result<()> {
    ...
    assert!(
        !result.is_success(),
        "Zero max_path_depth should be rejected"
    );
    println!("   ✓ Zero value correctly rejected");
...
async fn test_admin_config_apis_blocked_in_readonly_mode() -> anyhow::Result<()> {
    ...
    // =========================================================================
    // TEST 1: update_config fails in ReadOnly mode
    // =========================================================================
    println!("   Step 1: update_config in ReadOnly mode (should FAIL)...");
    let update_result = contract
        .call("update_config")
        .args_json(json!({
            "update": {
                "max_batch_size": current_max_batch + 10
            }
        }))
        .deposit(NearToken::from_yoctonear(1))
        .gas(Gas::from_tgas(50))
    ...
    assert!(
        ...
        "update_config should fail in ReadOnly mode"
    );
    let failure_msg = format!("{:?}", update_result.failures());
    assert!(
        failure_msg.contains("Live")
            || failure_msg.contains("ContractReadOnly")
            || failure_msg.contains("read"),
        "Error should mention Live state requirement, got: {}",
        failure_msg
    );
    ...
    assert!(
        !update_mgr_result.is_success(),
        "update_manager should fail in ReadOnly mode"
    );
    println!("   ✓ update_manager correctly rejected in ReadOnly mode");

    // =========================================================================
    // TEST 5: Verify config unchanged (state integrity)
    // =========================================================================
    println!("\n   Step 5: Verify config unchanged after all rejections...");
    let config_after: serde_json::Value = contract
        .view("get_config")
        .args_json(json!({}))
        .await?
    ...
    // =========================================================================
    // TEST 6: Operations succeed after resume_live
    // =========================================================================
    println!("\n   Step 6: resume_live and verify operations work...");
    let resume = contract
        .call("resume_live")
        .deposit(NearToken::from_yoctonear(1))
        .transact()
    ...
    let update_after_resume = contract
        .call("update_config")
        .args_json(json!({ "update": { "max_batch_size": current_max_batch + 10 } }))
        .deposit(NearToken::from_yoctonear(1))
        .gas(Gas::from_tgas(50))
        .transact()
        .await?;
    assert!(
        update_after_resume.is_success(),
        "update_config should succeed after resume_live: {:?}",
        update_after_resume.failures()
    );
    println!("   ✓ update_config succeeds after resume_live");
...
async fn test_patch_config_and_intents_executor_require_manager() -> anyhow::Result<()> {
    ...
    // =========================================================================
    println!("   Step 1: Non-manager (Alice) tries update_config (should FAIL)...");
    let alice_update = alice
        .call(contract.id(), "update_config")
        .args_json(json!({ "update": { "max_batch_size": 50 } }))
        .deposit(NearToken::from_yoctonear(1))
        .gas(Gas::from_tgas(50))
        .transact()
        .await?;
    ...
    assert!(
        !alice_update.is_success(),
        "Non-manager should NOT be able to update_config"
    );
...
async fn test_status_transition_events_emitted() -> anyhow::Result<()> {
    ...
    let has_activate_event = logs
        .iter()
        .any(|log| log.contains("CONTRACT_UPDATE") && log.contains("activate_contract"));
    assert!(
        has_activate_event,
        "Expected CONTRACT_UPDATE/activate_contract event: {:?}",
        logs
    );

    // Verify event contains previous and new status
    let has_status_fields = logs
        .iter()
        .any(|log| log.contains("Genesis") && log.contains("Live"));
    assert!(
        has_status_fields,
        "Event should contain previous (Genesis) and new (Live) status: {:?}",
        logs
    );
    ...
    assert!(
        enter_ro_result.is_success(),
        "enter_read_only should succeed"
    );
    let logs = enter_ro_result.logs();
    let has_enter_ro_event = logs
        .iter()
        .any(|log| log.contains("CONTRACT_UPDATE") && log.contains("enter_read_only"));
    assert!(
        has_enter_ro_event,
        "Expected CONTRACT_UPDATE/enter_read_only event: {:?}",
        logs
    );

    let has_status_fields = logs
        .iter()
        .any(|log| log.contains("Live") && log.contains("ReadOnly"));
    assert!(
        has_status_fields,
        "Event should contain previous (Live) and new (ReadOnly) status: {:?}",
        logs
    );
    ...
    let has_resume_event = logs
        .iter()
        .any(|log| log.contains("CONTRACT_UPDATE") && log.contains("resume_live"));
    assert!(
        has_resume_event,
        "Expected CONTRACT_UPDATE/resume_live event: {:?}",
        logs
    );

    let has_status_fields = logs
        .iter()
        .any(|log| log.contains("ReadOnly") && log.contains("Live"));
    assert!(
        has_status_fields,
        "Event should contain previous (ReadOnly) and new (Live) status: {:?}",
        logs
    );
...
async fn test_update_manager_event_fields() -> anyhow::Result<()> {
    ...
    let has_update_event = logs
        .iter()
        .any(|log| log.contains("CONTRACT_UPDATE") && log.contains("update_manager"));
    assert!(
        has_update_event,
        "Expected CONTRACT_UPDATE/update_manager event: {:?}",
        logs
    );

    // Check for old_manager field
    let has_old_manager = logs
        .iter()
        .any(|log| log.contains("old_manager") && log.contains(&old_manager_id));
    assert!(
        has_old_manager,
        "Event should contain old_manager field with contract ID: {:?}",
        logs
    );

    // Check for new_manager field
    let has_new_manager = logs
        .iter()
        .any(|log| log.contains("new_manager") && log.contains(new_manager.id().as_str()));
    assert!(
        has_new_manager,
        "Event should contain new_manager field: {:?}",
        logs
    );
    ...
    let new_mgr_update = new_manager
        .call(contract.id(), "update_config")
        .args_json(json!({ "update": { "max_batch_size": 20 } }))
        .deposit(NearToken::from_yoctonear(1))
        .gas(Gas::from_tgas(50))
        .transact()
        .await?;
    ...
    assert!(
        new_mgr_update.is_success(),
        "New manager should be able to update_config: {:?}",
        new_mgr_update.failures()
    );
    println!("   ✓ New manager can successfully call update_config");

    // =========================================================================
    // TEST 3: Old manager can no longer use admin functions
    // =========================================================================
    println!("\n   Step 3: Old manager (contract) cannot use admin functions...");
    let old_mgr_update = contract
        .call("update_config")
        .args_json(json!({ "update": { "max_batch_size": 30 } }))
        .deposit(NearToken::from_yoctonear(1))
        .gas(Gas::from_tgas(50))
        .transact()
        .await?;
    ...
    assert!(
        !old_mgr_update.is_success(),
        "Old manager should NOT be able to update_config after transfer"
    );
