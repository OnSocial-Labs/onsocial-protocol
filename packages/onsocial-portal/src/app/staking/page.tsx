...
const LOCK_PERIODS = [
	{ months: 1, bonus: 5, label: '1 Month', color: '#6B7280' },
	{ months: 6, bonus: 10, label: '6 Months', color: '#60A5FA' },
	{ months: 12, bonus: 20, label: '12 Months', color: '#4ADE80', popular: true },
	{ months: 24, bonus: 35, label: '24 Months', color: '#C084FC' },
	{ months: 48, bonus: 50, label: '48 Months', color: '#FBBF24' },
]
						...
						<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2">
							{LOCK_PERIODS.map((lp, index) => (
								...
								>
									...
									<div className="text-lg font-bold" style={{ color: lp.color }}>+{lp.bonus}%</div>
								</button>
							))}
						</div>
					</div>
				...
				>
					<div className="border border-border/50 rounded-2xl p-6 bg-muted/30">
						...
						<p className="text-xs text-muted-foreground">
							0.2% of the scheduled pool releases weekly. Your share = your stake-seconds ÷ total. Rewards never run out abruptly.
						</p>
					</div>
					<div className="border border-border/50 rounded-2xl p-6 bg-muted/30">
					...
					<div className="border border-border/50 rounded-2xl p-6 bg-muted/30">
						...
						<p className="text-xs text-muted-foreground">
							All logic runs on <span className="font-mono text-foreground/70">staking.onsocial.near</span>. Auto-registered on first stake — no separate storage deposit.
						</p>
					</div>
				</motion.div>
			</div>
