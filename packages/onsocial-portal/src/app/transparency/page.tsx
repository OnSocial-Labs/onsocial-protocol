...
const TOKEN_UTILITY = [
	{ icon: Lock, label: 'Stake for rewards', desc: 'Lock tokens, earn pro-rata staking rewards at 0.2%/week' },
	{ icon: Key, label: 'Buy API credits', desc: '60% funds infrastructure, 40% flows back to staking rewards' },
	{ icon: Gift, label: 'Earn from dapps', desc: 'Dapps reward users with $SOCIAL for engagement' },
	{ icon: Layers, label: 'Governance', desc: 'Staked tokens provide voting power for protocol decisions' },
]
			...
			<div className="container mx-auto px-4 max-w-4xl">
				...
				>
					...
					<p className="text-lg text-muted-foreground max-w-xl mx-auto">
						1 billion $SOCIAL tokens. Fixed supply. No inflation. Every allocation is on-chain and verifiable.
					</p>
				</motion.div>
					...
					>
						<div className="flex rounded-full overflow-hidden h-4 mb-8 gap-[1px] bg-border/30">
							{TOKEN_DISTRIBUTION.map((d) => (
								<div
									key={d.label}
									style={{ width: `${d.pct}%`, backgroundColor: d.color, minWidth: '8px' }}
							...
							return (
								<div key={d.label} className="flex items-center justify-between text-sm">
									...
									{isOnChain ? (
										...
										<span className="font-mono text-xs text-muted-foreground">{d.note}</span>
									)}
								</div>
							)
						})}
