...
| Want to…                                    | Call                                                         |
| ------------------------------------------- | ------------------------------------------------------------ |
| Update my profile                           | `os.profiles.update({ name, bio, kind, avatar, banner, … })` |
...
| Buy from collection      | `os.scarces.collections.purchaseFrom(...)` _(wallet + value deposit)_        |
| List on market           | `os.scarces.market.sell({ tokenId, priceNear })`                             |
| Buy listed scarce        | `os.scarces.market.purchase(tokenId, { depositYocto })` _(wallet)_           |
| Auction                  | `os.scarces.auctions.start(...)` / `.placeBid(...)` / `.settle(...)`         |
| Make / accept offer      | `os.scarces.offers.make(...)` / `.accept(...)`                               |
| Lazy list / buy          | `os.scarces.lazy.create(...)` / `.purchase(id, { depositYocto })` _(wallet)_ |
| List from a post         | `os.scarces.fromPost.list(post, priceNear)` _(wallet)_                       |
