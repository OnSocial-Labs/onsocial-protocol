// Emits the rewards parity fixture consumed by
// `contracts/rewards-onsocial/src/sdk_parity_test.rs`.
...
describe('SDK ↔ rewards parity fixture export', () => {
  it('writes deterministic JSON for the Rust round-trip test', () => {
    const cases = getRewardsParityCases('testnet').map((c) => ({
      name: c.name,
    ...
    const payload = {
      ...
      generated_by:
        'packages/onsocial-sdk/src/advanced/dump-rewards-parity.test.ts',
      network: 'testnet',
      contract: 'rewards-onsocial',
      cases,
    };
