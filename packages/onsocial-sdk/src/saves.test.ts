...
function makeQuery() {
  const getSaves = vi.fn().mockResolvedValue([
    {
  ...
  return {
    spies: { getSaves },
    mod: { getSaves } as unknown as QueryModule,
  };
}
...
describe('SavesModule.list', () => {
  it('forwards viewer / limit / offset to query.getSaves', async () => {
    ...
    expect(spies.getSaves).toHaveBeenCalledWith('alice.near', {
      limit: 10,
      offset: 5,
    });
