...
// for writes + `os.query.getEndorsementsGiven` / `getEndorsementsReceived`
...
export class EndorsementsModule {
  ...
  async listGiven(
    ...
    const rows = await this._query.getEndorsementsGiven(issuer, opts);
    return rows.map((r) => ({
  ...
  async listReceived(
    ...
    const rows = await this._query.getEndorsementsReceived(target, opts);
    return rows.map((r) => ({
      ...parseEndorsementValue(r.value, r.issuer, r.target),
      blockHeight: r.blockHeight,
      blockTimestamp: r.blockTimestamp,
