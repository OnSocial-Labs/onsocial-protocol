...
function makeMod() {
  ...
  const issued = vi.fn().mockResolvedValue([
    {
      issuer: 'alice.near',
      subject: 'bob.near',
      claimType: 'skill',
  ...
  const about = vi.fn().mockResolvedValue([
    {
      issuer: 'alice.near',
      subject: 'bob.near',
      claimType: 'skill',
...
describe('AttestationsModule', () => {
  ...
  it('listIssued passes type filter through', async () => {
    ...
    expect(spies.issued).toHaveBeenCalledWith('alice.near', {
      claimType: 'skill',
      limit: 5,
    });
  });
  ...
  it('listAbout survives unparseable values', async () => {
    const { mod, spies } = makeMod();
    spies.about.mockResolvedValueOnce([
      {
        issuer: 'i.near',
        subject: 's.near',
        claimType: 't',
