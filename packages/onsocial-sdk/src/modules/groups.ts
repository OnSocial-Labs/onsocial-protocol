...
export class GroupsModule {
  ...
  // ── Permission helpers ─────────────────────────────────────────────────
  // Convenience wrappers — these belong on `os.groups` semantically; the
  // raw endpoints also remain on `os.permissions.hasGroupAdmin/Moderate`.

  /** True if `userId` has admin rights inside `groupId`. */
  async isAdmin(groupId: string, userId: string): Promise<boolean> {
    const p = new URLSearchParams({ groupId, userId });
    return this._http.get<boolean>(`/data/has-group-admin?${p}`);
  }

  /** True if `userId` may moderate content in `groupId`. */
  async canModerate(groupId: string, userId: string): Promise<boolean> {
    const p = new URLSearchParams({ groupId, userId });
