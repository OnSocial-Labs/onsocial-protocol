...
// `os.social.setProfile` and `os.query.profile` so app devs have one
...
//   • `os.query.profile()` returns one row per field — every consumer has
//     to fold it into an object themselves, and the JSON-encoded
//     `links` / `tags` columns trip people up.
...
export class ProfilesModule {
  ...
  async get(accountId: string): Promise<MaterialisedProfile | null> {
    const res = await this._query.profile(accountId);
    const rows = (res.data?.profilesCurrent ?? []) as ProfileRow[];
