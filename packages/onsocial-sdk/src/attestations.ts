...
// writes + `os.query.getClaimsIssued` / `getClaimsAbout` for materialised
...
export class AttestationsModule {
  ...
  async listIssued(
    ...
    const rows = await this._query.getClaimsIssued(issuer, {
  ...
  async listAbout(
    ...
    const rows = await this._query.getClaimsAbout(subject, {
      ...(opts.type ? { claimType: opts.type } : {}),
      ...(opts.limit != null ? { limit: opts.limit } : {}),
      ...(opts.offset != null ? { offset: opts.offset } : {}),
    });
