...
// Multer: in-memory storage, 1 GB max (same as /storage/upload)
const upload = multer({
...
// ---------------------------------------------------------------------------
composeRouter.post('/set', upload.array('files', 50), async (req: Request, res: Response) => {
  ...
  try {
    ...
    const result = await composeSet(accountId, { path, value, mediaField, targetAccount }, files);
...
//
// multipart/form-data:
//   Fields: title (required), description, price, copies, collectionId, extra (JSON)
//   Files:  image (single file)
//
// Example:
//   FormData: title="My Art", description="A sunset", price="1000000000000000000000000", image=<file>
// ---------------------------------------------------------------------------
composeRouter.post('/mint', upload.single('image'), async (req: Request, res: Response) => {
  const accountId = req.auth!.accountId;

  try {
    const { title, description, price, copies, collectionId, extra, targetAccount } = req.body;
    ...
    res.status(201).json({
      txHash: result.txHash,
      media: result.media
        ? { cid: result.media.cid, url: result.media.url, size: result.media.size, hash: result.media.hash }
        : undefined,
      metadata: result.metadata
        ? { cid: result.metadata.cid, url: result.metadata.url, size: result.metadata.size }
        : undefined,
    });
  } catch (error) {
    if (error instanceof ComposeError) {
    ...
    logger.error({ error, accountId: req.auth!.accountId }, 'Compose mint failed');
