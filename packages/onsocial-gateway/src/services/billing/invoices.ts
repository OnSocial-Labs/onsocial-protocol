...
async function toInvoice(
  ...
  const billingLine1 = normalizeAddressLine(input.billingLine1);
  const billingCity = normalizeAddressLine(input.billingCity, 80);
...
class PostgresInvoiceStore implements InvoiceStore {
  ...
  private mapRow(row: Record<string, unknown>): InvoiceRecord {
    return {
      ...
      billingLine1: (row.billing_line1 as string) || null,
