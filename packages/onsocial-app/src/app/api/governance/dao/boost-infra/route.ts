...
import { loadProtocolDaoBoostInfraContext } from '@/lib/protocol-dao-boost-infra';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const context = await loadProtocolDaoBoostInfraContext(daoAccountId);
    ...
    const detail =
      error instanceof Error
        ? error.message
        : 'Boost infra context unavailable';
