...
function readDaoAccountId(request: NextRequest): string {
  const daoAccountId =
    request.nextUrl.searchParams.get('daoAccountId')?.trim().toLowerCase() ||
    GOVERNANCE_DAO_ACCOUNT;
...
/** Proxy backend governance SSE (same shape as portal). */
export async function GET(request: NextRequest) {
  try {
    ...
    const message =
      error instanceof Error ? error.message : 'Failed to open event stream';
