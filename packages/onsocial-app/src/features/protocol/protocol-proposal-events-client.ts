...
/** Shared EventSource for Protocol board live proposal patches (portal pattern). */
export function ensureProtocolProposalEventSource(
  ...
  const normalized = daoAccountId.trim().toLowerCase();
  if (!normalized) return;

  if (sharedSource && sharedDaoAccountId === normalized) {
    return;
  }

  sharedSource?.close();
  sharedSource = null;
  sharedDaoAccountId = normalized;

  const search = new URLSearchParams({ daoAccountId: normalized });
...
export function subscribeProtocolProposalUpdates(
...
export function closeProtocolProposalEventSource(): void {
  sharedSource?.close();
  sharedSource = null;
