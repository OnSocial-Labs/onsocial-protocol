...
interface ChoiceDrawerProps<T extends string> {
  open: boolean;
  onClose: () => void;
  onClosed?: () => void;
  label: string;
...
/**
 * Content-hugging pick-one sheet — same open/spacing as the scarce list
 * choice drawers. Use with a chip (`ChoiceDrawerField`) or pill trigger
 * (`ChoiceDrawerMenu`).
 */
...
interface ChoiceDrawerFieldProps<T extends string> {
...
interface ChoiceDrawerMenuProps<T extends string> {
  label: string;
  value: T;
  options: readonly ChoiceOption<T>[];
  onChange: (next: T) => void;
...
/**
 * Toolbar pill that opens the shared choice drawer — used for standing
 * view switcher, market sort, and other pick-one menus that used to be
 * anchored floating panels on mobile.
 */
