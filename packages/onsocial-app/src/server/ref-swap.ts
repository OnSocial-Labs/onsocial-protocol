...
import type { AppSwapQuoteDetails } from '@/lib/app-swap-quote';
import { getSpendableNearBalance, viewAccount } from '@/lib/app-near-rpc';
import { resolveConfiguredNearRpcUrl } from '@onsocial/rpc';

import {
  APP_SWAP_DIRECT_POOL_IDS,
  APP_SWAP_SLIPPAGE_PERCENT,
  APP_SWAP_SOCIAL_TOKEN_ID,
  appSwapDirectPoolId,
  USDC_MAINNET_TOKEN_ID,
  type AppSwapInputKind,
} from '@/lib/app-swap-config';

interface AppSwapPools {
  simplePools: Pool[];
...
const appSwapDirectPoolIds = new Set<number>(APP_SWAP_DIRECT_POOL_IDS);
...
function selectAppSwapRoute(
  ...
  const preferredPools = directPools.filter((pool) =>
    appSwapDirectPoolIds.has(pool.id)
  );
...
let poolsCache: AppSwapPools | null = null;
let poolsLoadPromise: Promise<AppSwapPools> | null = null;
...
async function fetchAppSwapPools(): Promise<AppSwapPools> {
  ensureRefEnv();
...
): Promise<AppSwapPools> {
  const cached = await loadPools();
  const directPoolId = appSwapDirectPoolId(kind);
...
async function resolveSocialToken(): Promise<TokenMetadata> {
  ensureRefEnv();
  return ftGetTokenMetadata(APP_SWAP_SOCIAL_TOKEN_ID);
}

export async function getAppSwapTokens(): Promise<{
  tokenIn: TokenMetadata;
  tokenOut: TokenMetadata;
}> {
...
export async function getAppSwapInputBalance(
  accountId: string,
  kind: AppSwapInputKind
): Promise<string> {
...
export async function getAppSwapAccountBalances(
  accountId: string,
  kind: AppSwapInputKind
): Promise<{
  ...
  const [nearAccount, socialBalanceYocto, usdcBalanceYocto] = await Promise.all(
    [
      ...
      ftGetBalance(APP_SWAP_SOCIAL_TOKEN_ID, accountId).then((value) =>
        String(value ?? '0')
      ),
...
function buildAppSwapQuote(input: {
  trimmed: string;
  tokenIn: TokenMetadata;
  tokenOut: TokenMetadata;
  ...
  return {
    ...
    minReceived: amountOut
      ? percentLess(APP_SWAP_SLIPPAGE_PERCENT, amountOut)
      : '0',
    priceImpactPercent,
    ...
    slippagePercent: APP_SWAP_SLIPPAGE_PERCENT,
    tokenInSymbol: tokenIn.symbol,
    tokenOutSymbol: tokenOut.symbol,
  };
}

async function estimateAppSwapWithPools(
  input: {
    kind: AppSwapInputKind;
    amountIn: string;
  },
  ...
  const route = selectAppSwapRoute(tokenIn, tokenOut, pools);
...
export async function estimateAppSwap(input: {
  kind: AppSwapInputKind;
  amountIn: string;
}): Promise<{
  tokenIn: TokenMetadata;
  ...
  return estimateAppSwapWithPools(input, pools);
}

export async function prepareAppSwapTransactions(input: {
  kind: AppSwapInputKind;
  amountIn: string;
  accountId: string;
}): Promise<Transaction[]> {
  ...
  const { tokenIn, tokenOut, amountInYocto, estimates } =
    await estimateAppSwapWithPools(
      {
        kind: input.kind,
        amountIn: trimmed,
      },
  ...
  const transactions = await instantSwap({
    ...
    tokenOut,
    amountIn: trimmed,
    slippageTolerance: APP_SWAP_SLIPPAGE_PERCENT,
    swapTodos: estimates,
