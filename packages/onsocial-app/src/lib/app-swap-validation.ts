import {
  APP_SWAP_NEAR_GAS_RESERVE_YOCTO,
  APP_SWAP_WNEAR_STORAGE_BUFFER_YOCTO,
...
export type AppSwapHint =
  | 'insufficient-input'
  | 'gas-usdc-path'
  | 'gas-near-input'
  | 'wnear-storage';

export interface AppSwapValidationInput {
  tokenIn: AppSwapInputKind;
  amountIn: string;
...
export interface AppSwapValidationResult {
  canSwap: boolean;
  hint: AppSwapHint | null;
...
export function appSwapHintMessage(hint: AppSwapHint): string {
  switch (hint) {
...
export function evaluateAppSwapValidation(
  input: AppSwapValidationInput
): AppSwapValidationResult {
  const {
    tokenIn,
    amountIn,
    inputBalanceYocto,
  ...
  const gasReserve = APP_SWAP_NEAR_GAS_RESERVE_YOCTO;
  ...
  let hint: AppSwapHint | null = null;
