...
export interface ProtocolDaoManagedContract {
  contractId: string;
  label: string;
...
interface ProtocolDaoManagedContractDefinition {
  contractId: string;
  label: string;
...
const PROTOCOL_DAO_MANAGED_CONTRACT_DEFINITIONS: readonly ProtocolDaoManagedContractDefinition[] =
  [
    {
...
async function readContractOwner(
  definition: ProtocolDaoManagedContractDefinition
): Promise<string | null> {
  try {
    const result = await viewNearContract<unknown>(
...
function toManagedContract(
  ...
  return {
    ...
    upgradable: definition.upgradable,
  };
}

export async function loadProtocolDaoManagedContracts(
  daoAccountId: string
  ...
  const matches = await Promise.all(
    PROTOCOL_DAO_MANAGED_CONTRACT_DEFINITIONS.map(async (definition) => {
  ...
  return matches
    .filter(
      (contract): contract is ProtocolDaoManagedContract => contract != null
    )
