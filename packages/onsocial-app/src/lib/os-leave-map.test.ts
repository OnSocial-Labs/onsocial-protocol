...
describe('os leave map', () => {
  ...
  it('sends matching indexes to Home', () => {
  ...
  it('flags Drops index leave to Market as a decision, not a silent match', () => {
    expect(row('drops-index')).toMatchObject({
      dockBack: true,
      parent: APP_MARKET_PATH,
      modelParent: OS_INDEX_LEAVE_HREF,
      verdict: 'needs-decision',
    });
    expect(osLeaveRowsByVerdict('needs-decision').map((entry) => entry.id)).toEqual(
      ['drops-index']
    );
  });
  ...
  it('lists loading-parent mismatches without changing them', () => {
