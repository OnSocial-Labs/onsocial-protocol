...
/** Matches `.app-tx-toast` entrance — re-check hover after the slide settles. */
...
export function useToastDismissTimer({
  ...
  useEffect(() => {
    ...
    // Immediate + post-entrance revalidate: CSS slide-in under the cursor
    // does not fire pointerenter, and :hover is often false mid-transform.
    applyHoverFromDom();
    const host = hostRef.current;
  ...
  const pauseProps = useMemo(
    () => ({
      ...
      // Mouse aliases — some environments under-deliver pointerenter on portals.
