...
import {
  APP_SWAP_BALANCE_REFRESH_MS,
  APP_SWAP_ENABLED,
  APP_SWAP_QUOTE_REFRESH_MS,
...
export function useAppSwap(accountId: string | null) {
  const [tokenIn, setTokenIn] = useState<AppSwapInputKind>('near');
  const [amountIn, setAmountIn] = useState('');
  const [amountOut, setAmountOut] = useState('');
  const [quote, setQuote] = useState<AppSwapQuoteDetails | null>(null);
  ...
  useEffect(() => {
    setAmountIn((current) => {
      ...
      return finalizeAmountInput(current, appSwapAmountMaxDecimals(tokenIn));
    });
  }, [tokenIn]);

  const applyInputBalanceForToken = useCallback(
    (kind: AppSwapInputKind, near: string | null, usdc: string | null) => {
  ...
  useEffect(() => {
    if (!accountId || !APP_SWAP_ENABLED) return;
  ...
  useEffect(() => {
    ...
    const trimmed = amountIn.trim();
    if (!APP_SWAP_ENABLED || !trimmed || Number(trimmed) <= 0) {
      return;
    }
  ...
  > => {
    const trimmed = finalizeAmountInput(
      amountIn,
      appSwapAmountMaxDecimals(tokenIn)
    ).trim();
    if (!trimmed || Number(trimmed) <= 0) {
      throw new Error('Enter a valid amount.');
    }
    if (!accountId) {
      throw new Error(CONNECT_CONTINUE_ERROR);
    }
    if (!APP_SWAP_ENABLED) {
  ...
  const validation = evaluateAppSwapValidation({
    tokenIn,
    amountIn,
    ...
    swapping,
    accountId,
    enabled: APP_SWAP_ENABLED && !loadingPools,
  });
