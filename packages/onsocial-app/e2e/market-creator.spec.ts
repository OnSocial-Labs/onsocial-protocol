...
test.describe('market creator shop', () => {
  test('empty creator shop is a door, not a listings-only miss', async ({
    ...
    await expect(
      page.locator('[data-market-creator-shop]').first()
    ...
    await expect(
      page.getByRole('button', { name: /Open filter menu/ })
    ).toHaveCount(0);
    await expect(
      page.getByRole('button', { name: /Open sort menu/ })
    ).toHaveCount(0);
  });
  ...
  test('SSR catalog hit paints Night Drive without the shop skeleton', async ({
    ...
    await expect(
      page.locator('[data-market-creator-group="live"]')
    ).toBeVisible();
    await expect(
      page.locator('[data-market-creator-group="past"]')
    ).toBeVisible();
  });
