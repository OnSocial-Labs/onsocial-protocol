  ...
  test.describe('visitor writing at 390', () => {
    ...
    test('visitor writing drop opens Read under commerce', async ({
      ...
      const insets = await sheet.evaluate((root) => {
        const title = root.querySelector('.collection-writing-work-title');
    ...
    test('visitor writing cover opens the reader, not artwork zoom', async ({
      ...
      for (let attempt = 0; attempt < 2; attempt += 1) {
        ...
        try {
          ...
          await expect(
            sheet.locator('.collection-writing-work-title')
          ).toBeInViewport();
          await expect
            .poll(async () =>
              sheet.evaluate((el) => {
      ...
      await expect(sheet.locator('.os-media-face-title')).toHaveCount(0);
      await expect(sheet.locator('.collection-writing-work-title')).toHaveText(
        'Chapter One'
      );
