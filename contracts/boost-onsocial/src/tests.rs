//! Unit tests for SOCIAL boost contract (boost-seconds model)
//!
//! Tests cover:
//! - Initialization
//! - Storage (NEP-145)
...
//! - Effective boost calculations
//! - Boost-seconds accumulation
//! - Reward release under fixed and stepped schedules
...
const TEST_FIXED_WEEKLY_RATE_BPS: u128 = DEFAULT_MAX_WEEKLY_RATE_BPS as u128;
const TEST_TOKEN_ID: &str = "token.onsocial.near";
...
fn setup_contract() -> OnsocialBoost {
    let mut context = get_context("owner.near");
    context.block_timestamp(1_000_000_000_000_000_000); // 1 second in ns
    testing_env!(context.build());

    OnsocialBoost::new_with_schedule(BoostInitConfig {
        token_id: TEST_TOKEN_ID.parse().unwrap(),
        owner_id: "owner.near".parse().unwrap(),
        release_schedule_start_ns: 1_000_000_000_000_000_000,
        initial_weekly_rate_bps: DEFAULT_MAX_WEEKLY_RATE_BPS,
        rate_step_bps: 0,
...
fn setup_default_contract() -> OnsocialBoost {
    let mut context = get_context("owner.near");
    context.block_timestamp(1_000_000_000_000_000_000);
    testing_env!(context.build());

    OnsocialBoost::new(
        TEST_TOKEN_ID.parse().unwrap(),
        "owner.near".parse().unwrap(),
    )
}

fn setup_stepped_contract(
    ...
    OnsocialBoost::new_with_schedule(BoostInitConfig {
        ...
        owner_id: "owner.near".parse().unwrap(),
        release_schedule_start_ns: 1_000_000_000_000_000_000,
        initial_weekly_rate_bps,
        rate_step_bps,
        rate_step_interval_months,
        max_weekly_rate_bps,
    })
}

fn setup_with_storage(contract: &mut OnsocialBoost, account: &str) {
...
fn lock_tokens(contract: &mut OnsocialBoost, sender: &str, amount: u128, months: u64) {
    let mut context = get_context(TEST_TOKEN_ID);
    context.block_timestamp(1_000_000_000_000_000_000);
    testing_env!(context.build());
...
fn fund_pool(contract: &mut OnsocialBoost, amount: u128) {
...
fn fund_pool_at(contract: &mut OnsocialBoost, amount: u128, timestamp: u64) {
    let mut context = get_context(TEST_TOKEN_ID);
    context.block_timestamp(timestamp);
    testing_env!(context.build());
...
fn test_init() {
    ...
    assert_eq!(contract.total_boost_seconds, 0);
...
#[test]
fn test_default_schedule_starts_at_initial_rate() {
    let contract = setup_default_contract();
    let stats = contract.get_stats();

    assert_eq!(
        stats.initial_weekly_rate_bps,
        DEFAULT_INITIAL_WEEKLY_RATE_BPS as u32
    );
    assert_eq!(
        stats.active_weekly_rate_bps,
        DEFAULT_INITIAL_WEEKLY_RATE_BPS as u32
    );
    assert_eq!(stats.rate_step_bps, DEFAULT_RATE_STEP_BPS as u32);
    assert_eq!(
        stats.rate_step_interval_months,
        DEFAULT_RATE_STEP_INTERVAL_MONTHS as u32
    );
    assert_eq!(
        stats.max_weekly_rate_bps,
        DEFAULT_MAX_WEEKLY_RATE_BPS as u32
    );
}

#[test]
fn test_default_schedule_steps_after_interval() {
    let mut contract = setup_default_contract();
    setup_with_storage(&mut contract, "participant.near");

    let start_time = 1_000_000_000_000_000_000u64;
    fund_pool_at(&mut contract, 100 * ONE_SOCIAL, start_time);
    lock_tokens_at(&mut contract, "participant.near", ONE_SOCIAL, 6, start_time);

    let mut context = get_context("viewer.near");
    context.block_timestamp(start_time + 2 * MONTH_NS);
    testing_env!(context.build());

    let stats = contract.get_stats();
    assert_eq!(stats.active_weekly_rate_bps, 2);
}

#[test]
fn test_default_schedule_caps_at_max_rate() {
    let mut contract = setup_default_contract();
    setup_with_storage(&mut contract, "participant.near");
    ...
    let mut context = get_context("viewer.near");
    context.block_timestamp(start_time + 120 * MONTH_NS);
    testing_env!(context.build());

    let stats = contract.get_stats();
    assert_eq!(
        stats.active_weekly_rate_bps, DEFAULT_MAX_WEEKLY_RATE_BPS as u32,
        "Schedule should stop increasing once it reaches the configured max"
    );
}

#[test]
fn test_default_schedule_full_ramp_table() {
    let contract = setup_default_contract();
    let start_time = 1_000_000_000_000_000_000u64;
    ...
    for (elapsed_months, expected_rate_bps) in checkpoints {
        let mut context = get_context("viewer.near");
        context.block_timestamp(start_time + elapsed_months * MONTH_NS);
        testing_env!(context.build());

        let stats = contract.get_stats();
        assert_eq!(
            stats.active_weekly_rate_bps, expected_rate_bps,
            "default schedule should be {} bps at month {}",
            expected_rate_bps, elapsed_months
        );
    }
}

#[test]
fn test_release_spans_step_boundary_with_piecewise_rates() {
    let mut contract = setup_stepped_contract(10, 10, 1, 30);
    setup_with_storage(&mut contract, "alice.near");

    let start_time = 1_000_000_000_000_000_000u64;
    let initial_pool = 1000 * ONE_SOCIAL;
    fund_pool_at(&mut contract, initial_pool, start_time);
    lock_tokens_at(&mut contract, "alice.near", ONE_SOCIAL, 6, start_time);

    let checkpoint = start_time + MONTH_NS + WEEK_NS;
    let mut context = get_context("anyone.near");
    context.block_timestamp(checkpoint);
    testing_env!(context.build());

    contract.poke();

    let first_segment_release = compute_continuous_release(initial_pool, MONTH_NS, 10);
    let remaining_after_first = initial_pool.saturating_sub(first_segment_release);
    let second_segment_release = compute_continuous_release(remaining_after_first, WEEK_NS, 20);
    let expected_total = first_segment_release.saturating_add(second_segment_release);

    assert_eq!(
        contract.total_rewards_released, expected_total,
        "Release across a schedule boundary should use the pre-step and post-step rates piecewise"
    );
}

#[test]
fn test_schedule_changes_exactly_at_step_boundary() {
    let contract = setup_stepped_contract(10, 10, 1, 30);
    let start_time = 1_000_000_000_000_000_000u64;

    let mut context = get_context("viewer.near");
    context.block_timestamp(start_time + MONTH_NS - 1);
    testing_env!(context.build());
    assert_eq!(contract.get_stats().active_weekly_rate_bps, 10);

    let mut context = get_context("viewer.near");
    context.block_timestamp(start_time + MONTH_NS);
    testing_env!(context.build());
    assert_eq!(contract.get_stats().active_weekly_rate_bps, 20);
...
#[test]
fn test_release_spans_multiple_step_boundaries() {
    let mut contract = setup_stepped_contract(10, 10, 1, 40);
    setup_with_storage(&mut contract, "alice.near");
    ...
    let checkpoint = start_time + 2 * MONTH_NS + WEEK_NS;
    let mut context = get_context("anyone.near");
    context.block_timestamp(checkpoint);
    testing_env!(context.build());
    ...
    let first_segment_release = compute_continuous_release(initial_pool, MONTH_NS, 10);
    let remaining_after_first = initial_pool.saturating_sub(first_segment_release);
    let second_segment_release = compute_continuous_release(remaining_after_first, MONTH_NS, 20);
    let remaining_after_second = remaining_after_first.saturating_sub(second_segment_release);
    let third_segment_release = compute_continuous_release(remaining_after_second, WEEK_NS, 30);
    let expected_total = first_segment_release
        .saturating_add(second_segment_release)
        .saturating_add(third_segment_release);

    assert_eq!(
        contract.total_rewards_released, expected_total,
        "Release over multiple schedule boundaries should compound each segment with its active rate"
    );
}

#[test]
fn test_dormant_period_does_not_back_release_stepped_rewards() {
    let mut contract = setup_stepped_contract(10, 10, 1, 40);
    setup_with_storage(&mut contract, "alice.near");
    ...
    let dormant_end = start_time + 3 * MONTH_NS;
    let mut context = get_context("anyone.near");
    context.block_timestamp(dormant_end);
    testing_env!(context.build());

    contract.poke();
    assert_eq!(
        contract.total_rewards_released, 0,
        "No rewards should release while there are no participants"
    );

    lock_tokens_at(&mut contract, "alice.near", ONE_SOCIAL, 6, dormant_end);

    let mut context = get_context("anyone.near");
    context.block_timestamp(dormant_end + WEEK_NS);
    testing_env!(context.build());

    contract.poke();

    let expected_release = compute_continuous_release(initial_pool, WEEK_NS, 40);
    assert_eq!(
        contract.total_rewards_released, expected_release,
        "Dormant time should not retroactively release rewards; only the post-arrival week at the active stepped rate should count"
    );
}
...
fn test_lock_tokens_basic() {
    ...
    assert_eq!(account.effective_boost.0, ONE_SOCIAL * 110 / 100);

    // Contract totals
    assert_eq!(contract.total_locked, ONE_SOCIAL);
    assert_eq!(contract.total_effective_boost, ONE_SOCIAL * 110 / 100);
}
...
fn test_extend_lock() {
    ...
    // 12 months = 20% bonus
    assert_eq!(new_account.effective_boost.0, ONE_SOCIAL * 120 / 100);
}
...
#[test]
fn test_boost_seconds_accumulation() {
    let mut contract = setup_contract();
    setup_with_storage(&mut contract, "alice.near");
    ...
    // Check global boost-seconds
    // Should be effective_boost × 100 seconds
    let expected_ss = effective * 100;
    assert_eq!(contract.total_boost_seconds, expected_ss);
}

#[test]
fn test_boost_seconds_multiple_users() {
    let mut contract = setup_contract();
    setup_with_storage(&mut contract, "alice.near");
    ...
    assert_eq!(contract.total_boost_seconds, expected_total);
}
...
fn test_weekly_release() {
    ...
    // Fund pool and add a participant (required for rewards to release)
    fund_pool_at(&mut contract, 100 * ONE_SOCIAL, start_time);
    lock_tokens_at(&mut contract, "participant.near", ONE_SOCIAL, 6, start_time);
    ...
    let expected_release = 100 * ONE_SOCIAL * TEST_FIXED_WEEKLY_RATE_BPS / 10_000;
...
#[test]
fn test_release_handles_long_lock_period() {
    let mut contract = setup_contract();
    setup_with_storage(&mut contract, "participant.near");
...
#[test]
fn test_claimable_rewards_single_participant() {
    let mut contract = setup_contract();
    setup_with_storage(&mut contract, "alice.near");
    ...
    // Alice is the only participant, so she should get all released rewards
    // Released = 1000 × 0.002 = 2 SOCIAL
    let expected_release = 1000 * ONE_SOCIAL * TEST_FIXED_WEEKLY_RATE_BPS / 10_000;
...
#[test]
fn test_claimable_rewards_two_participants_equal() {
    let mut contract = setup_contract();
    setup_with_storage(&mut contract, "alice.near");
    setup_with_storage(&mut contract, "bob.near");
...
#[test]
fn test_claimable_rewards_proportional_to_boost() {
    let mut contract = setup_contract();
    setup_with_storage(&mut contract, "alice.near");
    setup_with_storage(&mut contract, "bob.near");
...
#[test]
fn test_rewards_respect_effective_boost_bonus() {
    let mut contract = setup_contract();
    setup_with_storage(&mut contract, "alice.near");
    setup_with_storage(&mut contract, "bob.near");
...
fn test_get_reward_rate() {
    ...
    assert_eq!(rate.effective_boost.0, ONE_SOCIAL * 110 / 100);
    assert_eq!(rate.total_effective_boost.0, ONE_SOCIAL * 110 / 100);

    // Weekly release = 1000 × 0.002 = 2 SOCIAL
    let expected_weekly = 1000 * ONE_SOCIAL * TEST_FIXED_WEEKLY_RATE_BPS / 10_000;
...
fn test_update_contract_from_hash_requires_owner() {
    ...
    assert!(matches!(result, Err(BoostError::Unauthorized(message)) if message == "Only owner"));
}
...
#[test]
fn test_zero_effective_boost_no_rewards() {
    let contract = setup_contract();
...
fn test_account_view_complete() {
    ...
    assert_eq!(view.effective_boost.0, ONE_SOCIAL * 120 / 100);
    assert_eq!(view.boost_seconds.0, 0); // Just locked, no time passed
...
fn test_bonus_12_months() {
    ...
    let account = contract.get_account("alice.near".parse().unwrap());
    // 12 months = 20% bonus
    assert_eq!(account.effective_boost.0, ONE_SOCIAL * 120 / 100);
}
...
fn test_bonus_24_months() {
    ...
    let account = contract.get_account("alice.near".parse().unwrap());
    // 24 months = 35% bonus
    assert_eq!(account.effective_boost.0, ONE_SOCIAL * 135 / 100);
}
...
fn test_bonus_48_months() {
    ...
    let account = contract.get_account("alice.near".parse().unwrap());
    // 48 months = 50% bonus
    assert_eq!(account.effective_boost.0, ONE_SOCIAL * 150 / 100);
}
...
fn test_late_joiner_gets_fair_share() {
    ...
    assert!(
        alice.claimable_rewards.0 > bob.claimable_rewards.0,
        "Alice (early participant) should have more rewards"
    );
}
...
fn test_large_lock_amounts() {
    ...
    // Trigger poke to accumulate boost_seconds and release rewards
    contract.poke();

    // Should not overflow - claimable_rewards projects boost_seconds internally
    ...
    let stats = contract.get_stats();
    assert!(
        stats.total_boost_seconds.0 > 0,
        "Total boost-seconds should be tracked globally"
    );
}

#[test]
fn test_multiple_equal_participants() {
    let mut contract = setup_contract();

    let start_time = 1_000_000_000_000_000_000u64;
    fund_pool_at(&mut contract, 1000 * ONE_SOCIAL, start_time);

    // Create 5 participants with equal locks
    ...
    assert!(
        diff < ONE_SOCIAL / 1000,
        "All equal participants should have equal rewards"
    );
}
// =============================================================================
// BUG DETECTION TEST: total_effective_boost Invariant Violation
// =============================================================================

/// BUG: After unlock, total_effective_boost retains phantom bonus tokens.
///
...
/// REGRESSION TEST: A lone participant should receive ~100% of rewards.
...
// =============================================================================
// BUG: on_unlock_callback Fails to Restore tracked_effective_boost
// =============================================================================

/// PROOF-OF-CONCEPT: on_unlock_callback failure causes total_effective_boost inflation.
///
/// VULNERABILITY: When unlock() transfer fails and the callback restores state,
/// it does NOT restore `tracked_effective_boost`. This causes the next sync_account()
/// to ADD phantom effective boost to total_effective_boost.
///
/// IMPACT:
/// - total_effective_boost becomes inflated (potentially doubled per failed unlock)
/// - Reward distribution becomes unfair (legitimate participants receive less than deserved)
...
/// account to inflate total_effective_boost.
#[test]
fn test_bug_unlock_callback_missing_tracked_effective_boost_restore() {
    let mut contract = setup_contract();
    setup_with_storage(&mut contract, "alice.near");
    ...
    assert_eq!(
        ...
        "Initial total_effective_boost should be 120 SOCIAL"
    );
    ...
    let account = contract
        ...
        .unwrap();
    assert_eq!(
        account.tracked_effective_boost, expected_effective,
        "Initial tracked_effective_boost should be 120 SOCIAL"
    );
    ...
    assert_eq!(
        ...
        "total_effective_boost should be 0 after unlock"
    );
    assert_eq!(
    ...
    let mut context = get_context("boost.near"); // Simulating self-call
...
///
/// Shows how phantom effective boost causes legitimate participants to receive
...
fn test_bug_unlock_callback_reward_dilution() {
    ...
    // Each has 120 effective boost, total = 240
    assert_eq!(contract.total_effective_boost, 240 * ONE_SOCIAL);
...
fn test_unlock_callback_success_path() {
    ...
    assert_eq!(contract.total_effective_boost, expected_effective);
    ...
    assert_eq!(
        account_after.tracked_effective_boost, 0,
        "tracked_effective_boost should be 0"
    );
    ...
    assert_eq!(
        contract.total_effective_boost, 0,
        "total_effective_boost should remain 0"
    );
...
// =============================================================================
// CRITICAL: Rewards Pause When No Participants Exist
// =============================================================================

/// Verifies that rewards do NOT release when there are no participants.
/// The release clock pauses during dormant periods to prevent reward leakage.
///
/// Scenario:
/// - T=0: Pool funded with 1000 SOCIAL
/// - T=0 to T=1 week: NO participants (dormant period)
/// - T=1 week: Poke called - NO rewards should release (clock pauses)
/// - T=1 week: First participant arrives
/// - T=2 weeks: First participant should get 1 week of rewards (not 2)
#[test]
fn test_rewards_pause_when_no_participants() {
    let mut contract = setup_contract();
    setup_with_storage(&mut contract, "alice.near");
    ...
    // Verify initial state: no participants
    assert_eq!(contract.total_effective_boost, 0, "No participants yet");
    assert_eq!(contract.total_boost_seconds, 0, "No boost-seconds yet");

    // Step 2: Advance 1 week WITHOUT any participants
    ...
    assert_eq!(
        contract.total_rewards_released, 0,
        "Rewards should NOT release when no participants exist"
    );

    // The last_release_time should have been moved forward (clock paused)
    assert_eq!(
        contract.last_release_time, one_week_later,
        "Release clock should pause (move forward) when no participants exist"
    );

    // Step 3: First participant arrives
    let alice_stake_time = one_week_later + NS_PER_SEC;
    lock_tokens_at(&mut contract, "alice.near", ONE_SOCIAL, 6, alice_stake_time);

    // Verify a participant is now active
    assert!(
        contract.total_effective_boost > 0,
        "Alice should now be an active participant"
    );
    ...
    let expected_release = 1000 * ONE_SOCIAL * TEST_FIXED_WEEKLY_RATE_BPS / 10_000; // 2 SOCIAL
    assert_eq!(
        contract.total_rewards_released, expected_release,
        "Should release 1 week of rewards after the first participant arrives"
    );
    ...
    // Alice should get 100% of released rewards (she's the only participant)
    ...
    assert!(
        pct >= 95.0,
        "First participant should get ~100% of rewards released while active, got {:.1}%",
        pct
    );
}

/// Test that rewards resume when participants return after everyone unlocks
#[test]
fn test_rewards_resume_when_participants_return() {
    let mut contract = setup_contract();
    setup_with_storage(&mut contract, "alice.near");
    ...
    assert!(
        released_week1 > 0,
        "Rewards should release while participants exist"
    );
    ...
    // Instead, manually set effective boost to 0 to simulate unlock
    contract.total_effective_boost = 0;
    ...
    // Advance another week with no participants
    ...
    assert_eq!(
        ...
        "No rewards should release during a no-participant period"
    );
}
...
fn test_lock_6mo_extend_to_48mo_then_unlock() {
    ...
    let account = contract.get_account("alice.near".parse().unwrap());
    assert_eq!(account.lock_months, 6);
    assert_eq!(account.effective_boost.0, ONE_SOCIAL * 110 / 100); // 10% bonus
    ...
    assert_eq!(account.effective_boost.0, ONE_SOCIAL * 150 / 100); // 50% bonus
...
fn test_ft_on_transfer_rejects_wrong_token() {
    ...
    assert!(matches!(result, Err(BoostError::InvalidInput(message)) if message == "Wrong token"));
}
...
fn test_ft_on_transfer_rejects_invalid_json() {
    ...
    assert!(matches!(result, Err(BoostError::InvalidInput(message)) if message == "Invalid JSON"));
}
...
fn test_ft_on_transfer_rejects_missing_action() {
    ...
    assert!(
        matches!(result, Err(BoostError::InvalidInput(message)) if message == "Missing action")
    );
}
...
fn test_ft_on_transfer_rejects_unknown_action() {
    ...
    assert!(
        matches!(result, Err(BoostError::InvalidInput(message)) if message == "Unknown action")
    );
}
...
fn test_ft_on_transfer_rejects_missing_months() {
    ...
    assert!(
        matches!(result, Err(BoostError::InvalidInput(message)) if message == "Missing months")
    );
}
...
fn test_ft_on_transfer_rejects_zero_amount() {
    ...
    assert!(matches!(result, Err(BoostError::InvalidAmount)));
}
...
fn test_multiple_users_different_extends() {
    ...
    assert_eq!(contract.total_effective_boost, 3 * ONE_SOCIAL * 105 / 100);
    ...
    assert_eq!(contract.total_effective_boost, expected);
}

/// Verify effective boost updates correctly through progressive extensions
#[test]
fn test_effective_boost_after_multiple_extends() {
    let mut contract = setup_contract();
    setup_with_storage(&mut contract, "alice.near");
    ...
    // Track total_effective_boost after each extension
    let effective_1mo = contract.total_effective_boost;
    ...
    contract.extend_lock(6).unwrap();
    assert_eq!(contract.total_effective_boost, 10 * ONE_SOCIAL * 110 / 100); // 11 SOCIAL (6mo = 10%)

    contract.extend_lock(12).unwrap();
    assert_eq!(contract.total_effective_boost, 10 * ONE_SOCIAL * 120 / 100); // 12 SOCIAL

    contract.extend_lock(24).unwrap();
    assert_eq!(contract.total_effective_boost, 10 * ONE_SOCIAL * 135 / 100); // 13.5 SOCIAL

    contract.extend_lock(48).unwrap();
    assert_eq!(contract.total_effective_boost, 10 * ONE_SOCIAL * 150 / 100); // 15 SOCIAL
}

/// Verify sum(tracked_effective_boost) == total_effective_boost invariant
#[test]
fn test_total_tracked_effective_boost_invariant() {
    let mut contract = setup_contract();
    setup_with_storage(&mut contract, "alice.near");
    setup_with_storage(&mut contract, "bob.near");
    setup_with_storage(&mut contract, "carol.near");
    ...
    // Calculate sum of tracked_effective_boost
    let alice = contract
        .accounts
        .get(&"alice.near".parse::<AccountId>().unwrap())
        .cloned()
    ...
    let sum_tracked =
        alice.tracked_effective_boost + bob.tracked_effective_boost + carol.tracked_effective_boost;

    assert_eq!(
        sum_tracked, contract.total_effective_boost,
        "INVARIANT: sum(tracked_effective_boost) should equal total_effective_boost"
    );
}
...
/// Test basic reward claiming for a single participant
#[test]
fn test_claim_rewards_basic() {
    ...
    let expected = 1000 * ONE_SOCIAL * TEST_FIXED_WEEKLY_RATE_BPS / 10_000;
...
#[test]
fn test_boost_seconds_exactness_after_rapid_operations() {
    let mut contract = setup_contract();
    setup_with_storage(&mut contract, "alice.near");
    ...
    assert_eq!(
        ...
        "No time elapsed = no boost-seconds"
    );
    ...
    assert_eq!(
        account.boost_seconds, expected,
        "Should have 1 second of boost-seconds"
    );
}
...
fn test_reward_calculation_with_tiny_amounts() {
    ...
    lock_tokens_at(&mut contract, "alice.near", MIN_BOOST_LOCK, 6, start_time);
...
fn test_binary_exponentiation_accuracy_100_years() {
    ...
    let remaining = compute_remaining_pool(pool, weeks, TEST_FIXED_WEEKLY_RATE_BPS);
...
fn test_u256_helpers_edge_cases() {
    ...
    let result = compute_remaining_pool(1000, 0, TEST_FIXED_WEEKLY_RATE_BPS);
    assert_eq!(result, 1000, "0 weeks = no decay");

    // Test compute_remaining_pool with 0 pool
    let result = compute_remaining_pool(0, 100, TEST_FIXED_WEEKLY_RATE_BPS);
    assert_eq!(result, 0, "0 pool = 0 remaining");
}

/// Test concurrent participants joining and leaving in the same block
#[test]
fn test_concurrent_participants_join_leave_same_block() {
    let mut contract = setup_contract();
    setup_with_storage(&mut contract, "alice.near");
    setup_with_storage(&mut contract, "bob.near");
    setup_with_storage(&mut contract, "carol.near");
    ...
    // All should have rewards proportional to their effective boost
    let alice = contract.get_account("alice.near".parse().unwrap());
    let bob = contract.get_account("bob.near".parse().unwrap());
    let carol = contract.get_account("carol.near".parse().unwrap());

    // Carol should have most rewards (highest effective boost)
...
/// Verify boost-seconds gives correct proportional rewards for different join times
#[test]
fn test_reward_fairness_different_join_times() {
    ...
    // Alice staked for 2 weeks, Bob for 1 week
    // Alice should have roughly 2x the boost-seconds
...
fn test_sync_account_idempotent() {
    ...
    let ss_after_first = contract
        ...
        .unwrap()
        .boost_seconds;
    let global_ss_first = contract.total_boost_seconds;
    ...
    let ss_after_second = contract
        ...
        .unwrap()
        .boost_seconds;
    let global_ss_second = contract.total_boost_seconds;
    let released_second = contract.total_rewards_released;

    // All values should be identical
    assert_eq!(
        ss_after_first, ss_after_second,
        "boost_seconds should not change on second sync"
    );
    assert_eq!(
        global_ss_first, global_ss_second,
        "total_boost_seconds should not change"
    );
    assert_eq!(
...
/// Test boost-seconds don't overflow with many users over long periods
#[test]
fn test_boost_seconds_no_overflow_long_term() {
    let mut contract = setup_contract();

    let start_time = 1_000_000_000_000_000_000u64;

    // Create 10 whale participants
    for i in 0..10 {
    ...
    assert!(
        contract.total_boost_seconds > 0,
        "Should have accumulated boost-seconds"
    );
}
...
fn test_precision_whale_and_minnow() {
    ...
    lock_tokens_at(&mut contract, "minnow.near", MIN_BOOST_LOCK, 6, start_time);
