...
fn test_migration_from_010_to_011() {
    ...
    assert!(
        logs.contains(
            &"EVENT_JSON:{\"standard\":\"nep297\",\"version\":\"1.0.0\",\"event\":\"state_migrated\",\"data\":{\"old_version\":\"0.1.0\",\"new_version\":\"0.1.1\"}}"
                .to_string()
        ),
        "Expected state_migrated event, got: {:?}",
        logs
    );
}
...
fn test_migration_from_011_to_011() {
    ...
    assert!(
        logs.contains(
            &"EVENT_JSON:{\"standard\":\"nep297\",\"version\":\"1.0.0\",\"event\":\"state_migrated\",\"data\":{\"old_version\":\"0.1.1\",\"new_version\":\"0.1.1\"}}"
                .to_string()
        ),
        "Expected state_migrated event, got: {:?}",
        logs
...
fn test_gas_logging_relay_meta_transaction() {
    ...
    let mut dummy_key = vec![0u8; 33]; // 33 bytes: 1 for key type + 32 for ED25519 public key
    ...
    assert!(
        logs.iter()
            .any(|log| log.contains("relay_meta_transaction")),
