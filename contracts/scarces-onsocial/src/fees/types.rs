...
impl FeeConfig {
    pub fn validate_patch(&self, patch: &FeeConfigUpdate) -> Result<(), MarketplaceError> {
        ...
        let platform = patch.platform_storage_fee_bps.unwrap_or(self.platform_storage_fee_bps);

        if !(MIN_TOTAL_FEE_BPS..=MAX_TOTAL_FEE_BPS).contains(&total) {
            return Err(MarketplaceError::InvalidInput(
                format!("total_fee_bps must be {MIN_TOTAL_FEE_BPS}..={MAX_TOTAL_FEE_BPS}"),
            ));
        }
        ...
        if app.max(platform) < MIN_POOL_FEE_BPS {
            return Err(MarketplaceError::InvalidInput(
                format!("at least one pool fee must be >= {MIN_POOL_FEE_BPS} bps"),
            ));
        }
        if app > MAX_POOL_FEE_BPS || platform > MAX_POOL_FEE_BPS {
            return Err(MarketplaceError::InvalidInput(
                format!("each pool fee must be <= {MAX_POOL_FEE_BPS} bps"),
            ));
        }
        Ok(())
    }
