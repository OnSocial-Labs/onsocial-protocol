...
impl Contract {
    ...
    pub fn list_nft_for_sale(
        ...
        assert!(
            ...
            "Sale already exists for this NFT"
        );
    ...
    pub fn process_listing(
        ...
        assert_eq!(
            ...
            2,
            "Expected 2 promise results"
        );

        // Check if marketplace is approved
        let is_approved = match env::promise_result(0) {
            PromiseResult::Successful(value) => near_sdk::serde_json::from_slice::<bool>(&value)
        ...
        env::log_str(&format!(
            ...
            owner_id, nft_contract_id, token_id, sale_conditions.0
        ));
    }
    ...
    pub fn remove_sale(&mut self, nft_contract_id: AccountId, token_id: String) {
        ...
        // Emit OnSocial event
        crate::events::emit_nft_delist_event(&owner_id, &nft_contract_id, vec![token_id.clone()]);
    ...
    pub fn resolve_purchase(
        ...
        if let Some(payout) = payout_option {
            ...
            if amount_after_fee > 0 {
                Promise::new(sale.owner_id.clone())
