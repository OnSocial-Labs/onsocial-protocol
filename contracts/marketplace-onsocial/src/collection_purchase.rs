...
impl Contract {
    ...
    pub fn purchase_from_collection(
        ...
        require!(
            deposit >= total_price,
            format!(
                "Insufficient payment: required {} yoctoNEAR, got {}",
                total_price, deposit
            )
        );
        ...
        if creator_payment > 0 {
            Promise::new(creator_id.clone()).transfer(NearToken::from_yoctonear(creator_payment));
        }
        ...
        env::log_str(&format!("Minted tokens: {:?}", minted_tokens));
    }
