...
fn create_collection_happy_path() {
    ...
    contract
        .internal_create_collection(&creator(), minimal_config("event-2026"))
        .unwrap();
...
fn create_collection_duplicate_id_fails() {
    ...
    contract
        .internal_create_collection(&creator(), minimal_config("dup"))
        .unwrap();
    let err = contract
        .internal_create_collection(&creator(), minimal_config("dup"))
        .unwrap_err();
...
fn create_collection_empty_id_fails() {
    let mut contract = new_contract();
    let err = contract
        .internal_create_collection(&creator(), minimal_config(""))
        .unwrap_err();
...
fn create_collection_colon_in_id_fails() {
    let mut contract = new_contract();
    let err = contract
        .internal_create_collection(&creator(), minimal_config("bad:id"))
        .unwrap_err();
    assert!(matches!(err, MarketplaceError::InvalidInput(_)));
}
...
fn create_collection_reserved_s_fails() {
    let mut contract = new_contract();
    let err = contract
        .internal_create_collection(&creator(), minimal_config("s"))
        .unwrap_err();
    assert!(matches!(err, MarketplaceError::InvalidInput(_)));
}
...
fn create_collection_reserved_ll_fails() {
    let mut contract = new_contract();
    let err = contract
        .internal_create_collection(&creator(), minimal_config("ll"))
        .unwrap_err();
    assert!(matches!(err, MarketplaceError::InvalidInput(_)));
}
...
fn create_collection_id_too_long() {
    ...
    let err = contract
        .internal_create_collection(&creator(), minimal_config(&long))
        .unwrap_err();
    assert!(matches!(err, MarketplaceError::InvalidInput(_)));
}
...
fn create_collection_zero_supply_fails() {
    ...
    cfg.total_supply = 0;
    let err = contract
        .internal_create_collection(&creator(), cfg)
        .unwrap_err();
    assert!(matches!(err, MarketplaceError::InvalidInput(_)));
}
...
fn collection_tracked_by_creator() {
    ...
    contract
        .internal_create_collection(&creator(), minimal_config("c1"))
        .unwrap();
    contract
        .internal_create_collection(&creator(), minimal_config("c2"))
        .unwrap();
...
fn pause_and_resume_collection() {
    ...
    contract
        .internal_create_collection(&creator(), minimal_config("pausable"))
        .unwrap();

    contract
        .internal_pause_collection(&creator(), "pausable")
        .unwrap();
    assert!(contract.collections.get("pausable").unwrap().paused);

    contract
        .internal_resume_collection(&creator(), "pausable")
        .unwrap();
    assert!(!contract.collections.get("pausable").unwrap().paused);
}
...
fn pause_wrong_creator_fails() {
    ...
    contract
        .internal_create_collection(&creator(), minimal_config("owned"))
        .unwrap();

    let err = contract
        .internal_pause_collection(&buyer(), "owned")
        .unwrap_err();
