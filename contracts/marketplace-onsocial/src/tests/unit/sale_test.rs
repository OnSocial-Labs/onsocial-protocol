...
fn make_standalone_token(contract: &mut Contract, owner_account: &AccountId) -> String {
    ...
    contract.internal_quick_mint(owner_account, metadata, options).unwrap()
}

// --- internal_list_native_scarce ---
...
fn list_native_scarce_happy() {
    ...
    contract
        .internal_list_native_scarce(&buyer(), &tid, U128(1_000), None)
        .unwrap();
...
fn list_native_scarce_not_owner_fails() {
    ...
    let err = contract
        .internal_list_native_scarce(&creator(), &tid, U128(1_000), None)
        .unwrap_err();
...
fn list_native_scarce_zero_price_fails() {
    ...
    let err = contract
        .internal_list_native_scarce(&buyer(), &tid, U128(0), None)
        .unwrap_err();
...
fn list_native_scarce_nonexistent_token_fails() {
    ...
    let err = contract
        .internal_list_native_scarce(&buyer(), "nope", U128(1_000), None)
        .unwrap_err();
...
fn list_native_scarce_duplicate_fails() {
    ...
    contract
        ...
        .unwrap();
    let err = contract
        .internal_list_native_scarce(&buyer(), &tid, U128(2_000), None)
        .unwrap_err();
...
fn list_native_scarce_past_expiry_fails() {
    ...
    let err = contract
        .internal_list_native_scarce(&buyer(), &tid, U128(1_000), Some(past))
        .unwrap_err();
    assert!(matches!(err, MarketplaceError::InvalidInput(_)));
}

// --- internal_delist_native_scarce ---
...
fn delist_native_scarce_happy() {
    ...
    contract
        ...
        .unwrap();
    contract
        .internal_delist_native_scarce(&buyer(), &tid)
        .unwrap();
...
fn delist_native_scarce_wrong_owner_fails() {
    ...
    contract
        ...
        .unwrap();
    let err = contract
        .internal_delist_native_scarce(&creator(), &tid)
        .unwrap_err();
    assert!(matches!(err, MarketplaceError::Unauthorized(_)));
}
...
fn delist_nonexistent_sale_fails() {
    ...
    let err = contract
        .internal_delist_native_scarce(&buyer(), "no-sale")
        .unwrap_err();
    assert!(matches!(err, MarketplaceError::NotFound(_)));
}

// --- internal_update_price ---
...
fn update_price_happy() {
    ...
    contract
        .internal_update_price(&buyer(), &mkt, &tid, U128(2_000))
        .unwrap();
...
fn update_price_zero_fails() {
    ...
    let mkt: AccountId = "marketplace.near".parse().unwrap();
    let err = contract
        .internal_update_price(&buyer(), &mkt, &tid, U128(0))
        .unwrap_err();
    assert!(matches!(err, MarketplaceError::InvalidInput(_)));
}
...
fn update_price_wrong_owner_fails() {
    ...
    let mkt: AccountId = "marketplace.near".parse().unwrap();
    let err = contract
        .internal_update_price(&creator(), &mkt, &tid, U128(5_000))
        .unwrap_err();
    assert!(matches!(err, MarketplaceError::Unauthorized(_)));
}
...
fn auction_list_happy() {
    ...
    contract
        .internal_list_native_scarce_auction(&buyer(), &tid, params)
        .unwrap();
...
fn cancel_auction_no_bids_happy() {
    ...
    contract
        .internal_list_native_scarce_auction(&buyer(), &tid, params)
        .unwrap();
    contract.internal_cancel_auction(&buyer(), &tid).unwrap();
...
fn cancel_auction_wrong_owner_fails() {
    ...
    contract
        ...
        .unwrap();
    let err = contract
        .internal_cancel_auction(&creator(), &tid)
        .unwrap_err();
    assert!(matches!(err, MarketplaceError::Unauthorized(_)));
}
...
fn list_soulbound_token_fails() {
    ...
    testing_env!(context(creator()).build());
    contract
        .internal_mint_from_collection(&creator(), "soul", 1, Some(&buyer()))
        .unwrap();

    testing_env!(context(buyer()).build());
    let err = contract
        .internal_list_native_scarce(&buyer(), "soul:1", U128(1_000), None)
        .unwrap_err();
    // Soulbound check
    assert!(matches!(err, MarketplaceError::InvalidState(_)));
}
...
fn list_revoked_token_fails() {
    ...
    testing_env!(context(creator()).build());
    contract
        .internal_mint_from_collection(&creator(), "rev", 1, Some(&buyer()))
        .unwrap();

    // Revoke it
    contract
        .internal_revoke_token(&creator(), "rev:1", "rev", None)
        .unwrap();

    testing_env!(context(buyer()).build());
    let err = contract
        .internal_list_native_scarce(&buyer(), "rev:1", U128(1_000), None)
        .unwrap_err();
    assert!(matches!(err, MarketplaceError::InvalidState(_)));
}
