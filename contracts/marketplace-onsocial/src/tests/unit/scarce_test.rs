...
fn mint_duplicate_id_fails() {
    ...
    let err = contract
        .internal_mint("t1".to_string(), ctx, metadata, None)
        .unwrap_err();
...
fn mint_token_id_too_long() {
    ...
    let err = contract
        .internal_mint(long_id, ctx, metadata, None)
        .unwrap_err();
...
fn transfer_changes_owner() {
    ...
    contract
        .internal_transfer(&owner(), &buyer(), "t1", None, None)
        .unwrap();
...
fn transfer_wrong_sender_fails() {
    ...
    let err = contract
        .internal_transfer(&buyer(), &creator(), "t1", None, None)
        .unwrap_err();
...
fn transfer_nonexistent_token_fails() {
    let mut contract = new_contract();
    let err = contract
        .internal_transfer(&owner(), &buyer(), "nope", None, None)
        .unwrap_err();
...
fn transfer_clears_approvals() {
    ...
    contract
        .internal_approve(&owner(), "t1", &buyer(), None)
        .unwrap();
    let token = contract.scarces_by_id.get("t1").unwrap();
    ...
    contract
        .internal_transfer(&owner(), &creator(), "t1", None, None)
        .unwrap();
    let token = contract.scarces_by_id.get("t1").unwrap();
    assert!(token.approved_account_ids.is_empty());
}
...
fn soulbound_token_cannot_transfer() {
    ...
    let err = contract
        .internal_transfer(&owner(), &buyer(), "soul1", None, None)
        .unwrap_err();
    assert!(matches!(err, MarketplaceError::InvalidState(_)));
}
...
fn approved_account_can_transfer() {
    ...
    contract
        .internal_transfer(&buyer(), &creator(), "t1", Some(approval_id), None)
        .unwrap();
    let token = contract.scarces_by_id.get("t1").unwrap();
...
fn invalid_approval_id_fails() {
    ...
    let err = contract
        .internal_transfer(&buyer(), &creator(), "t1", Some(999), None)
        .unwrap_err();
    assert!(matches!(err, MarketplaceError::Unauthorized(_)));
}
...
fn batch_transfer_moves_all() {
    ...
    contract.internal_batch_transfer(&owner(), transfers).unwrap();
...
fn batch_transfer_empty_fails() {
    let mut contract = new_contract();
    let err = contract
        .internal_batch_transfer(&owner(), vec![])
        .unwrap_err();
    assert!(matches!(err, MarketplaceError::InvalidInput(_)));
}
