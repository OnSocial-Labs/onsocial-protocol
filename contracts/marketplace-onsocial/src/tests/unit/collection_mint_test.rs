...
fn setup_with_collection(supply: u32, mint_mode: MintMode) -> Contract {
    ...
    contract
        .internal_create_collection(&creator(), mint_config("col", supply, mint_mode))
        .unwrap();
    contract
}
...
fn creator_mint_single() {
    ...
    contract
        .internal_mint_from_collection(&creator(), "col", 1, None)
        .unwrap();
...
fn creator_mint_batch() {
    ...
    contract
        .internal_mint_from_collection(&creator(), "col", 5, None)
        .unwrap();
...
fn creator_mint_to_receiver() {
    ...
    contract
        .internal_mint_from_collection(&creator(), "col", 1, Some(&buyer()))
        .unwrap();
...
fn mint_exceeds_supply_fails() {
    ...
    let err = contract
        .internal_mint_from_collection(&creator(), "col", 4, None)
        .unwrap_err();
...
fn mint_exact_remaining_supply() {
    ...
    contract
        .internal_mint_from_collection(&creator(), "col", 3, None)
        .unwrap();
...
fn mint_zero_quantity_fails() {
    ...
    let err = contract
        .internal_mint_from_collection(&creator(), "col", 0, None)
        .unwrap_err();
...
fn mint_exceeds_max_batch_fails() {
    ...
    let err = contract
        .internal_mint_from_collection(&creator(), "col", MAX_BATCH_MINT + 1, None)
        .unwrap_err();
    assert!(matches!(err, MarketplaceError::InvalidInput(_)));
}
...
fn non_creator_cannot_mint() {
    ...
    let err = contract
        .internal_mint_from_collection(&buyer(), "col", 1, None)
        .unwrap_err();
    assert!(matches!(err, MarketplaceError::Unauthorized(_)));
}
...
fn paused_collection_blocks_mint() {
    ...
    // Pause
    contract.internal_pause_collection(&creator(), "col").unwrap();
...
fn mint_from_nonexistent_collection_fails() {
    ...
    let err = contract
        .internal_mint_from_collection(&creator(), "nope", 1, None)
        .unwrap_err();
