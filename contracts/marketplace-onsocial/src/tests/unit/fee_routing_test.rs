...
fn fee_split_no_app_id() {
    ...
    let (total, app, platform, revenue) = contract.internal_calculate_fee_split(price, None);
...
fn fee_split_with_app_pool() {
    ...
    let (total, app_amt, platform, revenue) =
        contract.internal_calculate_fee_split(price, Some(&app));
...
fn fee_split_missing_app_pool_falls_to_platform() {
    ...
    let (total, app_amt, platform, revenue) =
        contract.internal_calculate_fee_split(1_000_000, Some(&app));
...
fn fee_split_zero_price() {
    let contract = new_contract();
    let (total, app, platform, revenue) = contract.internal_calculate_fee_split(0, None);
...
fn update_fee_config_happy() {
    ...
    contract
        .internal_update_fee_config(Some(500), Some(100), Some(100))
        .unwrap();
    assert_eq!(contract.fee_config.total_fee_bps, 500);
    assert_eq!(contract.fee_config.app_pool_fee_bps, 100);
    assert_eq!(contract.fee_config.platform_storage_fee_bps, 100);
}

#[test]
fn update_fee_config_total_above_10_percent_fails() {
    let mut contract = new_contract();
    testing_env!(context(owner()).build());

    let err = contract
        .internal_update_fee_config(Some(1001), None, None)
        .unwrap_err();
...
fn update_fee_config_app_exceeds_total_fails() {
    ...
    let err = contract
        .internal_update_fee_config(Some(200), Some(201), None)
        .unwrap_err();
    assert!(matches!(err, MarketplaceError::InvalidInput(_)));
}
...
fn update_fee_config_platform_exceeds_total_fails() {
    ...
    let err = contract
        .internal_update_fee_config(Some(200), None, Some(201))
        .unwrap_err();
    assert!(matches!(err, MarketplaceError::InvalidInput(_)));
}
...
fn update_fee_config_sum_exceeds_total_fails() {
    ...
    let err = contract
        .internal_update_fee_config(Some(200), Some(150), Some(100))
        .unwrap_err();
    assert!(matches!(err, MarketplaceError::InvalidInput(_)));
}
...
fn update_fee_config_partial_update() {
    ...
    // only update total, keep subs
    contract.internal_update_fee_config(Some(300), None, None).unwrap();
...
#[test]
fn update_fee_config_boundary_1000_bps() {
    let mut contract = new_contract();
    testing_env!(context(owner()).build());

    contract
        .internal_update_fee_config(Some(1000), Some(500), Some(500))
        .unwrap();
    assert_eq!(contract.fee_config.total_fee_bps, 1000);
}
