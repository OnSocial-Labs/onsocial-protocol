...
fn create_lazy_listing_happy() {
    ...
    let id = contract
        .internal_create_lazy_listing(&creator(), make_lazy_listing_params(1_000))
        .unwrap();
...
fn cancel_lazy_listing_happy() {
    ...
    let id = contract
        ...
        .unwrap();
    contract
        .internal_cancel_lazy_listing(&creator(), &id)
        .unwrap();
    assert!(!contract.lazy_listings.contains_key(&id));
}
...
fn cancel_lazy_listing_wrong_creator_fails() {
    ...
    let id = contract
        ...
        .unwrap();
    let err = contract
        .internal_cancel_lazy_listing(&buyer(), &id)
        .unwrap_err();
...
fn cancel_nonexistent_listing_fails() {
    ...
    let err = contract
        .internal_cancel_lazy_listing(&creator(), "ll:999")
        .unwrap_err();
    assert!(matches!(err, MarketplaceError::NotFound(_)));
}

// --- internal_update_lazy_listing_expiry ---
...
fn update_expiry_happy() {
    ...
    contract
        .internal_update_lazy_listing_expiry(&creator(), &id, Some(future))
        .unwrap();
...
fn update_expiry_past_fails() {
    ...
    let err = contract
        .internal_update_lazy_listing_expiry(&creator(), &id, Some(past))
        .unwrap_err();
    assert!(matches!(err, MarketplaceError::InvalidInput(_)));
}
...
fn update_expiry_wrong_creator_fails() {
    ...
    let id = contract
        ...
        .unwrap();
    let err = contract
        .internal_update_lazy_listing_expiry(&buyer(), &id, None)
        .unwrap_err();
    assert!(matches!(err, MarketplaceError::Unauthorized(_)));
}

// --- internal_update_lazy_listing_price ---
...
fn update_price_happy() {
    ...
    let id = contract
        ...
        .unwrap();
    contract
        .internal_update_lazy_listing_price(&creator(), &id, 5_000)
        .unwrap();
...
fn update_price_wrong_creator_fails() {
    ...
    let id = contract
        ...
        .unwrap();
    let err = contract
        .internal_update_lazy_listing_price(&buyer(), &id, 5_000)
        .unwrap_err();
    assert!(matches!(err, MarketplaceError::Unauthorized(_)));
}
...
fn update_price_nonexistent_fails() {
    ...
    let err = contract
        .internal_update_lazy_listing_price(&creator(), "ll:999", 5_000)
        .unwrap_err();
    assert!(matches!(err, MarketplaceError::NotFound(_)));
}
