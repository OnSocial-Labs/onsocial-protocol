...
fn payout_no_royalty_all_to_seller() {
    ...
    let payout = contract.internal_compute_payout(&token, &seller, 1_000_000, 10).unwrap();
...
fn payout_single_royalty_recipient() {
    ...
    let payout = contract.internal_compute_payout(&token, &seller, balance, 10).unwrap();
...
fn payout_too_many_recipients_fails() {
    ...
    let token = make_token(Some(royalty));
    let result = contract
        .internal_compute_payout(&token, &seller, 10_000, 3);
...
fn payout_zero_balance() {
    ...
    let payout = contract.internal_compute_payout(&token, &seller, 0, 10).unwrap();
