...
fn renew_happy_path() {
    ...
    contract
        .internal_renew_token(&creator(), &tid, "col", future)
        .unwrap();
...
fn renew_past_expiry_fails() {
    ...
    let err = contract
        .internal_renew_token(&creator(), &tid, "col", past)
        .unwrap_err();
...
fn renew_non_creator_fails() {
    ...
    let future = 2_000_000_000_000_000_000u64;
    let err = contract
        .internal_renew_token(&buyer(), &tid, "col", future)
        .unwrap_err();
...
fn revoke_invalidate_happy() {
    ...
    contract
        .internal_revoke_token(&creator(), &tid, "col", Some("bad behaviour".into()))
        .unwrap();
...
fn revoke_already_revoked_fails() {
    ...
    contract
        .internal_revoke_token(&creator(), &tid, "col", None)
        .unwrap();
    let err = contract
        .internal_revoke_token(&creator(), &tid, "col", None)
        .unwrap_err();
...
fn redeem_happy_path() {
    ...
    contract
        .internal_redeem_token(&creator(), &tid, "col")
        .unwrap();
...
fn burn_happy_path() {
    ...
    contract
        .internal_burn_scarce(&buyer(), &tid, "col")
        .unwrap();
...
fn burn_not_owner_fails() {
    ...
    let err = contract
        .internal_burn_scarce(&creator(), &tid, "col")
        .unwrap_err();
    assert!(matches!(err, MarketplaceError::Unauthorized(_)));
}
...
fn renew_wrong_collection_fails() {
    ...
    let err = contract
        .internal_renew_token(&creator(), "col:1", "other-col", 2_000_000_000_000_000_000)
        .unwrap_err();
