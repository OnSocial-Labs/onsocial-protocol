...
impl crate::groups::core::GroupStorage {
    pub fn can_grant_permissions(
        ...
        if level == crate::groups::kv_permissions::MANAGE {
            return false;
        }

        let group_config_path = Self::group_config_path(group_id);
        let group_owner =
            match crate::groups::kv_permissions::extract_path_owner(platform, &group_config_path) {
                Some(owner) => owner,
                None => return false,
            };
        ...
        if level == crate::groups::kv_permissions::NONE || level == crate::groups::kv_permissions::WRITE {
            if crate::groups::kv_permissions::can_moderate(
                platform,
                &group_owner,
                granter_id.as_str(),
                &group_config_path,
