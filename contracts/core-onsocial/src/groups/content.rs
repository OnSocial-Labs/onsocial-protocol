...
impl GroupContentManager {
    ...
    pub fn create_group_content(
        ...
        let info = crate::groups::kv_permissions::classify_group_path(group_path)
            .ok_or_else(|| crate::invalid_input!("Invalid group path format"))?;

        if info.kind == crate::groups::kv_permissions::GroupPathKind::Config {
        ...
        let can_write = crate::groups::kv_permissions::can_write(platform, group_id, author.as_str(), normalized_path);
        if !can_write {
