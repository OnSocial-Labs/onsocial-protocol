...
pub(super) fn parse_permission_change(changes: &Value) -> Result<crate::groups::ProposalType, SocialError> {
    ...
    Ok(crate::groups::ProposalType::PermissionChange {
        target_user,
        level,
...
pub(super) fn parse_path_permission_grant(changes: &Value) -> Result<crate::groups::ProposalType, SocialError> {
    let target_user_str = changes
        .get("target_user")
        .and_then(|v| v.as_str())
    ...
    Ok(crate::groups::ProposalType::PathPermissionGrant {
        target_user,
        path: path.to_string(),
        level,
...
pub(super) fn parse_path_permission_revoke(changes: &Value) -> Result<crate::groups::ProposalType, SocialError> {
    let target_user_str = changes
        .get("target_user")
        .and_then(|v| v.as_str())
    ...
    Ok(crate::groups::ProposalType::PathPermissionRevoke {
        target_user,
        path: path.to_string(),
        reason: reason.to_string(),
    })
