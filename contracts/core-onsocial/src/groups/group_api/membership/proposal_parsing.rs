...
pub(crate) fn parse_member_invite_proposal(
    changes: &Value,
) -> Result<crate::groups::ProposalType, SocialError> {
    ...
    let level = changes
        ...
        .unwrap_or(crate::groups::kv_permissions::NONE); // Default to NONE (member-only)
    let message = changes.get("message").and_then(|v| v.as_str());
    Ok(crate::groups::ProposalType::MemberInvite {
        target_user,
        level,
...
pub(crate) fn parse_join_request_proposal(
    ...
    let requested_permissions = changes
        ...
        .and_then(|v| v.as_u64())
        .and_then(|f| if f <= 255 { Some(f as u8) } else { None })
        .unwrap_or(crate::groups::kv_permissions::NONE);
    let message = changes.get("message").and_then(|v| v.as_str());
    Ok(crate::groups::ProposalType::JoinRequest {
        requester,
