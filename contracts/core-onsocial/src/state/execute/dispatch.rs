...
impl SocialPlatform {
    ...
    pub(super) fn dispatch_action(
        ...
        match action {
            ...
            Action::RemoveGroupMember { group_id, member_id } => {
                self.execute_action_remove_member(group_id, member_id, ctx)?;
                Ok(Value::Null)
            }

            Action::ApproveJoinRequest { group_id, requester_id } => {
                self.execute_action_approve_join(group_id, requester_id, ctx)?;
                Ok(Value::Null)
            }

            Action::RejectJoinRequest { group_id, requester_id, reason } => {
            ...
            Action::BlacklistGroupMember { group_id, member_id } => {
                self.execute_action_blacklist(group_id, member_id, ctx)?;
                Ok(Value::Null)
            }

            Action::UnblacklistGroupMember { group_id, member_id } => {
            ...
            Action::SetGroupPrivacy { group_id, is_private } => {
                self.execute_action_set_privacy(group_id, *is_private, ctx)?;
                Ok(Value::Null)
            }

            Action::CreateProposal { group_id, proposal_type, changes, auto_vote } => {
                let proposal_id = self.execute_action_create_proposal(
                    group_id, proposal_type, changes.clone(), *auto_vote, ctx
                )?;
                Ok(json!(proposal_id))
            }

            Action::VoteOnProposal { group_id, proposal_id, approve } => {
                self.execute_action_vote(group_id, proposal_id, *approve, ctx)?;
                Ok(Value::Null)
            }

            Action::CancelProposal { group_id, proposal_id } => {
            ...
            Action::SetKeyPermission { public_key, path, level, expires_at } => {
