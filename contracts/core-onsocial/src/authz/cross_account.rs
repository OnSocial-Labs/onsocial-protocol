    ...
    for key in data_obj.keys() {
        ...
        match kind {
            ApiOperationKey::DataPath(path) => {
                ...
                let path_owner = crate::groups::kv_permissions::extract_path_owner(platform, full_path)
                    .unwrap_or_else(|| target_account.as_str().to_string());

                let can_write = if is_group_path {
                    let account_ok = crate::groups::kv_permissions::can_write(
                        platform,
                               &path_owner,
                               actor_id.as_str(),
                               full_path,
                    ...
                    match actor_pk {
                        Some(pk) => crate::groups::kv_permissions::has_permissions_or_key_for_actor(
                            platform,
                            &path_owner,
                            full_path,
                               crate::groups::kv_permissions::WRITE,
                        ...
                        None => crate::groups::kv_permissions::can_write(
                            platform,
                            &path_owner,
                            actor_id.as_str(),
                            full_path,
