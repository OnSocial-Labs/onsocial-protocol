...
mod voting_proposal_types_tests {
    ...
    fn test_member_driven_blocks_direct_set_permission_on_group_paths() {
        ...
        contract
            .execute(create_group_request("md_gate".to_string(), config))
            .unwrap();
        ...
        assert!(
            direct_group_grant.is_err(),
            "Direct set_permission on member-driven group path must fail"
        );
        ...
        assert!(
            direct_personal_grant.is_ok(),
            "Non-group permissions should still be grantable"
        );
    }
    ...
    fn test_member_driven_manage_can_delegate_with_expiry_to_existing_members_only() {
        ...
        contract
            .execute(create_group_request("md_delegation".to_string(), config))
            .unwrap();
        ...
        let delegated_ok = contract.execute(set_permission_request(
            ...
            WRITE,
            Some(U64(
                crate::tests::test_utils::TEST_BASE_TIMESTAMP + 1_000_000_000
            )),
        ));
        assert!(
            delegated_ok.is_ok(),
            "Delegated grant within subtree should succeed: {:?}",
            delegated_ok.err()
        );
        ...
        assert!(
            delegated_missing_exp.is_err(),
            "Delegated grants without expires_at must fail"
        );
        ...
        let delegated_to_non_member = contract.execute(set_permission_request(
            ...
            WRITE,
            Some(U64(
                crate::tests::test_utils::TEST_BASE_TIMESTAMP + 2_000_000_000
            )),
        ));
        assert!(
            delegated_to_non_member.is_err(),
            "Delegated grants to non-members must fail"
        );
    }
    // ============================================================================
    ...
    fn test_path_permission_grant_proposal_approval() {
        ...
        contract
            .execute(create_group_request("path_test".to_string(), config))
            .unwrap();
    ...
    fn test_path_permission_grant_proposal_rejection() {
        ...
        contract
            .execute(create_group_request("reject_path".to_string(), config))
            .unwrap();
    ...
    fn test_path_permission_revoke_proposal_approval() {
        ...
        contract
            .execute(create_group_request("revoke_test".to_string(), config))
            .unwrap();
        ...
        let grant_proposal_id = contract
            .execute(create_proposal_request(
        ...
        let revoke_proposal_id = contract
            .execute(create_proposal_request(
                "revoke_test".to_string(),
        ...
        // 2 YES votes out of 3 members = 66% participation, 100% approval
        testing_env!(
            get_context_with_deposit(alice.clone(), test_deposits::member_operations()).build()
        );
        let vote_result = contract.execute(vote_proposal_request(
            "revoke_test".to_string(),
            revoke_proposal_id.clone(),
            true,
        ));
        assert!(
            vote_result.is_ok(),
    ...
    fn test_path_permission_revoke_nonexistent_permission() {
        ...
        assert!(
            vote_result.is_ok(),
            "Revoking nonexistent permission should succeed (idempotent): {:?}",
            vote_result.err()
        );

        println!(
            "✅ PathPermissionRevoke is idempotent - revoking nonexistent permission succeeds"
        );
    ...
    fn test_voting_config_change_quorum() {
        ...
        let initial_config = contract
            .platform
            .storage_get("groups/quorum_test/config")
            .unwrap();
        ...
        assert_eq!(
            updated_voting_config
                .get("participation_quorum_bps")
                .unwrap()
                .as_u64()
                .unwrap(),
            7500,
            "Quorum should be updated to 75%"
        );
        ...
        assert_eq!(
            updated_voting_config
                ...
                .unwrap(),
            5001,
            "Majority threshold should remain unchanged"
        );
        assert!(
            updated_voting_config.get("voting_period").is_some(),
            "Voting period should still exist"
        );
    ...
    fn test_voting_config_change_majority_threshold() {
        ...
        assert_eq!(
            initial_voting_config
                ...
                .unwrap(),
            5001,
            "Initial threshold should be >50%"
        );
        ...
        assert_eq!(
            updated_voting_config
                ...
                .unwrap(),
            7500,
            "Majority threshold should be updated to >75%"
        );
        ...
        assert_eq!(
            updated_voting_config
                ...
                .unwrap(),
            5100,
            "Participation quorum should remain unchanged"
        );
        assert!(
            updated_voting_config.get("voting_period").is_some(),
            "Voting period should still exist"
    ...
    fn test_voting_config_change_voting_period() {
        ...
        contract
            .execute(create_group_request("period_test".to_string(), config))
            .unwrap();
        ...
        let initial_config = contract
            .platform
            .storage_get("groups/period_test/config")
            .unwrap();
        let initial_voting_config = initial_config.get("voting_config").unwrap();
        ...
        assert_eq!(
            ...
            "Initial voting period should be 7 days"
        );
        ...
        let updated_period = updated_voting_config
            ...
            .unwrap();
        assert_eq!(
            updated_period, new_period,
            "Voting period should be updated to 3 days"
        );
    ...
    fn test_voting_config_change_self_referential() {
        ...
        contract
            .execute(create_group_request("self_ref_test".to_string(), config))
            .unwrap();
        ...
        let initial_config = contract
            .platform
            .storage_get("groups/self_ref_test/config")
            .unwrap();
        let initial_voting_config = initial_config.get("voting_config").unwrap();
        assert_eq!(
            initial_voting_config
        ...
        assert_eq!(
            updated_voting_config
                ...
                .unwrap(),
            8000,
            "Config should be updated despite only 60% participation"
        );
        ...
        let future_proposal_id = contract
            .execute(create_proposal_request(
                "self_ref_test".to_string(),
        ...
        let proposal_status = contract
            .platform
            .storage_get(&format!(
                "groups/self_ref_test/proposals/{}",
                future_proposal_id
            ))
            .unwrap();
        assert_eq!(
            proposal_status.get("status").unwrap().as_str().unwrap(),
            "active",
            "Future proposal should still be active due to insufficient participation under new rules"
        );
        ...
        let final_proposal_status = contract
            .platform
            .storage_get(&format!(
                "groups/self_ref_test/proposals/{}",
                future_proposal_id
        ...
        assert_eq!(
            final_proposal_status
                ...
                .unwrap(),
            "executed",
            "Future proposal should execute with 80% participation under new rules"
        );
    ...
    fn test_custom_proposal_voting_approval() {
        ...
        assert_eq!(
            proposal["type"], "custom_proposal",
            "Proposal type should be custom_proposal"
        );
        assert_eq!(
            proposal["proposer"],
            alice.to_string(),
        ...
        assert_eq!(
            proposal_after_vote1["status"], "active",
            "Proposal should still be active after 2/4 votes"
        );
        ...
        assert_eq!(
            proposal_after_vote2["status"], "executed",
            "Proposal should execute after reaching majority threshold"
        );

        // Verify proposal data is preserved in executed proposal
        assert_eq!(
            proposal_after_vote2["data"]["CustomProposal"]["title"],
            "Should we implement dark mode?",
            "Title should be preserved"
        );
        assert_eq!(
            proposal_after_vote2["data"]["CustomProposal"]["description"],
            "Community vote on adding dark mode theme to the platform",
            "Description should be preserved"
        );
        assert_eq!(
            proposal_after_vote2["data"]["CustomProposal"]["custom_data"]["feature"], "dark_mode",
            "Custom data should be preserved"
        );
        assert_eq!(
            proposal_after_vote2["data"]["CustomProposal"]["custom_data"]["estimated_cost"],
            "250 NEAR",
            "Custom data details should be preserved"
        );
        ...
        let late_vote = contract.execute(vote_proposal_request(
            "custom_approval_test".to_string(),
            proposal_id.clone(),
            true,
        ));
        assert!(
            late_vote.is_err(),
            "Should not be able to vote on executed proposal"
        );
        let error = late_vote.unwrap_err().to_string();
        assert!(
            error.contains("not active") || error.contains("executed"),
            "Expected 'not active' or 'executed' error, got: {}",
            error
        );
    ...
    fn test_custom_proposal_voting_rejection() {
        ...
        assert_eq!(
            ...
            "Proposal should be rejected when defeat becomes mathematically inevitable"
        );

        // Verify rejection details are preserved
        assert_eq!(
            proposal["data"]["CustomProposal"]["title"], "Should we ban all memes?",
            "Title should be preserved in rejected proposal"
        );
        assert_eq!(
            proposal["data"]["CustomProposal"]["custom_data"]["controversial"], true,
            "Custom data should be preserved in rejected proposal"
        );
        ...
        let quorum_proposal_id = contract
            .execute(create_proposal_request(
                "custom_rejection_test".to_string(),
                "custom_proposal".to_string(),
                quorum_test_data,
        ...
        let quorum_proposal_path = format!(
            "groups/custom_rejection_test/proposals/{}",
            quorum_proposal_id
        );
        let quorum_proposal = contract
            .platform
            .storage_get(&quorum_proposal_path)
            .unwrap();
        assert_eq!(
            quorum_proposal["status"], "rejected",
            "Proposal should be rejected due to insufficient participation"
        );
    ...
    fn test_custom_proposal_metadata_handling() {
        ...
        let complex_proposal_id = contract
            .execute(create_proposal_request(
        ...
        assert_eq!(
            preserved_data["custom_data"]["budget_year"], 2025,
            "Integer should be preserved"
        );
        assert_eq!(
            preserved_data["custom_data"]["total_amount"], 5000.5,
            "Float should be preserved"
        );
        assert_eq!(
            preserved_data["custom_data"]["currency"], "NEAR",
            "String should be preserved"
        );
        assert_eq!(
            preserved_data["custom_data"]["approved"], true,
        ...
        assert_eq!(
            ...
            "Nested object field should be preserved"
        );
        assert_eq!(
            engineering["allocation"], 2500.0,
            "Nested number should be preserved"
        );
        assert_eq!(
            engineering["projects"][0], "Platform upgrade",
            "Nested array element should be preserved"
        );

        // Check metadata object
        let metadata = &preserved_data["custom_data"]["metadata"];
        assert_eq!(
            metadata["created_by"], "finance_committee",
            "Deeply nested string should be preserved"
        );
        assert_eq!(
            metadata["reviewed_by"][0], "alice.near",
            "Nested array in object should be preserved"
        );
        assert_eq!(
            metadata["priority"], "high",
            "Metadata field should be preserved"
        );
        assert_eq!(
            metadata["tags"][1], "2025",
            "Tags array should be preserved"
        );
        assert_eq!(
            metadata["special_chars"], "UTF-8: 🚀 αβγ 中文 🎯",
            "Unicode characters should be preserved"
        );

        // Check null and empty values
        assert!(
            preserved_data["custom_data"]["null_field"].is_null(),
            "Null value should be preserved"
        );
        assert_eq!(
            preserved_data["custom_data"]["empty_array"]
                .as_array()
                .unwrap()
                .len(),
            0,
        ...
        assert!(
            preserved_data["custom_data"]["empty_object"]
                .as_object()
                .unwrap()
                .is_empty(),
        ...
        let minimal_proposal_id = contract
            .execute(create_proposal_request(
                "metadata_test".to_string(),
                "custom_proposal".to_string(),
                minimal_data.clone(),
        ...
        contract
            .execute(vote_proposal_request(
                "metadata_test".to_string(),
                minimal_proposal_id.clone(),
                true,
            ))
            .unwrap();

        // Verify minimal proposal executes and data is preserved
        let minimal_proposal_path =
            format!("groups/metadata_test/proposals/{}", minimal_proposal_id);
        let minimal_executed = contract
            .platform
            .storage_get(&minimal_proposal_path)
            .unwrap();
        assert_eq!(minimal_executed["status"], "executed");
        assert_eq!(
            minimal_executed["data"]["CustomProposal"]["title"],
            "Simple yes/no question"
        );
        assert_eq!(
            minimal_executed["data"]["CustomProposal"]["description"],
        ...
        let large_proposal_id = contract
            .execute(create_proposal_request(
                "metadata_test".to_string(),
                "custom_proposal".to_string(),
        ...
        assert_eq!(
            custom_data["risks_and_mitigations"]["technical_risks"][0]["risk"],
            "Scope creep"
        );
    ...
    fn test_custom_proposal_edge_cases_and_validation() {
        ...
        assert!(
            error_msg.contains("title") || error_msg.contains("required"),
            "Error should mention title requirement, got: {}",
            error_msg
        );
        ...
        let extreme_proposal_id = contract
            .execute(create_proposal_request(
                "edge_cases_test".to_string(),
                "custom_proposal".to_string(),
                extreme_data.clone(),
        ...
        contract
            .execute(vote_proposal_request(
                "edge_cases_test".to_string(),
                extreme_proposal_id.clone(),
                true,
            ))
            .unwrap();

        // Verify extreme data is preserved
        let extreme_proposal_path =
            format!("groups/edge_cases_test/proposals/{}", extreme_proposal_id);
        let extreme_executed = contract
            .platform
            .storage_get(&extreme_proposal_path)
            .unwrap();
        ...
        assert_eq!(
            custom_data["very_long_string"].as_str().unwrap().len(),
            1000,
            "Long string should be preserved"
        );
        assert_eq!(
            custom_data["deeply_nested"]["level1"]["level2"]["level3"]["level4"]["level5"]["data"],
            "deep value",
            "Deep nesting should be preserved"
        );
        assert_eq!(
            custom_data["array_with_many_elements"]
                .as_array()
                .unwrap()
                .len(),
            100,
        ...
        assert!(
            custom_data["special_characters"]
                .as_str()
                .unwrap()
                .contains("!@#$%"),
        ...
        assert!(
            custom_data["unicode_extreme"]
                .as_str()
                .unwrap()
                .contains("🚀"),
            "Extreme unicode should be preserved"
        );
        ...
        let vote_change_result = contract.execute(vote_proposal_request(
            "edge_cases_test".to_string(),
            vote_change_id.clone(),
            false,
        ));
        ...
        assert_eq!(
            prop1["status"], "active",
            "First concurrent proposal should be active"
        );
        assert_eq!(
            prop2["status"], "active",
            "Second concurrent proposal should be active"
        );
