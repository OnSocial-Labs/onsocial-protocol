...
mod group_creation_tests {
    ...
    fn test_basic_group_creation() {
        ...
        assert_eq!(
            config_data.get("owner"),
            Some(&json!(alice.to_string())),
            "Alice should be the owner"
        );
        assert_eq!(
            config_data.get("description"),
            Some(&json!("A test group")),
            "Description should match"
        );
        assert_eq!(
            config_data.get("is_private"),
            Some(&json!(false)),
            "Privacy setting should match"
        );
        assert_eq!(
            config_data.get("member_driven"),
            Some(&json!(false)),
            "Governance setting should match"
        );
        ...
        assert!(
            ...
            "Storage should have been used for group creation"
        );
    ...
    fn test_private_group_creation() {
        ...
        let context =
            get_context_with_deposit(charlie.clone(), 100_000_000_000_000_000_000_000_000); // 100 NEAR
        near_sdk::testing_env!(context.build());
    ...
    fn test_group_creation_insufficient_deposit() {
        ...
        assert!(
            result.is_err(),
            "Group creation with insufficient deposit should fail"
        );
        if let Err(error) = result {
            // Check that it's specifically a storage error
            let error_msg = format!("{:?}", error);
            assert!(
                error_msg.contains("InsufficientStorage"),
                "Should be storage error, got: {}",
                error_msg
            );
        }
    ...
    fn test_group_creation_storage_refund() {
        ...
        assert!(
            storage_info.used_bytes > 0,
            "Storage should be used for group creation"
        );
    ...
    fn test_group_creation_event_emission() {
        ...
        assert!(
            !logs.is_empty(),
            "Events should be emitted for group creation"
        );
        ...
        for log in &logs {
            ...
            let evt: Value =
                near_sdk::serde_json::from_str(json_str).expect("EVENT_JSON must be valid JSON");
            let data = evt
                .get("data")
    ...
    fn test_group_creation_default_config_values() {
        ...
        assert!(
            result.is_ok(),
            "Group creation with minimal config should succeed"
        );
        ...
        assert!(
            config_data.get("created_at").is_some(),
    ...
    fn test_multiple_groups_creation() {
        ...
        // Set up context for charlie
        let context =
            get_context_with_deposit(charlie.clone(), 500_000_000_000_000_000_000_000_000); // 500 NEAR
        near_sdk::testing_env!(context.build());
        ...
        let groups = vec![
            ...
            (
                "group2",
                json!({"description": "Second group", "is_private": true}),
            ),
            (
                "group3",
                json!({"description": "Third group", "member_driven": true}),
            ),
        ];
        ...
        assert_ne!(
            group1_config.get("description"),
            group2_config.get("description")
        );
        assert_ne!(
            group1_config.get("is_private"),
            group3_config.get("is_private")
        );
    ...
    fn test_ownership_transfer_api_signature() {
        ...
        assert!(
            result.is_err(),
            "Transfer to non-member should fail (but API signature works)"
        );
    ...
    fn test_ownership_transfer_basic_validation() {
        ...
        assert!(
            result.is_err(),
            "Should not be able to transfer to non-member"
        );
        let error_msg = format!("{:?}", result.unwrap_err());
        assert!(
            error_msg.contains("must be a member"),
        ...
        assert!(
            result.is_err(),
            "Should not be able to transfer to yourself"
        );

        // Test 3: Non-owner cannot transfer ownership
        let context =
            get_context_with_deposit(charlie.clone(), 500_000_000_000_000_000_000_000_000);
        near_sdk::testing_env!(context.build());
    ...
    fn test_group_creation_gas_cost() {
        ...
        println!(
            "Group creation gas used: {} gas ({:.3} TGas)",
            gas_used, gas_used_tgas
        );
        println!(
            "Estimated NEAR cost for gas: {:.8} NEAR",
            estimated_cost_near
        );
...
mod group_config_parsing_tests {
    ...
    fn test_try_from_value_invalid_owner_format() {
        ...
        assert!(
            result.is_err(),
            "Invalid owner account ID should return Err"
        );
    ...
    fn test_try_from_value_optional_fields_missing() {
        ...
        assert!(
            result.is_ok(),
            "Minimal config with only owner should succeed"
        );

        let config = result.unwrap();
        assert_eq!(config.owner.as_str(), "bob.testnet");
        assert!(
            !config.member_driven,
            "member_driven should default to false"
        );
