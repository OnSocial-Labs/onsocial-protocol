...
mod voting_edge_cases_tests {
    ...
    fn test_early_execution_when_majority_reached() {
        ...
        testing_env!(
            get_context_with_deposit(alice.clone(), test_deposits::proposal_creation()).build()
        );
        let config = json!({"member_driven": true, "is_private": true});
        contract
            .execute(create_group_request("early_exec_test".to_string(), config))
            .unwrap();
        ...
        testing_env!(
            get_context_with_deposit(charlie.clone(), test_deposits::member_operations()).build()
        );
        contract
            .execute(vote_proposal_request(
                "early_exec_test".to_string(),
        ...
        assert!(
            contract.is_blacklisted("early_exec_test".to_string(), eve.clone()),
            "Eve should be banned (proposal auto-executed with 60% YES votes)"
        );
        assert!(
            !contract.is_group_member("early_exec_test".to_string(), eve.clone()),
            "Eve should be removed from group"
        );

        // Verify dave cannot vote (proposal already executed)
        testing_env!(
            get_context_with_deposit(dave.clone(), test_deposits::member_operations()).build()
        );
        ...
        assert!(
            result.is_err(),
            "Should not allow voting on executed proposal"
        );
    }
    ...
    fn test_early_rejection_when_defeat_inevitable() {
        ...
        let config = json!({"member_driven": true, "is_private": true});
        contract
            .execute(create_group_request("rejection_test".to_string(), config))
            .unwrap();
        ...
        assert!(
            !contract.is_blacklisted("rejection_test".to_string(), eve.clone()),
            "Eve should not be banned (proposal rejected)"
        );
        assert!(
            contract.is_group_member("rejection_test".to_string(), eve.clone()),
            "Eve should still be in group"
        );

        // Verify eve cannot vote on rejected proposal
        testing_env!(
            get_context_with_deposit(eve.clone(), test_deposits::member_operations()).build()
        );
        let result = contract.execute(vote_proposal_request(
            "rejection_test".to_string(),
            proposal_id.clone(),
        ...
        assert!(
            result.is_err(),
            "Should not allow voting on rejected proposal"
        );
    }
    ...
    fn test_no_early_execution_if_quorum_not_met() {
        ...
        let config = json!({"member_driven": true, "is_private": true});
        contract
            .execute(create_group_request("quorum_test".to_string(), config))
            .unwrap();
        ...
        assert!(
            !contract.is_blacklisted("quorum_test".to_string(), eve.clone()),
            "Eve should not be banned: 40% participation < 51% quorum"
        );
        assert!(
            contract.is_group_member("quorum_test".to_string(), eve.clone()),
            "Eve should still be in group"
        );
    ...
    fn test_member_joins_during_voting_cannot_vote() {
        ...
        let config = json!({"member_driven": true, "is_private": true});
        contract
            .execute(create_group_request("join_test".to_string(), config))
            .unwrap();
        ...
        testing_env!(
            get_context_with_deposit(alice.clone(), test_deposits::member_operations())
        ...
        assert!(
            contract.is_group_member("join_test".to_string(), eve.clone()),
        ...
        assert!(
            result.is_err(),
            "New member should NOT be able to vote on proposals created before they joined"
        );
        let error_msg = format!("{:?}", result.unwrap_err());
        assert!(
            error_msg.contains("joined the group after"),
            "Error should indicate member joined after proposal, got: {}",
            error_msg
        );
    ...
    fn test_member_removed_during_voting_vote_still_counts() {
        ...
        let config = json!({"member_driven": true, "is_private": true});
        contract
            .execute(create_group_request("removal_test".to_string(), config))
            .unwrap();
        ...
        assert!(
            !contract.is_group_member("removal_test".to_string(), bob.clone()),
        ...
        let group_data = contract
            .platform
            .storage_get("groups/removal_test/config")
            .unwrap();
        let description = group_data
            ...
            .unwrap_or("");
        assert_eq!(
            description, "Testing vote persistence",
    ...
    fn test_threshold_based_on_locked_member_count() {
        ...
        let config = json!({"member_driven": true, "is_private": true});
        contract
            .execute(create_group_request("threshold_test".to_string(), config))
            .unwrap();
        ...
        let group_data = contract
            .platform
            .storage_get("groups/threshold_test/config")
            .unwrap();
        let description = group_data
            .get("description")
            .and_then(|v| v.as_str())
        ...
        assert_ne!(
            ...
            "Proposal should not execute with 40% participation"
        );
        ...
        let current_stats = contract
            .platform
            .storage_get("groups/threshold_test/stats")
            .unwrap();
        let current_count = current_stats
            .get("total_members")
            .and_then(|v| v.as_u64())
            .unwrap();
    ...
    fn test_multiple_active_proposals_vote_isolation() {
        ...
        testing_env!(
            get_context_with_deposit(alice.clone(), test_deposits::proposal_creation() * 2)
                .block_timestamp(TEST_BASE_TIMESTAMP)
                .build()
        );
        let config = json!({"member_driven": true, "is_private": true});
        contract
            .execute(create_group_request("concurrent_test".to_string(), config))
            .unwrap();
        ...
        testing_env!(
            get_context_for_proposal(alice.clone())
                .block_timestamp(TEST_BASE_TIMESTAMP + 1)
                .build()
        );
        ...
        testing_env!(
            get_context_for_proposal(alice.clone())
                .block_timestamp(TEST_BASE_TIMESTAMP + 2)
                .build()
        );
        let proposal_b = json!({
            "update_type": "metadata",
        ...
        testing_env!(
            get_context_for_proposal(alice.clone())
                .block_timestamp(TEST_BASE_TIMESTAMP + 3)
                .build()
        );
        let proposal_c = json!({
            "update_type": "metadata",
        ...
        testing_env!(
            get_context_with_deposit(bob.clone(), test_deposits::member_operations() * 2).build()
        );
        contract
            .execute(vote_proposal_request(
                "concurrent_test".to_string(),
        ...
        testing_env!(
            get_context_with_deposit(charlie.clone(), test_deposits::member_operations() * 2)
                .build()
        );
        contract
            .execute(vote_proposal_request(
        ...
        let proposal_a_data = contract
            .platform
            .storage_get(&format!(
                "groups/concurrent_test/proposals/{}",
                proposal_a_id
            ))
            .unwrap();
        let proposal_a_status = proposal_a_data
            .get("status")
            .and_then(|v| v.as_str())
            .unwrap_or("");
        assert_eq!(
            proposal_a_status, "active",
            "Proposal A should still be active (43% participation < 51%)"
        );

        let proposal_b_data = contract
            .platform
            .storage_get(&format!(
                "groups/concurrent_test/proposals/{}",
                proposal_b_id
            ...
            .unwrap();
        let proposal_b_status = proposal_b_data
            .get("status")
            .and_then(|v| v.as_str())
            .unwrap_or("");
        assert_eq!(
            proposal_b_status, "active",
            "Proposal B should still be active (43% participation < 51%)"
        );

        let proposal_c_data = contract
            .platform
            .storage_get(&format!(
                "groups/concurrent_test/proposals/{}",
                proposal_c_id
            ...
            .unwrap();
        let proposal_c_status = proposal_c_data
            .get("status")
            .and_then(|v| v.as_str())
            .unwrap_or("");
        assert_eq!(
            proposal_c_status, "active",
            "Proposal C should still be active (43% participation < 51%)"
        );

        // Dave casts 4th vote on each proposal to reach quorum
        // But vote in separate contexts to avoid execution interference
        testing_env!(
            get_context_with_deposit(dave.clone(), test_deposits::member_operations() * 2).build()
        );
        contract
            .execute(vote_proposal_request(
                "concurrent_test".to_string(),
            ...
            .unwrap(); // A: 4 YES (100% approval) → EXECUTES
        ...
        let proposal_a_status = proposal_a_data
            ...
            .unwrap_or("");
        assert_eq!(
            proposal_a_status, "executed",
            "Proposal A should execute with 4 YES votes"
        );
        ...
        contract
            ...
            .unwrap(); // B: 1 YES, 3 NO (25% approval) → ACTIVE
        contract
            .execute(vote_proposal_request(
                "concurrent_test".to_string(),
                proposal_c_id.clone(),
            ...
            .unwrap(); // C: 3 YES, 1 NO (75% approval) → EXECUTES
        ...
        assert_eq!(
            proposal_b_status, "active",
            "Proposal B should remain active (25% approval < 50.01%)"
        );
        ...
        let proposal_c_status = proposal_c_data
            ...
            .unwrap_or("");
        assert_eq!(
            proposal_c_status, "executed",
            "Proposal C should execute with 3 YES votes"
        );

        // Verify vote tallies are independent
        let tally_a = contract
            .platform
            .storage_get(&format!("groups/concurrent_test/votes/{}", proposal_a_id))
            .unwrap();
        assert_eq!(
            tally_a.get("yes_votes").and_then(|v| v.as_u64()).unwrap(),
            4,
            "Proposal A should have 4 YES votes"
        );
        assert_eq!(
            tally_a.get("total_votes").and_then(|v| v.as_u64()).unwrap(),
            4,
            "Proposal A should have 4 total votes"
        );

        let tally_b = contract
            .platform
            .storage_get(&format!("groups/concurrent_test/votes/{}", proposal_b_id))
            .unwrap();
        assert_eq!(
            tally_b.get("yes_votes").and_then(|v| v.as_u64()).unwrap(),
            1,
            "Proposal B should have 1 YES vote"
        );
        assert_eq!(
            tally_b.get("total_votes").and_then(|v| v.as_u64()).unwrap(),
            4,
            "Proposal B should have 4 total votes"
        );

        let tally_c = contract
            .platform
            .storage_get(&format!("groups/concurrent_test/votes/{}", proposal_c_id))
            .unwrap();
        assert_eq!(
            tally_c.get("yes_votes").and_then(|v| v.as_u64()).unwrap(),
            3,
            "Proposal C should have 3 YES votes"
        );
        assert_eq!(
            tally_c.get("total_votes").and_then(|v| v.as_u64()).unwrap(),
            4,
            "Proposal C should have 4 total votes"
        );

        // Verify final state: Proposal C executed last, so description should be "Description C"
        let group_data = contract
            .platform
            .storage_get("groups/concurrent_test/config")
            .unwrap();
        let description = group_data
            .get("description")
            .and_then(|v| v.as_str())
        ...
        assert_eq!(
            ...
            "Description should be from Proposal C (executed last)"
        );

        println!(
            "✅ Concurrent proposals: Vote isolation verified - A executed, B active, C executed"
        );
    }
    ...
    fn test_conflicting_proposals_both_can_execute() {
        ...
        // Create member-driven group with 7 members
        testing_env!(
            get_context_with_deposit(alice.clone(), test_deposits::proposal_creation())
                .block_timestamp(TEST_BASE_TIMESTAMP)
                .build()
        );
        let config = json!({"member_driven": true, "is_private": true});
        contract
            .execute(create_group_request("conflict_test".to_string(), config))
            .unwrap();
        ...
        let proposal_a_data = contract
            .platform
            .storage_get(&format!("groups/conflict_test/proposals/{}", proposal_a_id))
            .unwrap();
        let proposal_a_status = proposal_a_data
            .get("status")
            .and_then(|v| v.as_str())
        ...
        let group_data = contract
            .platform
            .storage_get("groups/conflict_test/config")
            .unwrap();
        let description = group_data
            .get("description")
            .and_then(|v| v.as_str())
        ...
        assert_eq!(
            ...
            "Description should be from Proposal A"
        );
        ...
        testing_env!(
            get_context_with_deposit(frank.clone(), test_deposits::member_operations()).build()
        );
        contract
            .execute(vote_proposal_request(
                "conflict_test".to_string(),
        ...
        testing_env!(
            get_context_with_deposit(grace.clone(), test_deposits::member_operations()).build()
        );
        contract
            .execute(vote_proposal_request(
                "conflict_test".to_string(),
        ...
        let proposal_b_data = contract
            .platform
            .storage_get(&format!("groups/conflict_test/proposals/{}", proposal_b_id))
            .unwrap();
        let proposal_b_status = proposal_b_data
            .get("status")
            .and_then(|v| v.as_str())
            .unwrap_or("");
        assert_eq!(
            proposal_b_status, "executed",
            "Proposal B should also execute"
        );
        ...
        assert_eq!(
            ...
            "Description should be overwritten by Proposal B (last one wins)"
        );
    ...
    fn test_proposal_execution_order_is_vote_completion_order() {
        ...
        testing_env!(
            get_context_with_deposit(charlie.clone(), test_deposits::proposal_creation()).build()
        );
        contract
            .execute(vote_proposal_request(
                "execution_order_test".to_string(),
        ...
        testing_env!(
            get_context_with_deposit(henry.clone(), test_deposits::member_operations()).build()
        );
        contract
            .execute(vote_proposal_request(
                "execution_order_test".to_string(),
        ...
        let proposal_2_data = contract
            .platform
            .storage_get(&format!(
                "groups/execution_order_test/proposals/{}",
                proposal_2_id
            ))
            .unwrap();
        let proposal_2_status = proposal_2_data
            .get("status")
            .and_then(|v| v.as_str())
            .unwrap_or("");
        assert_eq!(
            proposal_2_status, "executed",
            "Proposal 2 should execute first"
        );

        let group_data = contract
            .platform
            .storage_get("groups/execution_order_test/config")
            .unwrap();
        let tag = group_data.get("tag").and_then(|v| v.as_str()).unwrap_or("");
        assert_eq!(
            tag, "second_created",
            "Proposal 2 should execute first despite being created second"
        );

        // Verify Proposal 1 is still active (Alice YES, Bob YES, Charlie NO = 3 votes, 27% < 51% quorum)
        let proposal_1_data = contract
            .platform
            .storage_get(&format!(
                "groups/execution_order_test/proposals/{}",
                proposal_1_id
            ...
            .unwrap();
        let proposal_1_status = proposal_1_data
            .get("status")
            .and_then(|v| v.as_str())
            .unwrap_or("");
        assert_eq!(
            proposal_1_status, "active",
            "Proposal 1 should still be active"
        );

        // Now Proposal 1 gets enough votes to execute (Iris YES, Jack YES = 5 total votes, still < 51% quorum)
        testing_env!(
            get_context_with_deposit(iris.clone(), test_deposits::member_operations()).build()
        );
        contract
            .execute(vote_proposal_request(
                "execution_order_test".to_string(),
        ...
        testing_env!(
            get_context_with_deposit(jack.clone(), test_deposits::member_operations()).build()
        );
        contract
            .execute(vote_proposal_request(
                "execution_order_test".to_string(),
        ...
        assert_eq!(
            proposal_1_status, "active",
            "Proposal 1 should still be active with 5 votes"
        );

        // Final vote from Kate makes it execute (6 votes = 55% > 51% quorum, 4 YES out of 6 = 67% approval)
        testing_env!(
            get_context_with_deposit(kate.clone(), test_deposits::member_operations()).build()
        );
        contract
            .execute(vote_proposal_request(
                "execution_order_test".to_string(),
        ...
        let proposal_1_status = proposal_1_data
            ...
            .unwrap_or("");
        assert_eq!(
            proposal_1_status, "executed",
            "Proposal 1 should now be executed"
        );
        ...
        assert_eq!(
            tag, "first_created",
            "Proposal 1 should execute second and overwrite tag"
        );

        println!(
            "✅ Execution order: Proposals execute in vote completion order, not creation order"
        );
    }
    ...
    fn test_voting_with_50_members() {
        ...
        // Create member-driven group
        testing_env!(
            get_context_with_deposit(alice.clone(), test_deposits::member_operations() * 5)
                .block_timestamp(TEST_BASE_TIMESTAMP)
                .build()
        );
        let config = json!({"member_driven": true, "is_private": true});
        contract
            .execute(create_group_request("large_community".to_string(), config))
            .unwrap();
        ...
        contract
            .platform
            .storage_set("groups/large_community/stats", &stats)
            .unwrap();
        ...
        for i in 1..=5 {
            ...
            testing_env!(
                get_context_with_deposit(
                    ...
                    test_deposits::member_operations()
                )
                .build()
            );
            contract
        ...
        contract
            .platform
            .storage_set(
                &format!("groups/large_community/votes/{}", proposal_id),
                &vote_tally,
            )
            .unwrap();
        ...
        let group_config = contract
            .platform
            .storage_get("groups/large_community/config")
            .unwrap();
        let mut updated_config = group_config.clone();
        updated_config["description"] = json!("Updated by 50-member community");
        contract
            .platform
            .storage_set("groups/large_community/config", &updated_config)
            .unwrap();

        // Mark proposal as executed
        let mut proposal_data = contract
            .platform
            .storage_get(&format!("groups/large_community/proposals/{}", proposal_id))
            .unwrap();
        proposal_data["status"] = json!("executed");
        contract
            .platform
            .storage_set(
                &format!("groups/large_community/proposals/{}", proposal_id),
                &proposal_data,
            )
            .unwrap();

        // Verify proposal executed with simulated 26 YES votes
        let proposal_data = contract
            .platform
            .storage_get(&format!("groups/large_community/proposals/{}", proposal_id))
            .unwrap();
        let proposal_status = proposal_data
            .get("status")
            .and_then(|v| v.as_str())
            .unwrap_or("");
        assert_eq!(
            proposal_status, "executed",
            "Proposal should execute with 26/50 votes (52% quorum met)"
        );
        ...
        let description = group_data
            ...
            .unwrap_or("");
        assert_eq!(
            description, "Updated by 50-member community",
        ...
        let tally = contract
            .platform
            .storage_get(&format!("groups/large_community/votes/{}", proposal_id))
            .unwrap();
        assert_eq!(
            tally.get("yes_votes").and_then(|v| v.as_u64()).unwrap(),
            26,
            "Should have 26 YES votes"
        );
        assert_eq!(
            tally.get("total_votes").and_then(|v| v.as_u64()).unwrap(),
            26,
            "Should have 26 total votes"
        );
        assert_eq!(
            tally
                ...
                .unwrap(),
            50,
            "Should be locked at 50 members"
        );
    ...
    fn test_voting_with_100_members_quorum_challenge() {
        ...
        contract
            .platform
            .storage_set("groups/massive_community/stats", &stats)
            .unwrap();
        ...
        let proposal_insufficient_id = contract
            .execute(create_proposal_request(
                "massive_community".to_string(),
                "group_update".to_string(),
        ...
        contract
            ...
            .storage_set(
                &format!(
                    "groups/massive_community/votes/{}",
                    proposal_insufficient_id
                ),
                &vote_tally_insufficient,
            )
            .unwrap();
        ...
        let proposal_data = contract
            .platform
            .storage_get(&format!(
                "groups/massive_community/proposals/{}",
                proposal_insufficient_id
            ))
            .unwrap();
        let proposal_status = proposal_data
        ...
        assert_eq!(
            ...
            "Proposal should remain active (50% participation < 51% quorum)"
        );

        // Verify description unchanged
        let group_data = contract
            .platform
            .storage_get("groups/massive_community/config")
            .unwrap();
        let description = group_data
            .get("description")
            .and_then(|v| v.as_str())
            .unwrap_or("");
        assert_ne!(
            description, "Insufficient quorum test",
            "Description should not change without quorum"
        );
        ...
        let proposal_sufficient_id = contract
            .execute(create_proposal_request(
                "massive_community".to_string(),
                "group_update".to_string(),
                proposal_sufficient,
        ...
        contract
            .platform
            .storage_set(
                &format!("groups/massive_community/votes/{}", proposal_sufficient_id),
                &vote_tally_sufficient,
            )
            .unwrap();
        ...
        contract
            .platform
            .storage_set("groups/massive_community/config", &updated_config)
            .unwrap();
        ...
        assert_eq!(
            proposal_status, "executed",
            "Proposal should execute with 52% participation (>51% quorum)"
        );
        ...
        let description = group_data
            ...
            .unwrap_or("");
        assert_eq!(
            description, "Sufficient quorum test",
            "Description should be updated when quorum met"
        );

        // Verify vote tallies
        let tally_insufficient = contract
            .platform
            .storage_get(&format!(
                "groups/massive_community/votes/{}",
                proposal_insufficient_id
        ...
        assert_eq!(
            tally_insufficient
                ...
                .unwrap(),
            50,
            "First proposal should have 50 votes"
        );

        let tally_sufficient = contract
            .platform
            .storage_get(&format!(
                "groups/massive_community/votes/{}",
                proposal_sufficient_id
        ...
        assert_eq!(
            tally_sufficient
                ...
                .unwrap(),
            52,
            "Second proposal should have 52 votes"
        );
        assert_eq!(
            tally_sufficient
                .get("yes_votes")
                ...
                .unwrap(),
            52,
            "Second proposal should have 52 YES votes"
        );

        println!(
            "✅ 100-member community: Quorum requirements prevent execution without sufficient participation"
        );
    }
    ...
    fn test_voting_gas_limits_with_large_community() {
        ...
        testing_env!(
            get_context_with_deposit(alice.clone(), test_deposits::proposal_creation() * 5)
                .block_timestamp(TEST_BASE_TIMESTAMP)
                .build()
        );
        let config = json!({"member_driven": true, "is_private": true});
        ...
        for i in 1..21 {
            // Add 20 members for meaningful scale test
        ...
        for i in 1..11 {
            // 10 votes to reach quorum (11/21 = 52% > 51%)
            let member_account: AccountId = format!("gas_member{}.near", i).parse().unwrap();
            testing_env!(
                get_context_with_deposit(
                    member_account.clone(),
                    test_deposits::proposal_creation()
                )
                .build()
            );
            ...
            assert!(
                vote_result.is_ok(),
                "Vote should succeed without gas limit issues"
            );
        }
        ...
        let proposal_data = contract
            .platform
            .storage_get(&format!(
                "groups/gas_test_community/proposals/{}",
                proposal_id
            ))
            .unwrap();
        let proposal_status = proposal_data
        ...
        assert_eq!(
            proposal_status, "executed",
            "Proposal should execute efficiently"
        );

        // Verify vote tally accuracy (demonstrates O(1) counter operations work correctly)
        let tally = contract
            .platform
            .storage_get(&format!("groups/gas_test_community/votes/{}", proposal_id))
            .unwrap();
        assert_eq!(
            tally.get("yes_votes").and_then(|v| v.as_u64()).unwrap(),
            11,
            "Should have 11 YES votes (alice + 10)"
        );
        assert_eq!(
            tally.get("total_votes").and_then(|v| v.as_u64()).unwrap(),
            11,
            "Should have 11 total votes"
        );

        // Test that proposal execution is also O(1) - doesn't iterate over all members
        let group_data = contract
            .platform
            .storage_get("groups/gas_test_community/config")
            .unwrap();
        let description = group_data
            .get("description")
            .and_then(|v| v.as_str())
            .unwrap_or("");
        assert_eq!(
            description, "Gas efficiency test",
        ...
        for i in 11..16 {
            // 5 more votes on second proposal
            let member_account: AccountId = format!("gas_member{}.near", i).parse().unwrap();
            testing_env!(
                get_context_with_deposit(
                    member_account.clone(),
            ...
            assert!(
                vote_result.is_ok(),
                "Concurrent voting should not cause gas issues"
            );
        }
        ...
        let proposal_2_status = proposal_2_data
            ...
            .unwrap_or("");
        assert_eq!(
            proposal_2_status, "active",
            "Second proposal should remain active (5 votes < 11 quorum)"
        );
    ...
    fn test_expired_proposal_cannot_execute() {
        ...
        let config = json!({"member_driven": true, "is_private": true});
        contract
            .execute(create_group_request("expiration_test".to_string(), config))
            .unwrap();
        ...
        let proposal_data = contract
            .platform
            .storage_get(&format!("groups/expiration_test/proposals/{}", proposal_id))
            .unwrap();
        let proposal_status = proposal_data
            .get("status")
            .and_then(|v| v.as_str())
        ...
        assert_eq!(
            proposal_status, "active",
            "Proposal should initially be active"
        );
        ...
        let vote_tally = contract
            .platform
            .storage_get(&format!("groups/expiration_test/votes/{}", proposal_id))
            .unwrap();
        let total_votes = vote_tally
            .get("total_votes")
            .and_then(|v| v.as_u64())
            .unwrap_or(0);
        assert_eq!(
            total_votes, 1,
            "Should have only alice's auto-vote initially"
        );
        ...
        testing_env!(
            get_context_with_deposit(bob.clone(), test_deposits::member_operations())
                .block_timestamp(expired_timestamp)
                .build()
        );
        let expired_vote_result = contract.execute(vote_proposal_request(
            "expiration_test".to_string(),
            proposal_id.clone(),
            true,
        ));
        assert!(
            expired_vote_result.is_err(),
            "Vote should fail after expiration"
        );

        // Verify error message indicates expiration or inactive status
        let error_msg = format!("{:?}", expired_vote_result.unwrap_err());
        assert!(
            error_msg.contains("Voting period has expired")
                || error_msg.contains("expired")
                || error_msg.contains("not active")
                || error_msg.contains("Proposal is not active"),
            "Error should indicate proposal is no longer voteable: {}",
            error_msg
        );
        ...
        assert_eq!(
            proposal_status, "active",
            "Expired proposal should remain 'active' status until explicitly handled"
        );
        ...
        assert_eq!(
            total_votes, 1,
            "Vote count should remain 1 (alice auto-vote only), bob's expired vote rejected"
        );

        // Verify description hasn't changed (proposal hasn't executed)
        let group_data = contract
            .platform
            .storage_get("groups/expiration_test/config")
            .unwrap();
        let description = group_data
            .get("description")
            .and_then(|v| v.as_str())
            .unwrap_or("");
        assert_ne!(
            description, "This proposal will expire",
            "Proposal should not have executed with only 1/5 votes"
        );

        // Charlie tries to vote after expiration (should also fail)
        testing_env!(
            get_context_with_deposit(charlie.clone(), test_deposits::member_operations())
                .block_timestamp(expired_timestamp + 1000)
                .build()
        );
        let charlie_vote_result = contract.execute(vote_proposal_request(
            "expiration_test".to_string(),
            proposal_id.clone(),
            false,
        ));
        assert!(
            charlie_vote_result.is_err(),
            "Charlie's vote should also fail after expiration"
        );
        ...
        assert_eq!(
            total_votes, 1,
            "Vote count should remain 1, all expired votes rejected"
        );
    ...
    fn test_failed_proposal_can_be_reproposed() {
        ...
        testing_env!(
            get_context_for_proposal(alice.clone())
                .block_timestamp(TEST_BASE_TIMESTAMP + 1000)
                .build()
        );
        ...
        let first_proposal_id = contract
            .execute(create_proposal_request(
                "resubmission_test".to_string(),
                "group_update".to_string(),
        ...
        testing_env!(
            get_context_with_deposit(dave.clone(), test_deposits::proposal_creation()).build()
        );
        contract
            .execute(vote_proposal_request(
                "resubmission_test".to_string(),
        ...
        let first_proposal_data = contract
            .platform
            .storage_get(&format!(
                "groups/resubmission_test/proposals/{}",
                first_proposal_id
            ))
            .unwrap();
        let first_status = first_proposal_data
            .get("status")
            .and_then(|v| v.as_str())
            .unwrap_or("");
        assert_eq!(
            first_status, "rejected",
            "First proposal should be rejected with 25% approval"
        );

        // Verify vote tally of first proposal
        let first_tally = contract
            .platform
            .storage_get(&format!(
                "groups/resubmission_test/votes/{}",
                first_proposal_id
            ))
            .unwrap();
        assert_eq!(
            first_tally
                ...
                .unwrap(),
            1,
            "First proposal should have 1 YES vote"
        );
        assert_eq!(
            first_tally
                .get("total_votes")
                ...
                .unwrap(),
            4,
            "First proposal should have 4 total votes"
        );

        // Verify description wasn't changed (proposal was rejected)
        let group_data = contract
            .platform
            .storage_get("groups/resubmission_test/config")
            .unwrap();
        let description = group_data
            .get("description")
            .and_then(|v| v.as_str())
            .unwrap_or("");
        assert_ne!(
            description, "Controversial change",
            "Description should not change for rejected proposal"
        );
        ...
        testing_env!(
            get_context_for_proposal(alice.clone())
                .block_timestamp(TEST_BASE_TIMESTAMP + 100000)
                .build()
        );
        let second_proposal_id = contract
            .execute(create_proposal_request(
                "resubmission_test".to_string(),
                "group_update".to_string(),
        ...
        assert_ne!(
            ...
            "Second proposal should have different ID"
        );

        let second_proposal_data = contract
            .platform
            .storage_get(&format!(
                "groups/resubmission_test/proposals/{}",
                second_proposal_id
            ...
            .unwrap();
        let second_status = second_proposal_data
            .get("status")
            .and_then(|v| v.as_str())
            .unwrap_or("");
        assert_eq!(
            second_status, "active",
            "Second proposal should be active despite identical content"
        );

        // Verify both proposals exist simultaneously
        let first_exists = contract
            .platform
            .storage_get(&format!(
                "groups/resubmission_test/proposals/{}",
                first_proposal_id
        ...
        assert_eq!(
            ...
            "Second proposal should execute successfully"
        );
        ...
        assert_eq!(
            description, "Controversial change",
            "Description should change when second proposal succeeds"
        );

        // Verify vote tallies are independent
        let second_tally = contract
            .platform
            .storage_get(&format!(
                "groups/resubmission_test/votes/{}",
                second_proposal_id
        ...
        assert_eq!(
            second_tally
                ...
                .unwrap(),
            3,
            "Second proposal should have 3 YES votes"
        );
        assert_eq!(
            second_tally
                .get("total_votes")
                ...
                .unwrap(),
            3,
            "Second proposal should have 3 total votes"
        );
        ...
        assert_eq!(
            first_tally
                ...
                .unwrap(),
            1,
            "First proposal votes unchanged"
        );
        assert_eq!(
            first_tally
                .get("total_votes")
                ...
                .unwrap(),
            4,
            "First proposal total unchanged"
        );
    ...
    fn test_proposal_state_after_expiration() {
        ...
        testing_env!(
            get_context_with_deposit(bob.clone(), test_deposits::proposal_creation())
                .block_timestamp(proposal_timestamp + 1000)
                .build()
        );
        contract
            .execute(vote_proposal_request(
        ...
        testing_env!(
            get_context_with_deposit(charlie.clone(), test_deposits::proposal_creation())
                .block_timestamp(proposal_timestamp + 2000)
                .build()
        );
        contract
            .execute(vote_proposal_request(
        ...
        let proposal_data = contract
            .platform
            .storage_get(&format!(
                "groups/expiration_state_test/proposals/{}",
                proposal_id
            ))
            .unwrap();
        let proposal_status = proposal_data
        ...
        if proposal_status == "executed" {
            ...
            testing_env!(
                get_context_for_proposal(alice.clone())
                    .block_timestamp(proposal_timestamp + 5000)
                    .build()
            );
            ...
            testing_env!(
                get_context_with_deposit(dave.clone(), test_deposits::member_operations())
                    .block_timestamp(expired_timestamp)
                    .build()
            );
            let expired_vote_result = contract.execute(vote_proposal_request(
            ...
            let expired_proposal_data = contract
                .platform
                .storage_get(&format!(
                    "groups/expiration_state_test/proposals/{}",
                    proposal_id
                ...
                .unwrap();
            let expired_status = expired_proposal_data
                .get("status")
                .and_then(|v| v.as_str())
                .unwrap_or("");
            ...
            assert_eq!(
                ...
                "Expired proposals maintain 'active' status - no automatic transition to 'expired'"
            );

            // Verify proposal data is preserved (not cleaned up)
            assert!(
                expired_proposal_data.get("data").is_some(),
                "Proposal data should be preserved"
            );
            assert!(
                expired_proposal_data.get("created_at").is_some(),
                "Creation timestamp should be preserved"
            );
            assert!(
                expired_proposal_data.get("type").is_some(),
                "Proposal type should be preserved"
            );
            let proposer = expired_proposal_data
                .get("proposer")
                .and_then(|v| v.as_str())
                .unwrap_or("");
            ...
            let vote_tally = contract
                .platform
                .storage_get(&format!(
                    "groups/expiration_state_test/votes/{}",
                    proposal_id
                ))
                .unwrap();
            assert_eq!(
                vote_tally
                    ...
                    .unwrap(),
                1,
                "Vote tally should be preserved"
            );
            assert_eq!(
                vote_tally
                    .get("yes_votes")
            ...
            let group_data = contract
                .platform
                .storage_get("groups/expiration_state_test/config")
                .unwrap();
            let description = group_data
                .get("description")
                .and_then(|v| v.as_str())
                .unwrap_or("");
            assert_ne!(
                description, "This one will really expire",
                "Expired proposal should not have executed"
            );
        } else {
            ...
            assert_eq!(
                expired_status, "active",
                "Expired proposals should maintain 'active' status"
            );

            // Verify all data is preserved
            assert!(
                expired_proposal_data.get("data").is_some(),
                "Proposal data preserved"
            );
            assert!(
                expired_proposal_data.get("created_at").is_some(),
        ...
        println!(
            "✅ Proposal expiration state: Status remains 'active', data preserved, voting blocked"
        );
    }
    ...
    fn test_vote_at_exact_expiration_time() {
        ...
        let config = json!({"member_driven": true, "is_private": true});
        contract
            .execute(create_group_request("timing_test".to_string(), config))
            .unwrap();
        ...
        let proposal_data = contract
            .platform
            .storage_get(&format!("groups/timing_test/proposals/{}", proposal_id))
            .unwrap();
        let proposal_status = proposal_data
            .get("status")
            .and_then(|v| v.as_str())
        ...
        assert_eq!(
            proposal_status, "active",
            "Proposal should be active initially"
        );
        ...
        testing_env!(
            get_context_with_deposit(bob.clone(), test_deposits::member_operations())
                .block_timestamp(just_before_expiry)
                .build()
        );
        let before_expiry_result = contract.execute(vote_proposal_request(
            "timing_test".to_string(),
            proposal_id.clone(),
            true,
        ));
        assert!(
            ...
            "Vote should succeed 1 nanosecond before expiration"
        );

        // Verify vote was counted
        let vote_tally = contract
            .platform
            .storage_get(&format!("groups/timing_test/votes/{}", proposal_id))
            .unwrap();
        let total_votes = vote_tally
            .get("total_votes")
            .and_then(|v| v.as_u64())
        ...
        testing_env!(
            get_context_with_deposit(charlie.clone(), test_deposits::member_operations())
                .block_timestamp(exact_expiry)
                .build()
        );
        let exact_expiry_result = contract.execute(vote_proposal_request(
            "timing_test".to_string(),
            proposal_id.clone(),
            true,
        ));
        ...
        assert!(
            exact_expiry_result.is_err(),
            "Vote should be rejected at exact expiration timestamp"
        );
        ...
        assert_eq!(
            total_votes, 2,
            "Vote count should remain 2 (charlie's vote rejected at exact expiry)"
        );
        ...
        testing_env!(
            get_context_with_deposit(dave.clone(), test_deposits::member_operations())
                .block_timestamp(after_expiry)
                .build()
        );
        let after_expiry_result = contract.execute(vote_proposal_request(
            "timing_test".to_string(),
            proposal_id.clone(),
            false,
        ));
        assert!(
            after_expiry_result.is_err(),
            "Vote should be rejected after expiration"
        );
        ...
        assert_eq!(
            total_votes, 2,
            "Vote count should remain 2 (dave's vote rejected after expiration)"
        );
    ...
    fn test_new_member_added_can_vote_immediately_on_new_proposals() {
        ...
        let proposal_a_data = contract
            .platform
            .storage_get(&format!(
                "groups/membership_timing_test/proposals/{}",
                proposal_a_id
            ))
            .unwrap();
        let proposal_a_status = proposal_a_data
        ...
        test_add_member_bypass_proposals_with_timestamp(
            ...
            WRITE,
            &alice,
            TEST_BASE_TIMESTAMP + 2000,
        );

        // Step 3: Charlie creates proposal C (Bob is now a member)
        testing_env!(
            get_context_with_deposit(charlie.clone(), test_deposits::proposal_creation())
                .block_timestamp(TEST_BASE_TIMESTAMP + 3000)
                .build()
        );
        let proposal_c = json!({
            "update_type": "metadata",
        ...
        testing_env!(
            get_context_with_deposit(bob.clone(), test_deposits::member_operations())
                .block_timestamp(TEST_BASE_TIMESTAMP + 4000)
                .build()
        );
        let bob_vote_on_a = contract.execute(vote_proposal_request(
            "membership_timing_test".to_string(),
            proposal_a_id.clone(),
            true,
        ));
        assert!(
            bob_vote_on_a.is_err(),
            "Bob should not be able to vote on proposal A (created before he joined)"
        );

        // Verify error indicates membership requirement
        let error_msg = format!("{:?}", bob_vote_on_a.unwrap_err());
        assert!(
            error_msg.contains("not a member")
                || error_msg.contains("permission denied")
                || error_msg.contains("member")
                || error_msg.contains("Cannot vote"),
            "Error should indicate Bob cannot vote on proposal A: {}",
            error_msg
        );

        // Verify vote count on proposal A remains unchanged (only Alice auto-voted)
        let vote_tally_a = contract
            .platform
            .storage_get(&format!(
                "groups/membership_timing_test/votes/{}",
                proposal_a_id
            ))
            .unwrap();
        let total_votes_a = vote_tally_a
            .get("total_votes")
            .and_then(|v| v.as_u64())
            .unwrap();
        assert_eq!(
            total_votes_a, 1,
            "Proposal A should still have only Alice's auto-vote"
        );

        // Step 5: Bob votes on proposal C (created after he joined) - should SUCCEED
        testing_env!(
            get_context_with_deposit(bob.clone(), test_deposits::member_operations())
                .block_timestamp(TEST_BASE_TIMESTAMP + 5000)
                .build()
        );
        let bob_vote_on_c = contract.execute(vote_proposal_request(
            "membership_timing_test".to_string(),
            proposal_c_id.clone(),
            true,
        ));
        assert!(
            bob_vote_on_c.is_ok(),
            "Bob should be able to vote on proposal C (created after he joined)"
        );

        // Verify vote was counted on proposal C
        let vote_tally_c = contract
            .platform
            .storage_get(&format!(
                "groups/membership_timing_test/votes/{}",
                proposal_c_id
            ...
            .unwrap();
        let total_votes_c = vote_tally_c
            .get("total_votes")
            .and_then(|v| v.as_u64())
            .unwrap();
        assert_eq!(
            total_votes_c, 2,
            "Proposal C should have Charlie's auto-vote + Bob's vote"
        );

        let yes_votes_c = vote_tally_c
            .get("yes_votes")
            .and_then(|v| v.as_u64())
            .unwrap();
        ...
        let proposal_c_data_after = contract
            .platform
            .storage_get(&format!(
                "groups/membership_timing_test/proposals/{}",
                proposal_c_id
            ...
            .unwrap();
        let proposal_c_status_after = proposal_c_data_after
            .get("status")
            .and_then(|v| v.as_str())
            .unwrap_or("");
        assert_eq!(
            proposal_c_status_after, "executed",
            "Proposal C should execute with sufficient votes"
        );

        // Verify the change was applied
        let group_data = contract
            .platform
            .storage_get("groups/membership_timing_test/config")
            .unwrap();
        let description = group_data
            .get("description")
            .and_then(|v| v.as_str())
            .unwrap_or("");
        assert_eq!(
            description, "Proposal C - created after Bob joins",
            "Proposal C changes should be applied"
        );

        // Step 7: Verify proposal A remains unchanged (insufficient votes)
        let proposal_a_data_after = contract
            .platform
            .storage_get(&format!(
                "groups/membership_timing_test/proposals/{}",
                proposal_a_id
            ...
            .unwrap();
        let proposal_a_status_after = proposal_a_data_after
            .get("status")
            .and_then(|v| v.as_str())
            .unwrap_or("");
        assert_eq!(
            proposal_a_status_after, "active",
            "Proposal A should remain active (insufficient votes)"
        );

        let group_data_after = contract
            .platform
            .storage_get("groups/membership_timing_test/config")
            .unwrap();
        let description_after = group_data_after
            .get("description")
            .and_then(|v| v.as_str())
            .unwrap_or("");
        assert_ne!(
            description_after, "Proposal A - created before Bob joins",
            "Proposal A changes should NOT be applied"
        );
        ...
        println!(
            "✅ New member voting rights: Proper timing enforcement prevents retroactive voting"
        );
    }
    ...
    fn test_storage_costs_for_voting() {
        ...
        testing_env!(
            get_context_with_deposit(alice.clone(), test_deposits::proposal_creation() * 3)
                .block_timestamp(TEST_BASE_TIMESTAMP)
                .build()
        );
        let config = json!({
        ...
        contract
            .execute(create_group_request("storage_test".to_string(), config))
            .unwrap();
        ...
        let stats = contract
            .platform
            .storage_get("groups/storage_test/stats")
            .unwrap();
        ...
        let proposal_data = contract
            .platform
            .storage_get(&format!("groups/storage_test/proposals/{}", proposal_id))
            .unwrap();
        assert!(
            proposal_data.get("id").is_some(),
            "Proposal ID should be stored"
        );
        assert!(
            proposal_data.get("data").is_some(),
            "Proposal data should be stored"
        );
        assert!(
            proposal_data.get("created_at").is_some(),
            "Creation timestamp should be stored"
        );
        assert_eq!(
            proposal_data
                .get("status")
        ...
        let vote_tally = contract
            .platform
            .storage_get(&format!("groups/storage_test/votes/{}", proposal_id))
            .unwrap();
        assert_eq!(
            vote_tally
                .get("total_votes")
                ...
                .unwrap(),
            1,
            "Should have 1 vote (alice auto-vote)"
        );
        assert_eq!(
            vote_tally
                .get("yes_votes")
        ...
        assert_eq!(
            vote_tally
                ...
                .unwrap(),
            5,
            "Member count should be locked at 5"
        );
        ...
        testing_env!(
            get_context_with_deposit(bob.clone(), test_deposits::member_operations())
                .block_timestamp(TEST_BASE_TIMESTAMP + 2000)
                .build()
        );
        contract
            .execute(vote_proposal_request(
        ...
        let bob_vote = contract
            .platform
            .storage_get(&format!(
                "groups/storage_test/votes/{}/{}",
                proposal_id, bob
            ))
            .unwrap();
        assert_eq!(
            bob_vote.get("voter").and_then(|v| v.as_str()).unwrap(),
            bob.as_str(),
        ...
        let proposal_data_after = contract
            .platform
            .storage_get(&format!("groups/storage_test/proposals/{}", proposal_id))
            .unwrap();
        assert_eq!(
            proposal_data_after
                ...
                .unwrap(),
            "active",
            "Proposal should remain active with 40% participation"
        );
        ...
        let description = group_config
            .get("description")
            .and_then(|v| v.as_str())
            .unwrap_or("(no description)");
        assert_ne!(
            description, "Storage cost test",
            "Description should not change until proposal executes (currently: {})",
            description
        );
        ...
        let proposal_data_final = contract
            .platform
            .storage_get(&format!("groups/storage_test/proposals/{}", proposal_id));
        ...
        assert_eq!(
            proposal_data_final
                ...
                .unwrap(),
            "executed",
            "Proposal should execute with 60% participation and 100% approval"
        );

        // Verify the change was applied
        let group_config_final = contract
            .platform
            .storage_get("groups/storage_test/config")
            .unwrap();
        assert_eq!(
            group_config_final
                .get("description")
                ...
                .unwrap(),
            "Storage cost test",
            "Description should change after proposal executes"
        );

        // Verify final vote tally
        let vote_tally_final = contract
            .platform
            .storage_get(&format!("groups/storage_test/votes/{}", proposal_id))
            .unwrap();
        assert_eq!(
    ...
    fn test_proposal_creation_requires_sufficient_storage() {
        ...
        let config = json!({"member_driven": true, "is_private": true});
        contract
            .execute(create_group_request("deposit_test".to_string(), config))
            .unwrap();
        ...
        let proposal_data = contract
            .platform
            .storage_get(&format!("groups/deposit_test/proposals/{}", proposal_id))
            .unwrap();
        assert_eq!(
            proposal_data
                .get("status")
        ...
        let proposal_2_data = contract
            .platform
            .storage_get(&format!("groups/deposit_test/proposals/{}", proposal_2_id))
            .unwrap();
        assert_eq!(
            proposal_2_data
                .get("status")
                ...
                .unwrap(),
            "active",
            "Second proposal should be created"
        );
        assert_ne!(
        ...
        let vote_tally = contract
            .platform
            .storage_get(&format!("groups/deposit_test/votes/{}", proposal_id))
            .unwrap();
        assert_eq!(
            vote_tally
                .get("total_votes")
                ...
                .unwrap(),
            2,
            "First proposal should have 2 votes"
        );

        // Second proposal should remain unaffected
        let second_vote_tally = contract
            .platform
            .storage_get(&format!("groups/deposit_test/votes/{}", proposal_2_id))
            .unwrap();
        assert_eq!(
            second_vote_tally
                .get("total_votes")
                ...
                .unwrap(),
            1,
            "Second proposal should have only Alice's auto-vote"
        );
    ...
    fn test_complex_proposal_execution_gas_limit() {
        ...
        let config = json!({"member_driven": true, "is_private": true});
        contract
            .execute(create_group_request("gas_limit_test".to_string(), config))
            .unwrap();
        ...
        contract
            .execute(vote_proposal_request(
                "gas_limit_test".to_string(),
                ban_proposal_id.clone(),
                true,
            ))
            .unwrap();

        testing_env!(
            get_context_with_deposit(dave.clone(), test_deposits::proposal_creation())
                .block_timestamp(TEST_BASE_TIMESTAMP + 3000)
                .build()
        );
        contract
        ...
        let ban_proposal_data = contract
            .platform
            .storage_get(&format!(
                "groups/gas_limit_test/proposals/{}",
                ban_proposal_id
            ))
            .unwrap();
        assert_eq!(
            ban_proposal_data
                ...
                .unwrap(),
            "executed",
            "Ban proposal should execute successfully after 3 votes"
        );

        // Verify Bob was actually banned (complex operations worked)
        assert!(
            contract.is_blacklisted("gas_limit_test".to_string(), bob.clone()),
            "Bob should be blacklisted"
        );
        assert!(
            !contract.is_group_member("gas_limit_test".to_string(), bob.clone()),
            "Bob should be removed from group"
        );
        ...
        let permission_proposal_id = contract
            .execute(create_proposal_request(
                "gas_limit_test".to_string(),
                "group_update".to_string(),
                permission_proposal,
        ...
        testing_env!(
            get_context_with_deposit(charlie.clone(), test_deposits::proposal_creation())
                .block_timestamp(TEST_BASE_TIMESTAMP + 6000)
                .build()
        );
        contract
            .execute(vote_proposal_request(
                "gas_limit_test".to_string(),
                permission_proposal_id.clone(),
                true,
            ))
            .unwrap();
        ...
        testing_env!(
            get_context_with_deposit(dave.clone(), test_deposits::proposal_creation())
                .block_timestamp(TEST_BASE_TIMESTAMP + 7000)
                .build()
        );
        contract
            .execute(vote_proposal_request(
        ...
        let permission_proposal_data = contract
            .platform
            .storage_get(&format!(
                "groups/gas_limit_test/proposals/{}",
                permission_proposal_id
            ))
            .unwrap();
        assert_eq!(
            permission_proposal_data
                .get("status")
                .and_then(|v| v.as_str())
                .unwrap(),
            "executed",
            "Permission change proposal should execute successfully after 3 votes"
        );
        ...
        let voting_config = group_config
            .get("voting_config")
        ...
        assert_eq!(
            voting_config
                ...
                .unwrap(),
            6000,
            "Quorum should be updated"
        );

        // Test 3: Multiple complex proposals can execute in sequence
        testing_env!(
            get_context_with_deposit(alice.clone(), test_deposits::proposal_creation())
                .block_timestamp(TEST_BASE_TIMESTAMP + 9000)
                .build()
        );
        ...
        let unban_proposal_id = contract
            .execute(create_proposal_request(
                "gas_limit_test".to_string(),
                "group_update".to_string(),
                unban_proposal,
        ...
        testing_env!(
            get_context_with_deposit(charlie.clone(), test_deposits::member_operations())
                .block_timestamp(TEST_BASE_TIMESTAMP + 10000)
                .build()
        );
        contract
            .execute(vote_proposal_request(
                "gas_limit_test".to_string(),
                unban_proposal_id.clone(),
                true,
            ))
            .unwrap();
        ...
        testing_env!(
            get_context_with_deposit(dave.clone(), test_deposits::member_operations())
                .block_timestamp(TEST_BASE_TIMESTAMP + 11000)
                .build()
        );
        contract
            .execute(vote_proposal_request(
        ...
        let unban_proposal_data = contract
            .platform
            .storage_get(&format!(
                "groups/gas_limit_test/proposals/{}",
                unban_proposal_id
        ...
        assert_eq!(
            unban_proposal_data
                ...
                .unwrap(),
            "executed",
            "Unban proposal should execute successfully after 3 votes"
        );

        // Verify Bob was unbanned
        assert!(
            !contract.is_blacklisted("gas_limit_test".to_string(), bob.clone()),
            "Bob should be unbanned"
        );
    ...
    fn test_remove_member_who_has_active_proposals() {
        ...
        let config = json!({"member_driven": true, "is_private": true});
        contract
            .execute(create_group_request("cascading_test".to_string(), config))
            .unwrap();
        ...
        let proposal_a_data = contract
            .platform
            .storage_get(&format!(
                "groups/cascading_test/proposals/{}",
                proposal_a_id
            ))
            .unwrap();
        assert_eq!(
        ...
        assert_eq!(
            proposal_a_data
                ...
                .unwrap(),
            "alice.near",
            "Alice should be the proposer"
        );

        let vote_tally_a = contract
            .platform
            .storage_get(&format!("groups/cascading_test/votes/{}", proposal_a_id))
            .unwrap();
        assert_eq!(
            vote_tally_a
                .get("total_votes")
        ...
        let proposal_b_data_after = contract
            .platform
            .storage_get(&format!(
                "groups/cascading_test/proposals/{}",
                proposal_b_id
            ...
            .unwrap();
        assert_eq!(
            proposal_b_data_after
                .get("status")
                .and_then(|v| v.as_str())
                .unwrap(),
            "executed",
            "Proposal B should execute"
        );
        assert!(
            !contract.is_group_member("cascading_test".to_string(), charlie.clone()),
            "Charlie should be removed from group"
        );

        // Step 4: Verify proposal A still exists and is active
        let proposal_a_after_removal = contract
            .platform
            .storage_get(&format!(
                "groups/cascading_test/proposals/{}",
                proposal_a_id
            ...
            .unwrap();
        assert_eq!(
            proposal_a_after_removal
                .get("status")
                .and_then(|v| v.as_str())
                .unwrap(),
            "active",
            "Proposal A should still be active after Alice's removal"
        );
        assert_eq!(
            proposal_a_after_removal
                .get("proposer")
                ...
                .unwrap(),
            "alice.near",
            "Alice should still be recorded as proposer"
        );

        // Verify vote tally for proposal A is preserved
        let vote_tally_a_after = contract
            .platform
            .storage_get(&format!("groups/cascading_test/votes/{}", proposal_a_id))
            .unwrap();
        assert_eq!(
            vote_tally_a_after
                ...
                .unwrap(),
            1,
            "Alice's vote should still count"
        );
        assert_eq!(
            vote_tally_a_after
                .get("locked_member_count")
                ...
                .unwrap(),
            5,
            "Original member count should be locked"
        );
        ...
        assert_eq!(
            proposal_a_final
                ...
                .unwrap(),
            "executed",
            "Proposal A should execute after reaching quorum"
        );

        // Verify the change was applied
        let group_config = contract
            .platform
            .storage_get("groups/cascading_test/config")
            .unwrap();
        assert_eq!(
            group_config
                .get("description")
                .and_then(|v| v.as_str())
                .unwrap(),
            "Alice's proposal - should survive her removal",
            "Alice's proposal changes should be applied"
        );

        // Verify final vote tally for proposal A
        let vote_tally_a_final = contract
            .platform
            .storage_get(&format!("groups/cascading_test/votes/{}", proposal_a_id))
            .unwrap();
        assert_eq!(
            vote_tally_a_final
                ...
                .unwrap(),
            3,
            "Should have 3 total votes (alice, bob, dave)"
        );
        assert_eq!(
            vote_tally_a_final
                .get("yes_votes")
    ...
    fn test_change_permissions_of_proposal_creator() {
        ...
        let bob_proposal_data = contract
            .platform
            .storage_get(&format!(
                "groups/permission_cascade_test/proposals/{}",
                bob_proposal_id
            ))
            .unwrap();
        assert_eq!(
            bob_proposal_data
                ...
                .unwrap(),
            "active",
            "Bob's proposal should be active"
        );
        assert_eq!(
            bob_proposal_data
                .get("proposer")
        ...
        let permission_change_id = contract
            .execute(create_proposal_request(
                "permission_cascade_test".to_string(),
                "group_update".to_string(),
        ...
        assert_eq!(
            permission_proposal_data
                ...
                .unwrap(),
            "executed",
            "Metadata change should execute"
        );
        ...
        let bob_proposal_after_permission_change = contract
            .platform
            .storage_get(&format!(
                "groups/permission_cascade_test/proposals/{}",
                bob_proposal_id
            ...
            .unwrap();
        assert_eq!(
            bob_proposal_after_permission_change
                .get("status")
                .and_then(|v| v.as_str())
                .unwrap(),
            "active",
            "Bob's proposal should remain active after his permissions changed"
        );
        assert_eq!(
            bob_proposal_after_permission_change
                .get("proposer")
                ...
                .unwrap(),
            "bob.near",
            "Bob should still be recorded as proposer"
        );

        // Verify vote tally is preserved
        let vote_tally_bob = contract
            .platform
            .storage_get(&format!(
                "groups/permission_cascade_test/votes/{}",
                bob_proposal_id
            ))
            .unwrap();
        assert_eq!(
            vote_tally_bob
                ...
                .unwrap(),
            1,
            "Bob's auto-vote should still count"
        );
        ...
        let bob_proposal_final = contract
            .platform
            .storage_get(&format!(
                "groups/permission_cascade_test/proposals/{}",
                bob_proposal_id
        ...
        assert_eq!(
            bob_proposal_final
                ...
                .unwrap(),
            "executed",
            "Bob's proposal should execute despite his permission change"
        );

        // Verify the change was applied
        let group_config = contract
            .platform
            .storage_get("groups/permission_cascade_test/config")
            .unwrap();
        assert_eq!(
            group_config
                .get("description")
                .and_then(|v| v.as_str())
                .unwrap(),
            "Bob's proposal - created with WRITE permissions",
            "Bob's proposal changes should be applied"
        );
    ...
    fn test_ban_member_who_voted_on_active_proposals() {
        ...
        let config = json!({"member_driven": true, "is_private": true});
        contract
            .execute(create_group_request("ban_cascade_test".to_string(), config))
            .unwrap();
        ...
        let vote_tally_a_before_ban = contract
            .platform
            .storage_get(&format!("groups/ban_cascade_test/votes/{}", proposal_a_id))
            .unwrap();
        assert_eq!(
            vote_tally_a_before_ban
                .get("total_votes")
                .and_then(|v| v.as_u64())
                .unwrap(),
            2,
            "Should have 2 votes (alice + bob)"
        );
        assert_eq!(
            vote_tally_a_before_ban
                .get("yes_votes")
                ...
                .unwrap(),
            2,
            "Both votes should be YES"
        );

        let bob_vote = contract
            .platform
            .storage_get(&format!(
                "groups/ban_cascade_test/votes/{}/{}",
                proposal_a_id, bob
            ))
            .unwrap();
        assert_eq!(
        ...
        testing_env!(
            get_context_with_deposit(charlie.clone(), test_deposits::proposal_creation() * 2)
                .block_timestamp(TEST_BASE_TIMESTAMP + 3000)
                .build()
        );
        let proposal_b = json!({
        ...
        let vote_tally_b_before_ban = contract
            .platform
            .storage_get(&format!("groups/ban_cascade_test/votes/{}", proposal_b_id))
            .unwrap();
        assert_eq!(
            vote_tally_b_before_ban
                .get("total_votes")
                .and_then(|v| v.as_u64())
                .unwrap(),
            2,
            "Should have 2 votes (charlie + bob)"
        );
        ...
        let ban_proposal_data = contract
            .platform
            .storage_get(&format!(
                "groups/ban_cascade_test/proposals/{}",
                ban_proposal_id
            ))
            .unwrap();
        assert_eq!(
            ban_proposal_data
                ...
                .unwrap(),
            "executed",
            "Ban proposal should execute"
        );
        assert!(
            contract.is_blacklisted("ban_cascade_test".to_string(), bob.clone()),
            "Bob should be blacklisted"
        );
        assert!(
            !contract.is_group_member("ban_cascade_test".to_string(), bob.clone()),
            "Bob should be removed from group"
        );

        // Step 7: Verify Bob's votes on proposal A and B still count
        let vote_tally_a_after_ban = contract
            .platform
            .storage_get(&format!("groups/ban_cascade_test/votes/{}", proposal_a_id))
            .unwrap();
        assert_eq!(
            vote_tally_a_after_ban
                .get("total_votes")
                .and_then(|v| v.as_u64())
                .unwrap(),
            2,
            "Proposal A should still have 2 votes"
        );
        assert_eq!(
            vote_tally_a_after_ban
                .get("yes_votes")
                ...
                .unwrap(),
            2,
            "Both YES votes should still count"
        );

        let vote_tally_b_after_ban = contract
            .platform
            .storage_get(&format!("groups/ban_cascade_test/votes/{}", proposal_b_id))
            .unwrap();
        assert_eq!(
            vote_tally_b_after_ban
                .get("total_votes")
                .and_then(|v| v.as_u64())
                .unwrap(),
            2,
            "Proposal B should still have 2 votes"
        );
        assert_eq!(
            vote_tally_b_after_ban
                .get("yes_votes")
        ...
        let bob_vote_a_after_ban = contract.platform.storage_get(&format!(
            "groups/ban_cascade_test/votes/{}/{}",
            proposal_a_id, bob
        ));
        assert!(
            bob_vote_a_after_ban.is_some(),
            "Bob's vote record on proposal A should be preserved"
        );

        let bob_vote_b_after_ban = contract.platform.storage_get(&format!(
            "groups/ban_cascade_test/votes/{}/{}",
            proposal_b_id, bob
        ));
        assert!(
            bob_vote_b_after_ban.is_some(),
            "Bob's vote record on proposal B should be preserved"
        );
        ...
        testing_env!(
            get_context_with_deposit(dave.clone(), test_deposits::proposal_creation())
                .block_timestamp(TEST_BASE_TIMESTAMP + 8000)
                .build()
        );
        contract
            .execute(vote_proposal_request(
        ...
        assert_eq!(
            proposal_a_final
                ...
                .unwrap(),
            "executed",
            "Proposal A should execute with Bob's vote counting"
        );

        // Verify the change was applied
        let group_config = contract
            .platform
            .storage_get("groups/ban_cascade_test/config")
            .unwrap();
        assert_eq!(
            group_config
                .get("description")
                .and_then(|v| v.as_str())
                .unwrap(),
            "Proposal A - Bob will vote on this",
            "Proposal A changes should be applied"
        );

        // Final verification: Bob's votes contributed to the execution
        let final_tally_a = contract
            .platform
            .storage_get(&format!("groups/ban_cascade_test/votes/{}", proposal_a_id))
            .unwrap();
        assert_eq!(
            final_tally_a
                ...
                .unwrap(),
            3,
            "Should have 3 votes total (alice, bob, dave)"
        );
        assert_eq!(
            final_tally_a
                .get("yes_votes")
        ...
        assert!(
            bob_vote_on_c.is_err(),
            "Bob should not be able to vote after being banned"
        );
