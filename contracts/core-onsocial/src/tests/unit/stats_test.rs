...
mod stats_counter_tests {
    ...
    fn test_counter_underflow_protection_member_count() {
        ...
        contract.platform.storage_set(&stats_path, &zero_stats)
        ...
        assert_eq!(stats_before.as_ref().unwrap().get("total_members").and_then(|v| v.as_u64()), Some(0));
        ...
        let member_count = stats_after.as_ref().unwrap().get("total_members").and_then(|v| v.as_u64());
        assert_eq!(member_count, Some(0), "Counter should remain 0, not underflow to u64::MAX");
    ...
    fn test_counter_underflow_protection_join_requests() {
        ...
        contract.platform.storage_set(&stats_path, &zero_stats).expect("Set stats");
        ...
        // Verify no underflow
        let stats_after = GroupStorage::get_group_stats(&contract.platform, group_id);
        let join_count = stats_after.as_ref().unwrap().get("total_join_requests").and_then(|v| v.as_u64());
    ...
    fn test_counter_increment() {
        ...
        let initial_count = initial_stats.as_ref().unwrap().get("total_members").and_then(|v| v.as_u64()).unwrap_or(0);

        // Increment
        let mut event_batch = EventBatch::new();
        GroupStorage::increment_member_count(&mut contract.platform, group_id, &alice, &mut event_batch)
            .expect("Increment should succeed");

        // Verify increment
        let after_stats = GroupStorage::get_group_stats(&contract.platform, group_id);
        let after_count = after_stats.as_ref().unwrap().get("total_members").and_then(|v| v.as_u64()).unwrap_or(0);
        assert_eq!(after_count, initial_count + 1, "Counter should increment by 1");
    ...
    fn test_last_updated_timestamp() {
        ...
        let last_updated = stats.as_ref().unwrap().get("last_updated").and_then(|v| v.as_str());
        assert!(last_updated.is_some(), "last_updated should be set");
        assert!(!last_updated.unwrap().is_empty(), "last_updated should not be empty");

        let created_at = stats.as_ref().unwrap().get("created_at").and_then(|v| v.as_str());
