...
mod governance_tests {
    ...
    fn test_owner_can_create_proposals() {
        ...
        contract
            .execute(create_group_request("demogroup".to_string(), config))
            .unwrap();
        ...
        assert!(
            ...
            "Owner should be able to create member invite proposal: {:?}",
            proposal_result
        );
    ...
    fn test_member_driven_group_proposal_workflow() {
        ...
        assert!(
            add_result.is_ok(),
            "Should add first member via immediate proposal execution: {:?}",
            add_result
        );

        // Verify member was added
        assert!(
            contract.is_group_member("demogroup".to_string(), existing_member.clone()),
        ...
        assert!(
            add_result.is_ok(),
            "Owner should be able to create member invitation proposal: {:?}",
            add_result
        );

        // Target member should NOT be immediately added to the group (proposal needs voting)
        assert!(
            !contract.is_group_member("demogroup".to_string(), target_member.clone()),
    ...
    fn test_path_permission_grant_proposal() {
        ...
        assert!(
            proposal_result.is_ok(),
            "Owner should be able to create path permission proposal: {:?}",
            proposal_result
        );
    ...
    fn test_non_member_cannot_create_proposals() {
        ...
        assert!(
            ...
            "Non-member should not be able to create proposals"
        );
    ...
    fn test_member_driven_vs_traditional_group_behavior() {
        ...
        assert!(
            direct_add_result.is_ok(),
            "Traditional group should allow direct member addition"
        );
        ...
        let proposal_add_result = contract.execute(add_group_member_request(
            "democratic".to_string(),
            target.clone(),
        ));
        assert!(
            proposal_add_result.is_ok(),
            "Member-driven group should create proposal for member addition"
        );

        // Verify target is not immediately added to member-driven group (needs voting)
        assert!(
            !contract.is_group_member("democratic".to_string(), target.clone()),
            "Target should not be immediately added to member-driven group"
        );

        // Verify target is immediately added to traditional group
        assert!(
            contract.is_group_member("traditional".to_string(), target.clone()),
    ...
    fn test_proposal_validation_for_owner_only() {
        ...
        let basic_result = contract.execute(create_proposal_request(
            "demogroup".to_string(),
            "member_invite".to_string(),
            basic_proposal,
            None,
        ...
        assert!(
            basic_result.is_ok(),
            "Owner should be able to create basic proposals: {:?}",
            basic_result
        );
        ...
        let elevated_result = contract.execute(create_proposal_request(
            "demogroup".to_string(),
            "path_permission_grant".to_string(),
            elevated_proposal,
            None,
        ...
        assert!(
            ...
            "Owner should be able to create elevated permission proposals: {:?}",
            elevated_result
        );

        println!(
            "✅ Proposal validation: Owner can create all types of proposals in member-driven groups"
        );
    }
    ...
    fn test_governance_system_overview() {
        ...
        assert!(
            contract.is_group_member("community".to_string(), owner.clone()),
        ...
        assert!(
            add_result.is_ok(),
            "Should create proposal for member addition"
        );
        assert!(
            !contract.is_group_member("community".to_string(), accounts(1)),
            "Member not added until proposal is approved"
        );

        // 2. Direct permission granting is blocked (goes through proposals)
        let permission_result = contract.execute(create_proposal_request(
            "community".to_string(),
            "path_permission_grant".to_string(),
            json!({
        ...
        assert!(
            ...
            "Should create path permission proposal"
        );

        // 3. Only members can create proposals (non-members blocked)
        testing_env!(
            get_context_with_deposit(accounts(3), 1_000_000_000_000_000_000_000_000).build()
        );
        let non_member_result = contract.execute(create_proposal_request(
            "community".to_string(),
            "member_invite".to_string(),
            json!({
    ...
    fn test_non_members_can_create_join_requests() {
        ...
        let join_request_result = contract.execute(create_proposal_request(
            "community".to_string(),
        ...
        assert!(
            join_request_result.is_ok(),
            "Non-members should be able to create join request proposals: {:?}",
            join_request_result
        );
        ...
        let member_invite_result = contract.execute(create_proposal_request(
            "community".to_string(),
            "member_invite".to_string(),
            member_invite_data,
            None,
        ...
        assert!(
            member_invite_result.is_err(),
            "Non-members should not be able to create member invite proposals"
        );

        println!(
            "✅ Join Request Exception: Non-members can create join requests but not other proposals"
        );
    }
    ...
    fn test_existing_members_can_create_proposals() {
        ...
        let member_proposal_result = contract.execute(add_group_member_request(
            "traditional".to_string(),
            accounts(2),
        ));

        assert!(
            member_proposal_result.is_ok(),
            "Existing members should be able to add other members in traditional groups"
        );
        ...
        println!(
            "✅ Member Proposal Rights: Existing members can create proposals/take actions based on their permissions"
        );
        println!(
            "   - In traditional groups: Members with sufficient permissions can act directly"
        );
        println!(
            "   - In member-driven groups: All members can create proposals (requires voting)"
        );
    }
    ...
    fn test_permission_levels_in_member_driven_groups() {
        ...
        assert!(
            direct_add_result.is_ok(),
            "Should create proposal instead of direct addition"
        );
        assert!(
            !contract.is_group_member("democratic".to_string(), write_member.clone()),
        ...
        let owner_proposal_result = contract.execute(create_proposal_request(
            "democratic".to_string(),
            "member_invite".to_string(),
            owner_proposal_data,
            None,
        ...
        println!(
            "   - Proposal creation: DEMOCRATIC (any member can create, regardless of permission level)"
        );
        println!(
            "   - Traditional hierarchy (MANAGE > MODERATE > WRITE) doesn't apply to proposal creation"
        );
    ...
    fn test_permission_grant_proposals() {
        ...
        testing_env!(
            get_context_with_deposit(charlie.clone(), 1_000_000_000_000_000_000_000_000).build()
        );
        ...
        assert!(
            result.is_err(),
            "Traditional groups should not support permission grant proposals"
        );
        ...
        testing_env!(
            get_context_with_deposit(alice.clone(), 1_000_000_000_000_000_000_000_000).build()
        );
        ...
        assert!(
            ...
            "Member-driven group should support permission grant proposals"
        );

        println!("✅ Permission Grant Proposals ONLY work in member-driven groups:");
        println!(
            "   - Traditional groups: Direct permission changes via add_group_member, grant_permissions"
        );
        println!(
            "   - Member-driven groups: All permission changes go through democratic proposals"
        );
    ...
    fn test_blacklist_ban_proposals() {
        ...
        let result_traditional = contract.execute(create_proposal_request(
            "traditional".to_string(),
        ...
        assert!(
            result_traditional.is_err(),
            "Traditional groups should not support ban proposals"
        );
        println!(
            "✅ Traditional groups don't support ban proposals (use direct blacklist_group_member instead)"
        );
        ...
        assert!(
            ...
            "Ban proposal should be created successfully"
        );
        ...
        assert!(
            !unban_proposal_id.is_empty(),
            "Unban proposal should be created successfully"
        );
        ...
        assert!(
            result.is_err(),
            "Ban proposal creation should fail with missing target_user"
        );
        println!("✅ Ban proposals require target_user at creation time");

        println!("✅ Democratic Blacklist/Ban System:");
        println!(
            "   - Traditional groups: Direct ban via blacklist_group_member (owner/admin only)"
        );
    ...
    fn test_banned_members_complete_restrictions() {
        ...
        assert!(
            ...
            "FIXED: Banned user cannot rejoin traditional public groups!"
        );
        println!(
            "✅ BUG FIXED: Banned user CANNOT rejoin traditional public groups (blacklist check now works!)"
        );
        ...
        contract
            .execute(create_group_request("private".to_string(), private_config))
            .unwrap();
        contract
            .execute(add_group_member_request(
                "private".to_string(),
        ...
        assert!(
            private_rejoin_result.is_err(),
            "Banned user should not be able to join private groups"
        );
    ...
    fn test_ban_proposal_voting_workflow() {
        ...
        contract
            .execute(create_group_request("democratic_ban".to_string(), config))
            .unwrap();
        ...
        assert!(
            contract.is_group_member("democratic_ban".to_string(), charlie.clone()),
        ...
        assert!(
            alice_vote.is_ok(),
            "Alice's YES vote should succeed: {:?}",
            alice_vote.err()
        );
        ...
        let dave_vote = contract.execute(vote_proposal_request(
            "democratic_ban".to_string(),
            proposal_id.clone(),
            true,
        ));
        assert!(
            dave_vote.is_ok(),
            "Dave's YES vote should succeed and trigger execution (governance authority bypasses permissions): {:?}",
            dave_vote.err()
        );
        ...
        if let Some(entry) = contract.platform.get_entry(&member_path) {
            assert!(
                matches!(entry.value, crate::state::models::DataValue::Deleted(_)),
        ...
        let charlie_proposal = contract.execute(create_proposal_request(
            "democratic_ban".to_string(),
            "group_update".to_string(),
        ...
        let charlie_vote = contract.execute(vote_proposal_request(
            "democratic_ban".to_string(),
    ...
    fn test_unban_proposal_voting_workflow() {
        ...
        contract
            .execute(create_group_request("democratic_unban".to_string(), config))
            .unwrap();
        ...
        assert!(
            contract.is_blacklisted("democratic_unban".to_string(), charlie.clone()),
        ...
        assert!(
            dave_vote.is_err(),
            "Dave's vote should fail because proposal already executed"
        );
    ...
    fn test_ban_proposal_rejection_scenarios() {
        ...
        assert!(
            contract.is_group_member("ban_rejection_test".to_string(), charlie.clone()),
        ...
        let rejection_proposal_id = contract
            .execute(create_proposal_request(
                "ban_rejection_test".to_string(),
                "group_update".to_string(),
        ...
        let invalid_result = contract.execute(create_proposal_request(
            "ban_rejection_test".to_string(),
            "group_update".to_string(),
            invalid_ban_data,
            None,
        ...
        assert!(
            ...
            "Ban proposal for non-member should be created (validation at execution)"
        );
    ...
    fn test_transfer_ownership_proposals() {
        ...
        assert!(
            !proposal_id.is_empty(),
            "Transfer ownership proposal should be created successfully"
        );
        ...
        assert!(
            result.is_err(),
            "Traditional groups should not support transfer ownership proposals"
        );
        println!(
            "✅ Traditional groups don't support transfer ownership proposals (use direct transfer_group_ownership instead)"
        );
        ...
        assert!(
            result.is_err(),
            "Transfer ownership proposal with missing new_owner should fail at creation"
        );
        println!("✅ Transfer ownership proposals validate new_owner at creation time (fail fast)");

        println!("✅ Democratic Ownership Transfer:");
        println!(
            "   - Traditional groups: Direct transfer via transfer_group_ownership (owner only)"
        );
    ...
    fn test_permission_change_proposals() {
        ...
        assert!(
            !proposal_id.is_empty(),
            "Permission change proposal should be created successfully"
        );
        ...
        assert!(
            result.is_err(),
            "Traditional groups should not support permission change proposals"
        );
        println!(
            "✅ Traditional groups don't support permission change proposals (use direct grant_permissions instead)"
        );
        ...
        assert!(
            result.is_err(),
            "Permission change proposal should require target_user and level"
        );
        ...
        println!(
            "   - Traditional groups: Direct permission changes via grant_permissions (hierarchical)"
        );
        println!(
            "   - Member-driven groups: All permission changes go through democratic proposals"
        );
    ...
    fn test_member_invite_proposals_comprehensive() {
        ...
        assert!(
            !proposal_id.is_empty(),
            "Member invite proposal should be created successfully"
        );
        ...
        assert!(
            !proposal_id2.is_empty(),
            "Member invite proposal should work with default permissions"
        );
        ...
        assert!(
            result.is_err(),
            "Traditional groups should not support member invite proposals"
        );
        println!(
            "✅ Traditional groups don't support member invite proposals (use direct add_group_member instead)"
        );
        ...
        assert!(
            result.is_err(),
            "Member invite proposal should require target_user"
        );
        ...
        println!(
            "   - Traditional groups: Direct member addition via add_group_member (hierarchical)"
        );
