...
mod voting_config_tests {
    ...
    fn test_group_creation_with_custom_voting_config() {
        ...
        contract
            .execute(create_group_request("strict_dao".to_string(), config))
            .unwrap();
    ...
    fn test_group_creation_with_default_voting_config() {
        ...
        contract
            .execute(create_group_request("default_dao".to_string(), config))
            .unwrap();

        // Verify default config was set
        let group_config = contract
            .get_group_config("default_dao".to_string())
            .unwrap();
        let voting_config = group_config.get("voting_config").unwrap();
    ...
    fn test_custom_config_affects_proposal_execution() {
        ...
        contract
            .execute(create_group_request("supermajority".to_string(), config))
            .unwrap();
        ...
        contract
            .platform
            .storage_set(
                &format!("groups/supermajority/members/{}", bob.as_str()),
                &member_data,
            )
            .unwrap();
        contract
            .platform
            .storage_set(
                &format!("groups/supermajority/members/{}", charlie.as_str()),
                &member_data,
            )
            .unwrap();
        ...
        contract
            .platform
            .storage_set("groups/supermajority/stats", &stats)
            .unwrap();
        ...
        let proposal_data =
            json!({"update_type": "metadata", "changes": {"description": "Test supermajority"}});
        ...
        let proposal = contract
            .platform
            .storage_get(&format!("groups/supermajority/proposals/{}", proposal_id))
            .unwrap();
        ...
        println!(
            "✅ Custom voting config (participation quorum + majority threshold) correctly applied"
        );
    }
    ...
    fn test_propose_voting_config_change() {
        ...
        contract
            .execute(create_group_request("evolving_dao".to_string(), config))
            .unwrap();
        ...
        assert!(
            result.is_ok(),
            "Should be able to propose voting config change"
        );
    ...
    fn test_execute_voting_config_change() {
        ...
        let config = json!({"member_driven": true, "is_private": true});
        contract
            .execute(create_group_request("solo_dao".to_string(), config))
            .unwrap();
        ...
        let proposal = contract
            .platform
            .storage_get(&format!("groups/solo_dao/proposals/{}", proposal_id))
            .unwrap();
        assert_eq!(
            proposal.get("status").unwrap().as_str().unwrap(),
            "executed"
        );
    ...
    fn test_new_config_applies_to_subsequent_proposals() {
        ...
        contract
            .execute(create_group_request("changing_dao".to_string(), config))
            .unwrap();
        ...
        contract
            .platform
            .storage_set(
                &format!("groups/changing_dao/members/{}", bob.as_str()),
                &member_data,
            )
            .unwrap();
        let charlie = test_account(2);
        contract
            .platform
            .storage_set(
                &format!("groups/changing_dao/members/{}", charlie.as_str()),
                &member_data,
            )
            .unwrap();
        ...
        contract
            .platform
            .storage_set(
                &format!("groups/changing_dao/members/{}", dave.as_str()),
                &member_data,
            )
            .unwrap();
        ...
        contract
            .platform
            .storage_set("groups/changing_dao/stats", &stats)
            .unwrap();
        ...
        let proposal = contract
            .platform
            .storage_get(&format!("groups/changing_dao/proposals/{}", config_prop_id))
            .unwrap();
        assert_eq!(proposal.get("status").unwrap().as_str().unwrap(), "active");
        ...
        let test_proposal =
            json!({"update_type": "metadata", "changes": {"description": "Testing new config"}});
        let test_prop_id = contract
            .execute(create_proposal_request(
                "changing_dao".to_string(),
                "group_update".to_string(),
                test_proposal,
        ...
        let proposal = contract
            .platform
            .storage_get(&format!("groups/changing_dao/proposals/{}", test_prop_id))
            .unwrap();
        assert_eq!(proposal.get("status").unwrap().as_str().unwrap(), "active");
    ...
    fn test_partial_config_update() {
        ...
        let config = json!({"member_driven": true, "is_private": true});
        contract
            .execute(create_group_request("partial_dao".to_string(), config))
            .unwrap();
        ...
        let group_config = contract
            .get_group_config("partial_dao".to_string())
            .unwrap();
        let voting_config = group_config.get("voting_config").unwrap();
    ...
    fn test_reject_invalid_quorum() {
        ...
        let config = json!({"member_driven": true, "is_private": true});
        contract
            .execute(create_group_request("invalid_dao".to_string(), config))
            .unwrap();
        ...
        assert!(
            result
                .unwrap_err()
                .to_string()
                .contains("between 100 and 10000")
        );
        ...
        assert!(
            result
                .unwrap_err()
                .to_string()
                .contains("Invalid participation_quorum_bps")
        );
    ...
    fn test_reject_invalid_threshold() {
        ...
        let config = json!({"member_driven": true, "is_private": true});
        contract
            .execute(create_group_request("invalid_dao2".to_string(), config))
            .unwrap();
        ...
        assert!(
            result
                .unwrap_err()
                .to_string()
                .contains("between 5001 and 10000")
        );
    ...
    fn test_reject_invalid_voting_period() {
        ...
        let config = json!({"member_driven": true, "is_private": true});
        contract
            .execute(create_group_request("invalid_dao3".to_string(), config))
            .unwrap();
        ...
        assert!(
            result
                .unwrap_err()
                .to_string()
                .contains("between 1 hour and 365 days")
        );
    ...
    fn test_reject_empty_config_change() {
        ...
        let config = json!({"member_driven": true, "is_private": true});
        contract
            .execute(create_group_request("empty_dao".to_string(), config))
            .unwrap();
        ...
        assert!(
            result
                .unwrap_err()
                .to_string()
                .contains("At least one voting config parameter")
        );
    ...
    fn test_extreme_but_valid_configs() {
        ...
        contract
            .execute(create_group_request("permissive_dao".to_string(), config))
            .unwrap();

        // Verify it was accepted
        let group_config = contract
            .get_group_config("permissive_dao".to_string())
            .unwrap();
    ...
    fn test_multiple_config_changes() {
        ...
        let config = json!({"member_driven": true, "is_private": true});
        contract
            .execute(create_group_request("evolving".to_string(), config))
            .unwrap();
    ...
    fn test_config_change_in_traditional_group() {
        ...
        contract
            .execute(create_group_request("traditional".to_string(), config))
            .unwrap();
    ...
    fn test_multi_member_config_change_voting() {
        ...
        contract
            .execute(create_group_request("vote_test_dao".to_string(), config))
            .unwrap();
        ...
        contract
            .platform
            .storage_set(
                &format!("groups/vote_test_dao/members/{}", bob.as_str()),
                &member_data,
            )
            .unwrap();
        contract
            .platform
            .storage_set(
                &format!("groups/vote_test_dao/members/{}", charlie.as_str()),
                &member_data,
            )
            .unwrap();
        let stats = json!({"total_members": 3, "total_join_requests": 0, "created_at": 0, "last_updated": 0});
        contract
            .platform
            .storage_set("groups/vote_test_dao/stats", &stats)
            .unwrap();
        ...
        let proposal = contract
            .platform
            .storage_get(&format!("groups/vote_test_dao/proposals/{}", proposal_id))
            .unwrap();
        assert_eq!(proposal.get("status").unwrap().as_str().unwrap(), "active");

        // Verify config is still old values
        let group_config = contract
            .get_group_config("vote_test_dao".to_string())
            .unwrap();
        let voting_config = group_config.get("voting_config").unwrap();
        assert_eq!(
            voting_config
        ...
        assert_eq!(
            voting_config
                ...
                .unwrap(),
            6600,
            "Participation quorum should be updated"
        );
        assert_eq!(
            voting_config
                .get("majority_threshold_bps")
                ...
                .unwrap(),
            7500,
            "Majority threshold should be updated"
        );
        let period = voting_config
            .get("voting_period")
            .and_then(|v| {
    ...
    fn test_config_change_metadata_tracking() {
        ...
        let config = json!({"member_driven": true, "is_private": true});
        contract
            .execute(create_group_request("metadata_dao".to_string(), config))
            .unwrap();

        // Verify no metadata before any changes
        let group_config = contract
            .get_group_config("metadata_dao".to_string())
            .unwrap();
        assert!(
            group_config.get("voting_config_updated_at").is_none(),
            "Should have no update timestamp initially"
        );
        assert!(
            group_config.get("voting_config_updated_by").is_none(),
            "Should have no updater initially"
        );
        ...
        let group_config = contract
            .get_group_config("metadata_dao".to_string())
            .unwrap();
        let updated_at = group_config.get("voting_config_updated_at").and_then(|v| {
            v.as_u64()
                .or_else(|| v.as_str().and_then(|s| s.parse::<u64>().ok()))
        });
        assert_eq!(
