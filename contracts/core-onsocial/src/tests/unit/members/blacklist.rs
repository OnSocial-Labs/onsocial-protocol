...
mod blacklist_tests {
    ...
    fn test_unblacklist_non_blacklisted_user() {
        ...
        assert!(
            !contract.is_blacklisted("test_group".to_string(), member.clone()),
            "Member should not be blacklisted initially"
        );
        ...
        if unblacklist_result.is_err() {
            ...
            assert!(
                error_msg.contains("not blacklisted") || error_msg.contains("not found"),
                "Should get appropriate error for non-blacklisted user: {}",
                error_msg
            );
        }
    ...
    fn test_owner_can_blacklist_member() {
        ...
        assert!(
            ...
            "Owner should be able to blacklist member: {:?}",
            blacklist_result
        );
        ...
        assert!(
            contract.is_blacklisted("test_group".to_string(), member.clone()),
            "Member should be blacklisted"
        );
        assert!(
            !contract.is_group_member("test_group".to_string(), member.clone()),
            "Member should be removed from group after blacklisting"
        );
    ...
    fn test_owner_can_unblacklist_member() {
        ...
        let readd_blacklisted_result = contract.execute(add_group_member_request(
            "test_group".to_string(),
            member.clone(),
        ));
        assert!(
            readd_blacklisted_result.is_err(),
            "Should not be able to add blacklisted user without unblacklisting first"
        );
        ...
        assert!(
            ...
            "Owner should be able to unblacklist member: {:?}",
            unblacklist_result
        );

        // Note: Check implementation behavior for is_blacklisted after unblacklisting
        let is_still_blacklisted =
            contract.is_blacklisted("test_group".to_string(), member.clone());
        println!(
            "Member blacklisted after unblacklist: {}",
        ...
        assert!(
            !contract.is_group_member("test_group".to_string(), member.clone()),
            "Member should not be automatically re-added to group"
        );

        // After unblacklisting, should be able to re-add successfully
        let readd_after_unblacklist = contract.execute(add_group_member_request(
            "test_group".to_string(),
            member.clone(),
        ));
        assert!(
            readd_after_unblacklist.is_ok(),
            "Should be able to add member after unblacklisting: {:?}",
            readd_after_unblacklist
        );
        assert!(
            contract.is_group_member("test_group".to_string(), member.clone()),
            "Member should be in group after re-adding"
        );
    ...
    fn test_blacklisted_member_automatically_removed_from_group() {
        ...
        assert!(
            !contract.is_group_member("test_group".to_string(), member.clone()),
            "Member should be automatically removed from group"
        );
        ...
        assert!(
            member_data_after.is_none() || member_data_after == Some(serde_json::Value::Null),
            "Member data should be null after blacklisting: {:?}",
            member_data_after
        );
    ...
    fn test_admin_can_blacklist_regular_members() {
        ...
        if blacklist_result.is_ok() {
            ...
            println!(
                "⚠️ Admin blacklist failed: {:?}",
                blacklist_result.as_ref().unwrap_err()
            );
    ...
    fn test_admin_cannot_blacklist_owner() {
        ...
        assert!(
            ...
            "Admin should not be able to blacklist owner"
        );

        let error_msg = format!("{:?}", blacklist_result.unwrap_err());
        // Could be permission error or specific owner protection error
        assert!(
            error_msg.contains("Cannot blacklist group owner") || error_msg.contains("Permission"),
        ...
        assert!(
            !contract.is_blacklisted("test_group".to_string(), owner.clone()),
            "Owner should not be blacklisted"
        );
        assert!(
            contract.is_group_member("test_group".to_string(), owner.clone()),
            "Owner should still be in group"
        );
    ...
    fn test_regular_member_cannot_blacklist_anyone() {
        ...
        assert!(
            !contract.is_blacklisted("test_group".to_string(), target_member.clone()),
            "Target should not be blacklisted"
        );
    ...
    fn test_member_driven_group_creates_ban_proposal() {
        ...
        contract
            .platform
            .storage_set(
                &format!("groups/demo_group/members/{}", existing_member.as_str()),
                &member_data,
            )
            .unwrap();
        let stats = json!({
        ...
        contract
            .platform
            .storage_set("groups/demo_group/stats", &stats)
            .unwrap();
        ...
        assert!(
            blacklist_result.is_ok(),
            "Member-driven group should create ban proposal: {:?}",
            blacklist_result
        );

        // Verify member is not immediately blacklisted (needs proposal voting)
        assert!(
            !contract.is_blacklisted("demo_group".to_string(), member.clone()),
            "Member should not be immediately blacklisted - proposal needs approval"
        );

        println!(
            "✅ Member-driven groups correctly create ban proposals instead of direct blacklisting"
        );
    }
    ...
    fn test_blacklist_non_member() {
        ...
        assert!(
            !contract.is_group_member("test_group".to_string(), non_member.clone()),
        ...
        assert!(
            blacklist_result.is_ok(),
            "Should be able to blacklist non-members (preemptive ban): {:?}",
            blacklist_result
        );

        // Verify non-member is blacklisted
        assert!(
            contract.is_blacklisted("test_group".to_string(), non_member.clone()),
            "Non-member should be blacklisted"
        );
        ...
        assert!(
            join_result.is_err(),
            "Blacklisted user should not be able to join group"
        );
    ...
    fn test_blacklist_already_blacklisted_user() {
        ...
        let second_blacklist_result = contract.execute(blacklist_group_member_request(
            "test_group".to_string(),
            member.clone(),
        ));
        assert!(
            second_blacklist_result.is_ok(),
            "Blacklisting already blacklisted user should be idempotent: {:?}",
            second_blacklist_result
        );

        // Verify still blacklisted
        assert!(
            contract.is_blacklisted("test_group".to_string(), member.clone()),
            "Member should still be blacklisted"
        );
    ...
    fn test_multiple_blacklist_unblacklist_cycles() {
        ...
        contract
            .execute(unblacklist_group_member_request(
                "test_group".to_string(),
                member.clone(),
            ))
            .unwrap();
        let is_blacklisted_after_unban =
            contract.is_blacklisted("test_group".to_string(), member.clone());
        println!(
            "Blacklisted after first unban: {}",
            is_blacklisted_after_unban
        );
        ...
        contract
            ...
            .unwrap();
        let is_blacklisted_after_second_unban =
            contract.is_blacklisted("test_group".to_string(), member.clone());
        println!(
            "Blacklisted after second unban: {}",
            is_blacklisted_after_second_unban
        );
    ...
    fn test_blacklist_nonexistent_group() {
        ...
        assert!(
            blacklist_result.is_err(),
            "Should not be able to blacklist in non-existent group"
        );
    ...
    fn test_blacklist_event_emission() {
        ...
        assert!(
            unblacklist_event_emitted,
            "Unblacklist event should be emitted"
        );
    ...
    fn test_blacklist_with_storage_costs() {
        ...
        assert!(
            blacklist_result.is_ok(),
            "Blacklist should succeed with sufficient storage: {:?}",
            blacklist_result
        );
    ...
    fn test_unblacklist_preserves_no_permissions() {
        ...
        let has_write_before = contract.has_permission(
            owner.clone(),
            member.clone(),
            "groups/test_group".to_string(),
            WRITE,
        );
        let has_moderate_before = contract.has_permission(
            owner.clone(),
            member.clone(),
            permission_path.to_string(),
            MODERATE,
        ...
        println!(
            "Before blacklist - Write permission: {}, Moderate permission: {}",
            has_write_before, has_moderate_before
        );
        ...
        let has_write_blacklisted = contract.has_permission(
            owner.clone(),
            member.clone(),
            "groups/test_group".to_string(),
            WRITE,
        );
        let has_moderate_blacklisted = contract.has_permission(
            owner.clone(),
            member.clone(),
            permission_path.to_string(),
            MODERATE,
        ...
        println!(
            "During blacklist - Write permission: {}, Moderate permission: {}",
            has_write_blacklisted, has_moderate_blacklisted
        );
        ...
        assert!(
            !contract.is_blacklisted("test_group".to_string(), member.clone()),
            "Member should no longer be blacklisted"
        );
        assert!(
            !contract.is_group_member("test_group".to_string(), member.clone()),
            "Member should NOT be automatically re-added to group"
        ...
        let has_write_after = contract.has_permission(
            owner.clone(),
            member.clone(),
            "groups/test_group".to_string(),
            WRITE,
        );
        let has_moderate_after = contract.has_permission(
            owner.clone(),
            member.clone(),
            permission_path.to_string(),
            MODERATE,
        ...
        println!(
            "After unblacklist - Write permission: {}, Moderate permission: {}",
            has_write_after, has_moderate_after
        );

        // Verify permissions are GONE (membership-based security prevents stale permissions)
        assert!(
            !has_write_after,
            "Write permissions should be gone (member was removed from group)"
        );
        assert!(
            !has_moderate_after,
            "Moderate permissions should be gone (member is no longer in group)"
        );
        ...
        assert!(
            contract.is_group_member("test_group".to_string(), member.clone()),
            "Member should be able to rejoin after unblacklisting"
        );

        // Check permissions after rejoining - old permissions should NOT resurrect
        let has_moderate_after_rejoin = contract.has_permission(
            owner.clone(),
            member.clone(),
            permission_path.to_string(),
            MODERATE,
        ...
        assert!(
            !has_moderate_after_rejoin,
            "Old permissions should not resurrect after rejoining"
        );
