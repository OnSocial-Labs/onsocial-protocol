...
mod member_security_tests {
    ...
    fn test_prevent_self_permission_grant() {
        ...
        let self_grant_result = contract.add_group_member("test_group".to_string(), member.clone(), MANAGE, None);
        
        if self_grant_result.is_err() {
            let error_msg = format!("{:?}", self_grant_result.unwrap_err());
            assert!(error_msg.contains("permission") || error_msg.contains("denied") || error_msg.contains("already exists"), 
    ...
    fn test_prevent_circular_permission_delegation() {
        ...
        let config = json!({"member_driven": false, "is_private": false});
        contract.create_group("test_group".to_string(), config).unwrap();
        contract.add_group_member("test_group".to_string(), user_a.clone(), MODERATE, None).unwrap();
        contract.add_group_member("test_group".to_string(), user_b.clone(), MODERATE, None).unwrap();
        ...
        let grant_result = contract.add_group_member("test_group".to_string(), user_b.clone(), MANAGE, None);
        ...
        let grant_result_2 = contract.add_group_member("test_group".to_string(), user_a.clone(), MANAGE, None);
    ...
    fn test_prevent_permission_escalation_via_group_operations() {
        ...
        let config = json!({"member_driven": false, "is_private": false});
        contract.create_group("test_group".to_string(), config).unwrap();
        contract.add_group_member("test_group".to_string(), attacker.clone(), WRITE, None).unwrap();
        ...
        let blacklist_result = contract.blacklist_group_member("test_group".to_string(), owner.clone(), None);
        ...
        let remove_result = contract.remove_group_member("test_group".to_string(), owner.clone(), None);
        ...
        // This should fail because self-join in public groups is limited to WRITE only
        let join_result = contract.join_group("test_group".to_string(), MANAGE);
        assert!(join_result.is_err(), "Self-join with MANAGE permission should fail");
        assert!(join_result.unwrap_err().to_string().contains("Self-join in public groups is limited to WRITE permission only"));
    ...
    fn test_prevent_blacklist_bypass_via_rejoin() {
        ...
        let config = json!({"member_driven": false, "is_private": false});
        contract.create_group("test_group".to_string(), config).unwrap();
        contract.add_group_member("test_group".to_string(), bad_user.clone(), WRITE, None).unwrap();

        // Blacklist the user
        contract.blacklist_group_member("test_group".to_string(), bad_user.clone(), None).unwrap();
        ...
        let rejoin_result = contract.join_group("test_group".to_string(), WRITE);
    ...
    fn test_prevent_path_traversal_permission_attacks() {
        ...
        contract.add_group_member("group_a".to_string(), attacker.clone(), WRITE, None).unwrap();
        ...
        let add_result = contract.add_group_member("group_b".to_string(), fake_user.clone(), WRITE, None);
        ...
        let blacklist_result = contract.blacklist_group_member("group_b".to_string(), fake_user.clone(), None);
    ...
    fn test_cross_group_permission_isolation() {
        ...
        // Give user MANAGE permissions in admin_group
        contract.add_group_member("admin_group".to_string(), user.clone(), MANAGE, None).unwrap();
        
        // Give user only WRITE permissions in user_group
        contract.add_group_member("user_group".to_string(), user.clone(), WRITE, None).unwrap();
        contract.add_group_member("user_group".to_string(), target.clone(), WRITE, None).unwrap();
        ...
        let _admin_blacklist_result = contract.blacklist_group_member("admin_group".to_string(), target.clone(), None);
        ...
        let user_blacklist_result = contract.blacklist_group_member("user_group".to_string(), target.clone(), None);
    ...
    fn test_prevent_permission_timing_window_attacks() {
        ...
        let config = json!({"member_driven": false, "is_private": false});
        contract.create_group("test_group".to_string(), config).unwrap();
        contract.add_group_member("test_group".to_string(), user.clone(), MANAGE, None).unwrap();
        contract.add_group_member("test_group".to_string(), target.clone(), WRITE, None).unwrap();

        // Owner removes user from group (revokes all permissions)
        contract.remove_group_member("test_group".to_string(), user.clone(), None).unwrap();
        ...
        let blacklist_result = contract.blacklist_group_member("test_group".to_string(), target.clone(), None);
        ...
        let add_result = contract.add_group_member("test_group".to_string(), new_member.clone(), WRITE, None);
