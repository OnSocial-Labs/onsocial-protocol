...
mod core_member_operations {
    ...
    fn test_owner_add_member_to_traditional_group() {
        ...
        assert!(
            add_result.is_ok(),
            "Member addition by owner should succeed"
        );
        ...
        assert_eq!(
            data.get("level"),
            Some(&json!(0)),
            "Member should start member-only"
        );
    ...
    fn test_non_owner_cannot_add_members() {
        ...
        let context =
            get_context_with_deposit(non_owner.clone(), 1_000_000_000_000_000_000_000_000);
        near_sdk::testing_env!(context.build());
        ...
        assert!(
            error_msg.contains("Permission denied") || error_msg.contains("permission denied"),
    ...
    fn test_add_member_with_storage_costs() {
        ...
        assert!(
            add_result.is_ok(),
            "Member addition should succeed with sufficient deposit"
        );
    ...
    fn test_add_member_event_emission() {
        ...
        let group_config = json!({"member_driven": false, "is_private": false});
        contract
            .execute(create_group_request("testgroup".to_string(), group_config))
            .unwrap();
    ...
    fn test_duplicate_member_addition() {
        ...
        let duplicate_result = contract.execute(add_group_member_request(
            "testgroup".to_string(),
            member.clone(),
        ));
    ...
    fn test_member_can_leave_traditional_group() {
        ...
        assert!(
            leave_result.is_ok(),
            "Member should be able to leave group: {:?}",
            leave_result
        );
        ...
        assert!(
            !is_still_member,
            "Member should not be in group after leaving, but is_member returned: {}",
            is_still_member
        );
        ...
        assert!(
            ...
            "Member data should be null after leaving: {:?}",
            member_data
        );
    ...
    fn test_owner_cannot_leave_group() {
        ...
        assert!(
            leave_result.is_err(),
            "Owner should not be able to leave group"
        );

        let error_msg = format!("{:?}", leave_result.unwrap_err());
        assert!(
            error_msg.contains("Owner cannot leave group"),
            "Should get owner cannot leave error: {}",
            error_msg
        );
    ...
    fn test_non_member_cannot_leave_group() {
        ...
        assert!(
            leave_result.is_err(),
            "Non-member should not be able to leave group"
        );

        let error_msg = format!("{:?}", leave_result.unwrap_err());
        assert!(
            error_msg.contains("Member not found"),
            "Should get member not found error: {}",
            error_msg
        );
    ...
    fn test_member_can_leave_member_driven_group() {
        ...
        // Verify member was added
        assert!(
            contract.is_group_member("demo_group".to_string(), member.clone()),
            "Member should be added"
        );
    ...
    fn test_owner_can_remove_other_members_in_traditional_group() {
        ...
        assert!(
            contract.is_group_member("test_group".to_string(), member1.clone()),
            "Member1 should be added"
        );
        assert!(
            contract.is_group_member("test_group".to_string(), member2.clone()),
    ...
    fn test_member_driven_group_removal_restrictions() {
        ...
        assert!(
            self_leave_result.is_ok(),
            "Member should be able to leave group"
        );

        assert!(
            !contract.is_group_member("test_group".to_string(), member1.clone()),
            "Member1 should be removed after leaving"
        );
    ...
    fn test_leave_nonexistent_group() {
        ...
        assert!(
            leave_result.is_err(),
            "Should not be able to leave non-existent group"
        );
    ...
    fn test_comprehensive_event_emission_validation() {
        ...
        let config = json!({"member_driven": false, "is_private": false});
        contract
            .execute(create_group_request("event_test".to_string(), config))
            .unwrap();
        ...
        assert!(
            any_event_found,
            "Events should be emitted during member operations"
        );
    ...
    fn test_event_ordering_in_complex_scenarios() {
        ...
        let config = json!({"member_driven": false, "is_private": false});
        contract
            .execute(create_group_request("ordering_test".to_string(), config))
            .unwrap();
        ...
        let event_logs: Vec<&String> = all_logs
            .iter()
        ...
        assert!(
            event_logs.len() >= 3,
            "Should have events for add, add, permission, blacklist operations"
        );
    ...
    fn test_event_payload_structure_validation() {
        ...
        let config = json!({"member_driven": false, "is_private": false});
        contract
            .execute(create_group_request("payload_test".to_string(), config))
            .unwrap();
        ...
        assert!(
            event_log.starts_with("EVENT_JSON:"),
            "Should be marked as EVENT_JSON"
        );
        // Note: Events are base64 encoded, so we just verify the event was emitted
        assert!(
            event_log.len() > 10,
            "Event should have substantial content"
        );
    ...
    fn test_event_emission_failure_handling() {
        ...
        let config = json!({"member_driven": false, "is_private": false});
        contract
            .execute(create_group_request("failure_test".to_string(), config))
            .unwrap();
        ...
        let new_event_logs: Vec<&String> = logs_after_failure
            .iter()
        ...
        assert!(
            duplicate_result.is_err(),
            "Should fail to add duplicate member"
        );
