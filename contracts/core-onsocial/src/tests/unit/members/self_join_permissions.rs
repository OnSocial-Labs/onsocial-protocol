...
fn test_self_join_public_group_allows_member_only() {
    ...
    assert!(
        member_only_result.is_ok(),
        "Self-join with 0 should succeed: {:?}",
        member_only_result
    );

    // Verify member has 0 global role
    let member_data = contract
        .get_member_data("publicgroup".to_string(), joiner.clone())
        .unwrap();
...
fn test_private_group_join_request_starts_member_only() {
    ...
    assert!(
        join_result.is_ok(),
        "Private group join request with 0 should succeed: {:?}",
        join_result
    );
    ...
    assert!(contract.is_group_member("privategroup".to_string(), joiner.clone()));
    let member_data = contract
        .get_member_data("privategroup".to_string(), joiner.clone())
        .unwrap();
    assert_eq!(member_data.get("level"), Some(&json!(0)));
...
fn test_owner_can_grant_higher_permissions_directly() {
    ...
    let member_data = contract
        .get_member_data("publicgroup".to_string(), member.clone())
        .unwrap();
    assert_eq!(member_data.get("level"), Some(&json!(0)));
...
fn test_existing_member_can_be_upgraded() {
    ...
    near_sdk::testing_env!(
        get_context_with_deposit(member.clone(), 2_000_000_000_000_000_000_000_000).build()
    );
    contract
        .execute(join_group_request("publicgroup".to_string()))
        .unwrap();
    ...
    assert!(
        update_result.is_ok(),
        "Owner should upgrade permissions: {:?}",
        update_result
    );
    ...
    assert!(
        manage_result.is_ok(),
        "Owner should upgrade to MANAGE: {:?}",
        manage_result
    );
    ...
    println!(
        "✅ Existing members can be upgraded to higher permissions (self-join restriction only applies to initial join)"
    );
}
