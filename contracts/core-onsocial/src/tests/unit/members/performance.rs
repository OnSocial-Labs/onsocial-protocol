...
mod member_performance_tests {
    ...
    fn test_bulk_member_operations() {
        ...
        contract
            .execute(create_group_request("bulk_test_group".to_string(), config))
            .unwrap();
        ...
        let avg_gas_add = if successful_adds > 0 {
            total_gas_add / successful_adds as u64
        } else {
            0
        };
        ...
        println!(
            "✅ Bulk add: {} members in {:?}",
            successful_adds, add_duration
        );
        println!(
            "   Average gas per add: {} Tgas",
            avg_gas_add / 1_000_000_000_000
        );
        ...
        let avg_gas_remove = if successful_removes > 0 {
            total_gas_remove / successful_removes as u64
        } else {
            0
        };

        println!(
            "✅ Bulk remove: {} members in {:?}",
            successful_removes, remove_duration
        );
        println!(
            "   Average gas per remove: {} Tgas",
            avg_gas_remove / 1_000_000_000_000
        );
        ...
        assert_eq!(
            successful_adds,
            { bulk_size },
            "All members should be added successfully"
        );
        assert_eq!(
            successful_removes,
            bulk_size / 2,
            "Half the members should be removed"
        );
        ...
        assert!(
            avg_gas_add < 50_000_000_000_000,
            "Average add gas should be under 50 Tgas"
        );
        assert!(
            avg_gas_remove < 50_000_000_000_000,
            "Average remove gas should be under 50 Tgas"
        );

        // Validate realistic production batch sizes with events enabled
        assert!(
            (3..=4).contains(&bulk_size),
            "Bulk operations should use 3-4 ops/tx to stay within NEAR's 16KB log limit with events"
        );
        println!("✅ Gas measurements: Operations well within NEAR limits");
        println!(
            "✅ Batch size: {} operations per transaction (production-realistic with events)",
            bulk_size
        );
    }
    ...
    fn test_operation_timing_analysis() {
        ...
        println!(
            "add_group_member: {:?}, {} Tgas",
            add_duration,
            add_gas / 1_000_000_000_000
        );
        ...
        println!(
            "is_group_member: {:?}, {} Tgas",
            check_duration,
            check_gas / 1_000_000_000_000
        );
        ...
        println!(
            "has_permission: {:?}, {} Tgas",
            has_perm_duration,
            has_perm_gas / 1_000_000_000_000
        );
        ...
        println!(
            "blacklist_group_member: {:?}, {} Tgas",
            blacklist_duration,
            blacklist_gas / 1_000_000_000_000
        );

        // Gas assertions - ensure operations are within reasonable bounds for NEAR
        assert!(
            add_gas < 50_000_000_000_000,
            "add_group_member should use < 50 Tgas"
        );
        assert!(
            check_gas < 10_000_000_000_000,
            "membership check should use < 10 Tgas"
        );
        assert!(
            perm_gas < 30_000_000_000_000,
            "permission grant should use < 30 Tgas"
        );
        assert!(
            has_perm_gas < 10_000_000_000_000,
            "permission check should use < 10 Tgas"
        );
        assert!(
            blacklist_gas < 50_000_000_000_000,
            "blacklist operation should use < 50 Tgas"
        );
    ...
    fn test_permission_hierarchy_performance() {
        ...
        for (depth, path) in test_paths.iter().enumerate() {
            ...
            let has_perm =
                contract.has_permission(owner.clone(), member.clone(), path.to_string(), MODERATE);
            let check_duration = check_start.elapsed();

            println!(
                "Depth {}: {} - Permission check in {:?} ({})",
                depth + 2,
                path,
                check_duration,
                if has_perm { "✅" } else { "❌" }
            ...
            assert!(
                has_perm,
                "Permission should be granted at depth {}",
                depth + 2
            );
            assert!(
        ...
        println!(
            "Hierarchy traversal check in {:?} ({})",
            hierarchy_duration,
            if inherits_permission { "✅" } else { "❌" }
        );

        assert!(
            inherits_permission,
            "Child path should inherit parent permission"
        );
        assert!(
            hierarchy_duration.as_millis() < 200,
            "Hierarchy traversal should be reasonably fast"
        );
    ...
    fn test_storage_efficiency_analysis() {
        ...
        assert!(
            avg_per_member < 10_000,
            "Storage per member should be reasonable (got {} bytes)",
            avg_per_member
        );
        assert!(
            cleaned_up_bytes > 0,
            "Storage cleanup should reclaim some space"
        );
        assert_eq!(member_count, 4, "Should process expected number of members");
        assert_eq!(
            members_to_remove, 2,
            "Should remove expected number of members"
        );
        assert!(
            member_count <= 5,
            "Storage tests should use NEAR-realistic batch sizes"
        );

        // Critical NEAR economics validation (corrected based on official docs)
        assert!(
            storage_cost_near < 0.01,
            "Storage cost per member should be under 0.001 NEAR (got {:.6} NEAR)",
            storage_cost_near
        );
        assert!(
            avg_per_member > 50,
            "Storage per member should be above minimum threshold (prevents spam)"
        );

        // Production economics summary
        println!(
            "✅ Storage economics: {} bytes/member = {:.6} NEAR per member",
            avg_per_member, storage_cost_near
        );
    }
    ...
    fn test_storage_cost_minimum_requirements() {
        ...
        let config = json!({"member_driven": false, "is_private": false});
        contract
            .execute(create_group_request("min_cost_test".to_string(), config))
            .unwrap();
        ...
        assert!(
            storage_used_for_member > 0,
            "Member addition should consume storage"
        );
        assert!(
            storage_used_for_member < 1000,
            "Storage per member should be reasonable (got {} bytes)",
            storage_used_for_member
        );
        assert!(
            actual_cost_near < 0.01,
            "Cost per member should be under 0.01 NEAR (got {:.6} NEAR)",
            actual_cost_near
        );
        ...
        assert!(
            recommended_min_deposit < 100_000_000_000_000_000_000_000,
            "Minimum deposit should be under 0.1 NEAR"
        );

        println!("✅ Storage cost requirements:");
        println!("  - Bytes per member: {}", storage_used_for_member);
        println!(
            "  - Theoretical cost per member: {:.6} NEAR",
            actual_cost_near
        );
        println!(
            "  - Recommended minimum deposit: {:.3} NEAR",
            recommended_min_deposit as f64 / 1e24
        );
    ...
    fn test_bulk_storage_deposit_requirements() {
        ...
        for i in 0..target_members {
            let member_id: near_sdk::AccountId = format!("bulk_deposit_member_{}.testnet", i)
                .parse()
                .unwrap();
        ...
        assert!(
            ...
            "Bulk operations should consume storage"
        );
        assert!(
            total_storage_cost_near < 0.1,
            "Total storage cost should be reasonable (got {:.6} NEAR)",
            total_storage_cost_near
        );
        assert!(
            avg_cost_per_member < 0.01,
            "Average cost per member should be under 0.01 NEAR"
        );
        ...
        let max_members_with_1_near =
            (1e24 as u128) / (total_storage_cost_yocto / target_members as u128);

        println!("✅ Bulk storage deposit analysis:");
        println!(
            "  - Total storage for {} members: {} bytes",
            target_members, total_storage_used
        );
        println!("  - Total cost: {:.6} NEAR", total_storage_cost_near);
        println!("  - Average per member: {:.6} NEAR", avg_cost_per_member);
        println!(
            "  - Recommended bulk deposit: {:.6} NEAR",
            recommended_bulk_deposit as f64 / 1e24
        );
        println!("  - Max members with 1 NEAR: ~{}", max_members_with_1_near);

        // Production assertions (updated for correct economics)
        assert!(
            max_members_with_1_near >= 100,
            "Should be able to add at least 100 members with 1 NEAR"
        );
        // Updated threshold to account for permission key storage costs
        assert!(
            recommended_bulk_deposit < 100_000_000_000_000_000_000_000,
            "Bulk deposit should be under 0.1 NEAR"
        );
    }
