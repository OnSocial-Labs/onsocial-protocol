...
mod permission_tests {
    ...
    fn test_path_specific_permissions_isolation() {
        ...
        let member1_write = contract.set(json!({
        ...
        let member1_unauthorized = contract.set(json!({
        ...
        let member2_write = contract.set(json!({
            "groups/testgroup/posts/post1": {
        ...
        let member2_unauthorized = contract.set(json!({
    ...
    fn test_manager_can_grant_path_permissions_to_members() {
        ...
        // Owner grants manager MANAGE+MODERATE+WRITE permissions
    ...
    fn test_permission_hierarchy_enforcement() {
        ...
        // Add manager with only WRITE permission (cannot grant MANAGE)
        contract.add_group_member("testgroup".to_string(), manager.clone(), WRITE, None).unwrap();
    ...
    fn test_member_role_promotion_workflow() {
        ...
        // Start member with basic WRITE permission
        contract.add_group_member("promotion_test".to_string(), member.clone(), WRITE, None).unwrap();
        
        // Verify initial role
        let member_data = contract.get_member_data("promotion_test".to_string(), member.clone()).unwrap();
        assert_eq!(member_data["permission_flags"], json!(WRITE), "Should start with WRITE permission");
    ...
    fn test_role_hierarchy_enforcement() {
        ...
        contract.add_group_member("hierarchy_test".to_string(), admin.clone(), MANAGE, None).unwrap();
        contract.add_group_member("hierarchy_test".to_string(), moderator.clone(), MODERATE, None).unwrap();
        contract.add_group_member("hierarchy_test".to_string(), writer.clone(), WRITE, None).unwrap();
    ...
    fn test_permission_inheritance_during_transitions() {
        ...
        contract.add_group_member("inheritance_test".to_string(), member.clone(), WRITE, None).unwrap();
    ...
    fn test_member_driven_path_permission_grant_via_voting() {
        ...
        let vote_result = contract.vote_on_proposal("demo_group".to_string(), proposal_id.clone(), true, None);
        ...
        let write_result = contract.set(json!({
        ...
        let unauthorized_write = contract.set(json!({
    ...
    fn test_member_driven_path_permission_revoke_via_voting() {
        ...
        near_sdk::testing_env!(get_context_with_deposit(charlie.clone(), test_deposits::member_operations()).build());
        contract.vote_on_proposal("revoke_demo".to_string(), grant_proposal_id.clone(), true, None).unwrap();
        ...
        let revoke_vote = contract.vote_on_proposal("revoke_demo".to_string(), revoke_proposal_id.clone(), true, None);
        ...
        let write_attempt = contract.set(json!({
    ...
    fn test_new_member_default_path_permissions() {
        ...
        // Add bob as new member with default WRITE permission
        test_add_member_bypass_proposals_with_timestamp(&mut contract, "default_perms", &bob, WRITE, &alice, 1000);

        // Define common paths
        let _content_path = "groups/default_perms/content".to_string();
        let admin_path = "groups/default_perms/admin".to_string();
        let config_path = "groups/default_perms/config".to_string();

        // Verify bob has basic group-level WRITE permission (from member addition)
        let member_data = contract.get_member_data("default_perms".to_string(), bob.clone()).unwrap();
        assert_eq!(member_data["permission_flags"], json!(WRITE), "Bob should have WRITE permission flag");
        ...
        // Note: Path-specific WRITE to content would need to be explicitly granted via proposal
        // Group-level WRITE permission does not automatically grant path-specific permissions

        println!("✅ New member default permissions: Members get basic group permissions but not path-specific access");
    }
    ...
    fn test_member_driven_multiple_path_permissions_workflow() {
        ...
        near_sdk::testing_env!(get_context_with_deposit(alice.clone(), test_deposits::member_operations()).build());
        contract.vote_on_proposal("multi_path".to_string(), bob_proposal_id.clone(), true, None).unwrap();
        ...
        // Need 3 votes for 51% participation with 4 members
        near_sdk::testing_env!(get_context_with_deposit(alice.clone(), test_deposits::member_operations()).build());
        contract.vote_on_proposal("multi_path".to_string(), charlie_proposal_id.clone(), true, None).unwrap();
        ...
        let bob_write = contract.set(json!({
        ...
        let bob_unauthorized = contract.set(json!({
        ...
        let charlie_write = contract.set(json!({
            "groups/multi_path/events/event1": {
        ...
        let charlie_unauthorized = contract.set(json!({
