...
mod permission_tests {
    ...
    fn test_path_specific_permissions_isolation() {
        ...
        assert!(
            grant_result1.is_ok(),
            "Owner should be able to grant permissions to events path"
        );
        ...
        assert!(
            grant_result2.is_ok(),
            "Owner should be able to grant permissions to posts path"
        );

        // Test path isolation: member1 should have access to events but NOT posts
        assert!(
            contract.has_permission(owner.clone(), member1.clone(), events_path.clone(), WRITE),
            "Member1 should have WRITE permission on events path"
        );

        assert!(
            !contract.has_permission(owner.clone(), member1.clone(), posts_path.clone(), WRITE),
            "Member1 should NOT have WRITE permission on posts path"
        );

        assert!(
            !contract.has_permission(owner.clone(), member1.clone(), admin_path.clone(), WRITE),
        ...
        assert!(
            contract.has_permission(owner.clone(), member2.clone(), posts_path.clone(), WRITE),
            "Member2 should have WRITE permission on posts path"
        );

        assert!(
            !contract.has_permission(owner.clone(), member2.clone(), events_path.clone(), WRITE),
            "Member2 should NOT have WRITE permission on events path"
        );

        assert!(
            !contract.has_permission(owner.clone(), member2.clone(), admin_path.clone(), WRITE),
            "Member2 should NOT have WRITE permission on admin path"
        );

        // Test that owner still has access to all paths
        assert!(
            contract.has_permission(owner.clone(), owner.clone(), events_path.clone(), WRITE),
            "Owner should have WRITE permission on events path"
        );

        assert!(
            contract.has_permission(owner.clone(), owner.clone(), posts_path.clone(), WRITE),
            "Owner should have WRITE permission on posts path"
        );

        assert!(
            contract.has_permission(owner.clone(), owner.clone(), admin_path.clone(), WRITE),
        ...
        near_sdk::testing_env!(
            get_context_with_deposit(member1.clone(), 1_000_000_000_000_000_000_000_000).build()
        );

        let member1_write = contract.execute(set_request(json!({
        ...
        let member1_unauthorized = contract.execute(set_request(json!({
        ...
        let member2_write = contract.execute(set_request(json!({
            "groups/testgroup/posts/post1": {
        ...
        let member2_unauthorized = contract.execute(set_request(json!({
        ...
        println!(
            "✅ Path-specific permissions: Members only have access to their assigned paths (verified with actual writes)"
        );
    }
    ...
    fn test_manager_can_grant_path_permissions_to_members() {
        ...
        assert!(
            contract.has_permission(owner.clone(), manager.clone(), config_path.clone(), MANAGE),
        ...
        assert!(
            owner_grant_events.is_ok(),
            "Owner should be able to grant MANAGE to events path: {:?}",
            owner_grant_events
        );
        ...
        assert!(
            owner_grant_moderation.is_ok(),
            "Owner should be able to grant MANAGE to moderation path: {:?}",
            owner_grant_moderation
        );
        ...
        assert!(
            grant_write_result.is_ok(),
            "Manager with MANAGE on events should be able to grant WRITE permissions: {:?}",
            grant_write_result
        );
        ...
        assert!(
            grant_manage_delegation.is_ok(),
            "Manager with MANAGE on moderation should be able to grant WRITE permissions: {:?}",
            grant_manage_delegation
        );
        ...
        assert!(
            owner_grant_moderate_only.is_ok(),
            "Owner should be able to grant MODERATE to admin path: {:?}",
            owner_grant_moderate_only
        );
        ...
        assert!(
            moderate_cannot_delegate.is_err(),
            "User with only MODERATE should NOT be able to delegate permissions"
        );
        ...
        assert!(
            grant_unauthorized.is_err(),
            "User should NOT be able to grant permissions to paths they have no authority on"
        );
        ...
        assert!(
            !contract.has_permission(manager.clone(), member1.clone(), admin_path.clone(), WRITE),
        ...
        println!(
            "✅ Manager delegation: Managers can delegate permissions only to paths where owner has granted them authority"
        );
    }
    ...
    fn test_permission_hierarchy_enforcement() {
        ...
        assert!(
            grant_manage_result.is_err(),
            "Manager with only WRITE should not be able to grant MANAGE permissions"
        );
        ...
        assert!(
            grant_moderate_result.is_err(),
            "Manager with only WRITE should not be able to grant MODERATE permissions"
        );
        ...
        assert!(
            grant_write_result.is_ok(),
            "Manager with MANAGE should be able to grant WRITE permissions: {:?}",
            grant_write_result
        );

        println!(
            "✅ Permission hierarchy properly enforced - cannot grant higher permissions than possessed"
        );
    }
    ...
    fn test_permission_revocation() {
        ...
        if revoke_result.is_ok() {
            ...
            println!(
                "✅ Permission revocation test - implementation may vary: {:?}",
                revoke_result
            );
        }
    }
    ...
    fn test_multiple_permission_levels_on_same_path() {
        ...
        let write_still_available =
            contract.has_permission(owner.clone(), member.clone(), test_path.clone(), WRITE);
        println!(
            "WRITE permission after MODERATE grant: {}",
            write_still_available
        );
    ...
    fn test_member_role_promotion_workflow() {
        ...
        contract
            .execute(create_group_request("promotion_test".to_string(), config))
            .unwrap();

        // Start member as member-only
        contract
            .execute(add_group_member_request(
                "promotion_test".to_string(),
                member.clone(),
            ))
            .unwrap();
        ...
        let member_data = contract
            .get_member_data("promotion_test".to_string(), member.clone())
            .unwrap();
        assert_eq!(
            member_data["level"],
            json!(0),
    ...
    fn test_member_role_demotion_workflow() {
        ...
        let config = json!({"member_driven": false, "is_private": false});
        contract
            .execute(create_group_request("demotion_test".to_string(), config))
            .unwrap();
        contract
            .execute(add_group_member_request(
                "demotion_test".to_string(),
                admin_member.clone(),
            ))
            .unwrap();
    ...
    fn test_role_hierarchy_enforcement() {
        ...
        let config = json!({"member_driven": false, "is_private": false});
        contract
            .execute(create_group_request("hierarchy_test".to_string(), config))
            .unwrap();
        ...
        near_sdk::testing_env!(
            get_context_with_deposit(admin.clone(), 1_000_000_000_000_000_000_000_000).build()
        );
        ...
        assert!(
            admin_promote_result.is_ok(),
            "Admin should be able to promote members"
        );
        ...
        assert!(
            moderator_promote_result.is_err(),
            "Moderator should not be able to grant MANAGE permissions"
        );
        ...
        assert!(
            writer_promote_result.is_err(),
            "Writer should not be able to grant higher permissions"
        );
    ...
    fn test_permission_inheritance_during_transitions() {
        ...
        let config = json!({"member_driven": false, "is_private": false});
        contract
            .execute(create_group_request("inheritance_test".to_string(), config))
            .unwrap();
        contract
            .execute(add_group_member_request(
                "inheritance_test".to_string(),
                member.clone(),
            ))
            .unwrap();
    ...
    fn test_member_driven_path_permission_grant_via_voting() {
        ...
        contract
            .execute(create_group_request("demo_group".to_string(), config))
            .unwrap();
        ...
        near_sdk::testing_env!(
            get_context_with_deposit(alice.clone(), test_deposits::member_operations() * 2).build()
        );
        ...
        let write_result = contract.execute(set_request(json!({
        ...
        let unauthorized_write = contract.execute(set_request(json!({
        ...
        println!(
            "✅ Member-driven permission grant via voting: Members democratically grant path permissions AND can actually use them"
        );
    }
    ...
    fn test_member_driven_path_permission_revoke_via_voting() {
        ...
        contract
            .execute(create_group_request("revoke_demo".to_string(), config))
            .unwrap();
        ...
        let revoke_vote = contract.execute(vote_proposal_request(
            "revoke_demo".to_string(),
        ...
        assert!(
            revoke_vote.is_ok(),
            "Alice's vote on revoke should succeed: {:?}",
            revoke_vote.err()
        );
        ...
        let write_attempt = contract.execute(set_request(json!({
        ...
        println!(
            "✅ Member-driven permission revoke via voting: Members democratically revoke path permissions AND writes are blocked"
        );
    }
    ...
    fn test_new_member_default_path_permissions() {
        ...
        contract
            .execute(create_group_request("default_perms".to_string(), config))
            .unwrap();
        ...
        let member_data = contract
            .get_member_data("default_perms".to_string(), bob.clone())
            .unwrap();
        assert_eq!(
            member_data["level"],
            json!(0),
            "Bob should have 0 global role by default"
        );
        ...
        println!(
            "✅ New member default permissions: Members default to 0 global role but can write to /content"
        );
    }
    ...
    fn test_member_driven_multiple_path_permissions_workflow() {
        ...
        contract
            .execute(create_group_request("multi_path".to_string(), config))
            .unwrap();
        ...
        let charlie_proposal_id = contract
            .execute(create_proposal_request(
                "multi_path".to_string(),
                "path_permission_grant".to_string(),
                charlie_proposal,
        ...
        let bob_write = contract.execute(set_request(json!({
        ...
        let bob_unauthorized = contract.execute(set_request(json!({
        ...
        let charlie_write = contract.execute(set_request(json!({
            "groups/multi_path/events/event1": {
        ...
        let charlie_unauthorized = contract.execute(set_request(json!({
        ...
        println!(
            "✅ Multiple path permissions workflow: Members can have different permissions to different paths AND actually use them"
        );
    }
}
