...
mod storage_flow_tests {
    ...
    fn test_moderator_pays_storage_for_member_addition() {
        ...
        let mod_context =
            get_context_with_deposit(moderator.clone(), 2_000_000_000_000_000_000_000_000);
        ...
        assert!(
            add_result.is_ok(),
            "Moderator should be able to add member: {:?}",
            add_result
        );

        // Verify new member was added
        assert!(
            contract.is_group_member("testgroup".to_string(), new_member.clone()),
    ...
    fn test_public_group_joiner_pays_own_storage() {
        ...
        let config = json!({"member_driven": false, "is_private": false});
        contract
            .execute(create_group_request("publicgroup".to_string(), config))
            .unwrap();
        ...
        let joiner_context =
            get_context_with_deposit(joiner.clone(), 2_000_000_000_000_000_000_000_000);
        near_sdk::testing_env!(joiner_context.build());

        let join_result = contract.execute(join_group_request("publicgroup".to_string()));
        assert!(
            join_result.is_ok(),
            "Join should succeed: {:?}",
            join_result
        ...
        assert!(
            contract.is_group_member("publicgroup".to_string(), joiner.clone()),
        ...
        assert!(
            joiner_balance_after.used_bytes > 0,
            "Joiner's storage should be used (paid for own membership)"
        );

        println!(
            "✅ Joiner paid {} bytes for own membership",
            joiner_balance_after.used_bytes
        );
        println!("   Expected: ~300-400 bytes (member record + permissions)");

        // Reasonable range check
        assert!(
            joiner_balance_after.used_bytes >= 200 && joiner_balance_after.used_bytes <= 600,
            "Storage cost should be reasonable: {} bytes",
            joiner_balance_after.used_bytes
        );
    }
    ...
    fn test_join_request_storage_split_payment() {
        ...
        assert!(
            contract.is_group_member("privategroup".to_string(), requester.clone()),
            "Requester should be a member after approval"
        );

        // Get storage balance after approval (moderator paid for member record)
        let moderator_balance_after_approval =
            contract.get_storage_balance(moderator.clone()).unwrap();

        // VERIFICATION PART 2: Moderator paid for member record
        assert!(
            moderator_balance_after_approval.used_bytes > 0,
        ...
        println!(
            "✅ Moderator paid {} bytes for member record",
            approval_cost
        );
        println!("   Expected: ~300-400 bytes (member record + permissions)");
    ...
    fn test_proposer_pays_for_proposal_execution_storage() {
        ...
        let proposal_id = proposal_result.unwrap().as_str().unwrap().to_string();

        // Voter1 votes (doesn't trigger execution - need 3/4 votes)
        let voter1_context =
            get_context_with_deposit(voter1.clone(), 2_000_000_000_000_000_000_000_000);
        ...
        let voter2_context =
            get_context_with_deposit(voter2.clone(), 3_000_000_000_000_000_000_000_000);
        ...
        assert!(
            vote_result.is_ok(),
            "Vote should trigger execution: {:?}",
            vote_result
        );

        // Verify new member was added (proposal executed)
        assert!(
            contract.is_group_member("demogroup".to_string(), new_member.clone()),
        ...
        assert!(
            owner_balance_after.used_bytes > owner_balance_before.used_bytes,
        ...
        println!(
            "✅ Proposer paid {} bytes for proposal + execution",
            execution_cost
        );
        println!(
            "   Expected: proposal (~300) + vote (~150) + member record (~300) + status update (~200)"
        );

        // Execution includes: proposal record + vote record + member record + status update
        assert!(
            execution_cost >= 400,
            "Proposer execution storage should be reasonable: {} bytes",
            execution_cost
        );
    ...
    fn test_permission_grant_storage_paid_by_granter() {
        ...
        let config = json!({"member_driven": false, "is_private": false});
        contract
            .execute(create_group_request("permgroup".to_string(), config))
            .unwrap();
        ...
        assert!(
            add_result.is_ok(),
            "Member addition should succeed: {:?}",
            add_result
        );
        ...
        assert!(
            mod_balance_after.used_bytes > 0,
        ...
        println!(
            "✅ Moderator paid {} bytes for member addition (includes permission keys)",
            permission_cost
        );
    ...
    fn test_approve_join_request_storage_paid_by_approver() {
        ...
        let config = json!({"member_driven": false, "is_private": true});
        contract
            .execute(create_group_request("approvalgroup".to_string(), config))
            .unwrap();

        // Owner adds approver (clean-add), then delegates moderation on join_requests.
        contract
            .execute(add_group_member_request(
                "approvalgroup".to_string(),
                approver.clone(),
            ))
            .unwrap();
        contract
        ...
        contract
            .execute(join_group_request("approvalgroup".to_string()))
            .unwrap();
        ...
        let app_context =
            get_context_with_deposit(approver.clone(), 2_000_000_000_000_000_000_000_000);
        ...
        // Verify requester is now a member
        assert!(
            contract.is_group_member("approvalgroup".to_string(), requester.clone()),
            "Requester should be a member after approval"
        );
        ...
        assert!(
            approver_balance_after.used_bytes > 0,
    ...
    fn test_storage_refund_on_member_removal_by_moderator() {
        ...
        let config = json!({"member_driven": false, "is_private": false});
        contract
            .execute(create_group_request("refundgroup".to_string(), config))
            .unwrap();

        contract
            .execute(add_group_member_request(
                "refundgroup".to_string(),
                moderator.clone(),
            ))
            .unwrap();
        contract
        ...
        assert!(
            !contract.is_group_member("refundgroup".to_string(), member.clone()),
        ...
        if let Some(balance) = mod_balance_after {
            println!(
                "✅ Moderator has storage balance: {} bytes",
                balance.used_bytes
            );
        } else {
    ...
    fn test_blacklist_storage_paid_by_blacklister() {
        ...
        let config = json!({"member_driven": false, "is_private": false});
        contract
            .execute(create_group_request("securegroup".to_string(), config))
            .unwrap();

        // Add moderator (clean-add), then grant MANAGE on config for blacklisting.
        contract
            .execute(add_group_member_request(
                "securegroup".to_string(),
                moderator.clone(),
            ))
            .unwrap();
        contract
        ...
        assert!(
            !contract.is_group_member("securegroup".to_string(), bad_member.clone()),
        ...
        println!(
            "✅ Moderator paid {} bytes for blacklist record",
            blacklist_cost
        );
    ...
    fn test_reject_join_request_no_additional_storage() {
        ...
        let config = json!({"member_driven": false, "is_private": true});
        contract
            .execute(create_group_request("rejectgroup".to_string(), config))
            .unwrap();

        // Owner adds approver (clean-add), then delegates moderation on join_requests.
        contract
            .execute(add_group_member_request(
                "rejectgroup".to_string(),
                approver.clone(),
            ))
            .unwrap();
        contract
        ...
        contract
            .execute(join_group_request("rejectgroup".to_string()))
            .unwrap();
        ...
        let approver_balance_before_bytes = contract
            .get_storage_balance(approver.clone())
            .map(|s| s.used_bytes)
            .unwrap_or(0);
        ...
        assert!(
            !contract.is_group_member("rejectgroup".to_string(), requester.clone()),
            "Requester should NOT be a member after rejection"
        );

        // Get approver's storage balance after rejection
        let approver_balance_after_bytes = contract
            .get_storage_balance(approver.clone())
            .map(|s| s.used_bytes)
            .unwrap_or(0);

        // CRITICAL VERIFICATION: Rejection should not add significant storage for approver
        let approver_storage_diff =
            approver_balance_after_bytes.saturating_sub(approver_balance_before_bytes);

        println!("✅ Requester paid {} bytes for join request", request_cost);
        println!(
            "✅ Approver's storage change on rejection: {} bytes",
            approver_storage_diff
        );
        println!("   Expected: ~0-100 bytes (minimal status update)");

        // Rejection should only update status field, minimal storage
        assert!(
            approver_storage_diff <= 150,
            "Rejection should not add significant storage: {} bytes",
            approver_storage_diff
        );
    ...
    fn test_member_leave_no_storage_refund_abuse() {
        ...
        let config = json!({"member_driven": false, "is_private": false});
        contract
            .execute(create_group_request("leavegroup".to_string(), config))
            .unwrap();

        // Member self-joins (pays for own storage)
        let member_context =
            get_context_with_deposit(member.clone(), 2_000_000_000_000_000_000_000_000);
        near_sdk::testing_env!(member_context.build());

        contract
            .execute(join_group_request("leavegroup".to_string()))
            .unwrap();
        ...
        let leave_result = contract.execute(leave_group_request("leavegroup".to_string()));
        assert!(
            leave_result.is_ok(),
        ...
        assert!(
            !contract.is_group_member("leavegroup".to_string(), member.clone()),
    ...
    fn test_content_post_storage_paid_by_author() {
        ...
        let config = json!({"member_driven": false, "is_private": false});
        contract
            .execute(create_group_request("contentgroup".to_string(), config))
            .unwrap();

        // Author joins group
        let author_context =
            get_context_with_deposit(author.clone(), 5_000_000_000_000_000_000_000_000);
        near_sdk::testing_env!(author_context.build());

        contract
            .execute(join_group_request("contentgroup".to_string()))
            .unwrap();
        ...
        let post_result = contract.execute(set_request(content));
        ...
        if post_result.is_ok() {
            ...
            let content_cost = author_balance_after_post
                .used_bytes
                .saturating_sub(author_balance_after_join.used_bytes);
    ...
    fn test_insufficient_deposit_fails_operation() {
        ...
        let config = json!({"member_driven": false, "is_private": false});
        contract
            .execute(create_group_request("depositgroup".to_string(), config))
            .unwrap();
        ...
        let join_result = contract.execute(join_group_request("depositgroup".to_string()));

        // Expected behavior: Either fail with clear error OR auto-allocate from contract
        if join_result.is_err() {
            println!(
                "✅ Join correctly rejected with zero deposit: {:?}",
    ...
    fn test_storage_accumulation_multiple_operations() {
        ...
        let config = json!({"member_driven": false, "is_private": false});
        contract
            .execute(create_group_request("activegroup".to_string(), config))
            .unwrap();

        // Active user joins
        let user_context =
            get_context_with_deposit(active_user.clone(), 10_000_000_000_000_000_000_000_000);
        near_sdk::testing_env!(user_context.build());

        contract
            .execute(join_group_request("activegroup".to_string()))
            .unwrap();
        ...
        near_sdk::testing_env!(
            get_context_with_deposit(owner.clone(), 5_000_000_000_000_000_000_000_000).build()
        );
        let config2 = json!({"member_driven": false, "is_private": false});
        contract
            .execute(create_group_request("activegroup2".to_string(), config2))
            .unwrap();

        near_sdk::testing_env!(
            get_context_with_deposit(active_user.clone(), 5_000_000_000_000_000_000_000_000)
                .build()
        );
        contract
            .execute(join_group_request("activegroup2".to_string()))
            .unwrap();

        let balance_after_second_join = contract.get_storage_balance(active_user.clone()).unwrap();
        let second_join_cost = balance_after_second_join
            .used_bytes
            .saturating_sub(join_cost);

        // VERIFICATION: Storage accumulates across multiple operations
        assert!(
            balance_after_second_join.used_bytes > balance_after_join.used_bytes,
        ...
        println!(
            "✅ Total storage: {} bytes",
            balance_after_second_join.used_bytes
        );
    ...
    fn test_unblacklist_storage_paid_by_moderator() {
        ...
        near_sdk::testing_env!(
            get_context_with_deposit(moderator.clone(), 2_000_000_000_000_000_000_000_000).build()
        );
        contract
        ...
        let unblacklist_result = contract.execute(unblacklist_group_member_request(
            "unbangroup".to_string(),
            banned_member.clone(),
        ));
        assert!(
            unblacklist_result.is_ok(),
            "Unblacklist should succeed: {:?}",
            unblacklist_result
        );

        // Get moderator's balance after unblacklist
        let mod_balance_after_unblacklist =
            contract.get_storage_balance(moderator.clone()).unwrap();
        ...
        println!(
            "✅ Moderator's storage before unblacklist: {} bytes",
            mod_balance_after_blacklist.used_bytes
        );
        println!(
            "✅ Moderator's storage after unblacklist: {} bytes",
            mod_balance_after_unblacklist.used_bytes
        );

        // Unblacklist should remove storage (refund) or keep it neutral
        assert!(
            mod_balance_after_unblacklist.used_bytes
                <= mod_balance_after_blacklist.used_bytes + 100,
    ...
    fn test_cancel_join_request_storage_cleanup() {
        ...
        let config = json!({"member_driven": false, "is_private": true});
        contract
            .execute(create_group_request("cancelgroup".to_string(), config))
            .unwrap();
        ...
        contract
            .execute(join_group_request("cancelgroup".to_string()))
            .unwrap();
        ...
        let cancel_context =
            get_context_with_deposit(requester.clone(), 1_000_000_000_000_000_000_000_000);
        near_sdk::testing_env!(cancel_context.build());

        let cancel_result = contract.execute(cancel_join_request("cancelgroup".to_string()));
        assert!(
            cancel_result.is_ok(),
    ...
    fn test_revoke_permissions_storage_refund() {
        ...
        let config = json!({"member_driven": false, "is_private": false});
        contract
            .execute(create_group_request("revokegroup".to_string(), config))
            .unwrap();

        // Add member (clean-add); path permissions are granted explicitly below
        contract
            .execute(add_group_member_request(
                "revokegroup".to_string(),
                member.clone(),
            ))
            .unwrap();
        ...
        if revoke_result.is_ok() {
            ...
            println!(
                "✅ Owner's storage after grant: {} bytes",
                owner_balance_after_grant.used_bytes
            );
            println!(
                "✅ Owner's storage after revoke: {} bytes",
                owner_balance_after_revoke.used_bytes
            );

            // Revoke should remove the permission key, freeing storage
            // Allow small variance but should not increase
            assert!(
                owner_balance_after_revoke.used_bytes <= owner_balance_after_grant.used_bytes + 100,
            ...
            println!(
                "⚠️ Permission revocation uses different API or requires permission removal method"
            );
        }
    }
}
