...
mod member_data_lifecycle_tests {
    ...
    fn test_member_data_persistence_across_operations() {
        ...
        contract
            .execute(create_group_request("data_test".to_string(), config))
            .unwrap();
        contract
        ...
        let initial_data = contract
            .get_member_data("data_test".to_string(), member.clone())
            .unwrap();
        assert!(initial_data.get("level").is_some(), "Should have level");
        assert!(
            initial_data.get("joined_at").is_some(),
            "Should have joined_at timestamp"
        );
        ...
        let after_permission_data = contract
            .get_member_data("data_test".to_string(), member.clone())
            .unwrap();
        assert_eq!(
            after_permission_data["joined_at"],
            json!(initial_joined_at),
    ...
    fn test_member_data_cleanup_on_removal() {
        ...
        let config = json!({"member_driven": false, "is_private": false});
        contract
            .execute(create_group_request("cleanup_test".to_string(), config))
            .unwrap();
        contract
            .execute(add_group_member_request(
        ...
        assert!(
            contract.is_group_member("cleanup_test".to_string(), member.clone()),
            "Should be a member"
        );
        ...
        let after_removal_data =
            contract.get_member_data("cleanup_test".to_string(), member.clone());
        assert!(
            after_removal_data.is_none() || after_removal_data == Some(serde_json::Value::Null),
    ...
    fn test_member_data_cleanup_on_blacklist() {
        ...
        contract
            .execute(create_group_request(
                "blacklist_cleanup_test".to_string(),
                config,
            ))
            .unwrap();
        contract
        ...
        assert!(
            contract.is_group_member("blacklist_cleanup_test".to_string(), member.clone()),
            "Should be a member"
        );
        assert!(
            contract.has_permission(
        ...
        assert!(
            contract.is_blacklisted("blacklist_cleanup_test".to_string(), member.clone()),
        ...
        let member_data =
            contract.get_member_data("blacklist_cleanup_test".to_string(), member.clone());
        assert!(
            member_data.is_none() || member_data == Some(serde_json::Value::Null),
            "Member data should be cleaned up after blacklisting"
        );

        println!(
            "✅ Member data is properly cleaned up on blacklisting while maintaining blacklist entry and revoking permissions"
        );
    }
    ...
    fn test_member_metadata_management() {
        ...
        let member_data = contract
            .get_member_data("metadata_test".to_string(), member.clone())
            .unwrap();

        // Check required metadata fields
        assert!(
            member_data.get("level").is_some(),
            "Should have level metadata"
        );
        assert!(
            member_data.get("joined_at").is_some(),
            "Should have joined_at metadata"
        );
        ...
        assert_eq!(
            updated_data["level"],
            json!(0),
            "Member data permission flags remain the same"
        );
        assert_eq!(
            updated_data["joined_at"],
            json!(joined_at),
            "Original joined_at should be preserved"
        );
    ...
    fn test_member_data_updates_and_versioning() {
        ...
        let config = json!({"member_driven": false, "is_private": false});
        contract
            .execute(create_group_request("versioning_test".to_string(), config))
            .unwrap();
        contract
            .execute(add_group_member_request(
                "versioning_test".to_string(),
                admin.clone(),
            ))
            .unwrap();
        contract
        ...
        // Get initial data
        let initial_data = contract
            .get_member_data("versioning_test".to_string(), member.clone())
            .unwrap();
        ...
        let has_moderate = contract.has_permission(
            admin.clone(),
            member.clone(),
            "groups/versioning_test/config".to_string(),
            MODERATE,
        ...
        assert!(
            has_moderate,
            "Member should have MODERATE permissions after update"
        );
        ...
        assert_eq!(
            updated_data["joined_at"],
            json!(initial_timestamp),
            "Original join timestamp preserved"
        );
        ...
        let has_manage = contract.has_permission(
            owner.clone(),
            member.clone(),
            "groups/versioning_test/config".to_string(),
            MANAGE,
        ...
        assert!(
            has_manage,
            "Member should have MANAGE permissions after final update"
        );
        ...
        let final_data = contract
            ...
            .unwrap();
        assert_eq!(
            final_data["joined_at"],
            json!(initial_timestamp),
            "Original join timestamp still preserved"
        );
    ...
    fn test_member_data_integrity_across_complex_scenarios() {
        ...
        let config = json!({"member_driven": false, "is_private": false});
        contract
            .execute(create_group_request("integrity_test".to_string(), config))
            .unwrap();
        ...
        contract
            ...
            .unwrap();
        let first_join_data = contract
            .get_member_data("integrity_test".to_string(), member.clone())
            .unwrap();
        let first_join_time: u64 = first_join_data["joined_at"]
            .as_str()
            .unwrap()
            .parse()
            .unwrap();
        ...
        contract
            ...
            .unwrap();
        assert!(
            !contract.is_group_member("integrity_test".to_string(), member.clone()),
            "Should be removed"
        );
        ...
        assert_eq!(
            rejoin_data["level"],
            json!(0),
            "Members start member-only on rejoin"
        );
        assert!(
            rejoin_time >= first_join_time,
            "Rejoin timestamp should be newer or equal"
        );
        ...
        contract
            ...
            .unwrap();
        assert!(
            contract.is_blacklisted("integrity_test".to_string(), member.clone()),
            "Should be blacklisted"
        );
        assert!(
            !contract.is_group_member("integrity_test".to_string(), member.clone()),
        ...
        assert!(
            !contract.is_group_member("integrity_test".to_string(), member.clone()),
            "Should not automatically be member"
        );
    ...
    fn test_permission_persistence_during_blacklist_cycles() {
        ...
        contract
            .execute(create_group_request(
                "permission_persistence_test".to_string(),
                config,
            ))
            .unwrap();
        contract
        ...
        assert!(
            !contract.is_group_member("permission_persistence_test".to_string(), member.clone()),
            "Should not be a member after blacklisting"
        );
        assert!(
            !contract.has_permission(
        ...
        contract
            ...
            .unwrap();
        assert!(
            !contract.is_blacklisted("permission_persistence_test".to_string(), member.clone()),
        ...
        println!(
            "✅ Permission behavior correct during blacklist cycles - old permissions do not resurrect after rejoin"
        );
    }
}
