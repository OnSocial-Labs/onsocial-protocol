...
mod voting_group_updates_tests {
    ...
    fn test_ban_member_via_voting_approval() {
        ...
        contract
            .execute(create_group_request("ban_test".to_string(), config))
            .unwrap();
        ...
        assert!(
            contract.is_group_member("ban_test".to_string(), bob.clone()),
            "Bob should be a member"
        );
        assert!(
            !contract.is_blacklisted("ban_test".to_string(), bob.clone()),
            "Bob should not be banned initially"
        );
        ...
        assert!(
            create_proposal_result.is_err(),
            "Banned member should not be able to create proposals"
        );
    ...
    fn test_ban_member_via_voting_rejection() {
        ...
        let config = json!({"member_driven": true, "is_private": true});
        contract
            .execute(create_group_request("reject_ban".to_string(), config))
            .unwrap();
        ...
        assert!(
            !contract.is_blacklisted("reject_ban".to_string(), bob.clone()),
            "Bob should not be blacklisted"
        );
        assert!(
            contract.is_group_member("reject_ban".to_string(), bob.clone()),
            "Bob should remain a member"
        );
        ...
        assert!(
            create_proposal_result.is_ok(),
            "Bob should still be able to create proposals"
        );
    ...
    fn test_ban_removes_member_automatically() {
        ...
        let config = json!({"member_driven": true, "is_private": true});
        contract
            .execute(create_group_request("auto_remove".to_string(), config))
            .unwrap();
        ...
        assert!(
            contract.is_group_member("auto_remove".to_string(), bob.clone()),
            "Bob should be a member"
        );
        ...
        assert!(
            contract.is_blacklisted("auto_remove".to_string(), bob.clone()),
            "Bob should be blacklisted"
        );
        assert!(
            !contract.is_group_member("auto_remove".to_string(), bob.clone()),
            "Bob should be automatically removed"
        );
    ...
    fn test_cannot_ban_group_owner() {
        ...
        let config = json!({"member_driven": true, "is_private": true});
        contract
            .execute(create_group_request("owner_protect".to_string(), config))
            .unwrap();
        ...
        assert!(
            vote_result.is_err()
                || !contract.is_blacklisted("owner_protect".to_string(), alice.clone()),
            "Owner should be protected from banning"
        );

        // Verify alice is still the owner and member
        assert!(
            contract.is_group_member("owner_protect".to_string(), alice.clone()),
            "Owner should remain a member"
        );
        assert!(
            !contract.is_blacklisted("owner_protect".to_string(), alice.clone()),
    ...
    fn test_banned_member_cannot_create_proposals() {
        ...
        let config = json!({"member_driven": true, "is_private": true});
        contract
            .execute(create_group_request("restrict_banned".to_string(), config))
            .unwrap();
        ...
        assert!(
            contract.is_blacklisted("restrict_banned".to_string(), bob.clone()),
    ...
    fn test_unban_member_via_voting_approval() {
        ...
        let config = json!({"member_driven": true, "is_private": true});
        contract
            .execute(create_group_request("unban_test".to_string(), config))
            .unwrap();
        ...
        assert!(
            contract.is_blacklisted("unban_test".to_string(), bob.clone()),
            "Bob should be banned"
        );
        assert!(
            !contract.is_group_member("unban_test".to_string(), bob.clone()),
        ...
        assert!(
            !contract.is_group_member("unban_test".to_string(), bob.clone()),
            "Bob should not be auto-readded"
        );
        ...
        assert!(
            rejoin_result.is_ok(),
            "Unbanned member should be able to request to rejoin: {:?}",
            rejoin_result.err()
        );
    ...
    fn test_unban_does_not_auto_readd_member() {
        ...
        let config = json!({"member_driven": true, "is_private": true});
        contract
            .execute(create_group_request("no_auto_readd".to_string(), config))
            .unwrap();
        ...
        assert!(
            contract.is_blacklisted("no_auto_readd".to_string(), bob.clone()),
            "Bob should be banned"
        );
        assert!(
            !contract.is_group_member("no_auto_readd".to_string(), bob.clone()),
            "Bob should be removed"
        );
    ...
    fn test_unban_nonexistent_ban() {
        ...
        let config = json!({"member_driven": true, "is_private": true});
        contract
            .execute(create_group_request("unban_edge".to_string(), config))
            .unwrap();
        ...
        assert!(
            !contract.is_blacklisted("unban_edge".to_string(), bob.clone()),
            "Bob should not be banned"
        );
        ...
        assert!(
            result.is_ok(),
            "Unbanning non-banned user should succeed (idempotent): {:?}",
            result.err()
        );
        ...
        assert!(
            !contract.is_blacklisted("unban_edge".to_string(), bob.clone()),
            "Bob should still not be banned"
        );
        assert!(
            contract.is_group_member("unban_edge".to_string(), bob.clone()),
    ...
    fn test_transfer_ownership_via_voting_approval() {
        ...
        let initial_config = contract
            .get_group_config("ownership_transfer".to_string())
            .unwrap();
        assert_eq!(
            initial_config.get("owner").and_then(|v| v.as_str()),
        ...
        let updated_config = contract
            ...
            .unwrap();
        assert_eq!(
            updated_config.get("owner").and_then(|v| v.as_str()),
        ...
        assert!(
            !contract.is_group_member("ownership_transfer".to_string(), alice.clone()),
        ...
        let final_config = contract
            ...
            .unwrap();
        assert_eq!(
            final_config.get("owner").and_then(|v| v.as_str()),
            Some(bob.to_string().as_str()),
        ...
        println!(
            "   - Non-owner member successfully proposed ownership transfer (democratic governance)"
        );
    ...
    fn test_transfer_ownership_to_non_member_fails() {
        ...
        assert!(
            !contract.is_group_member("ownership_validation".to_string(), charlie.clone()),
        ...
        assert!(
            result.is_err(),
            "Creating proposal to transfer ownership to non-member should fail"
        );

        let error_msg = result.unwrap_err().to_string();
        assert!(
            error_msg.contains("must be a member") || error_msg.contains("not a member"),
            "Error should indicate new owner must be a member, got: {}",
            error_msg
        );

        // Verify ownership unchanged
        let config = contract
            .get_group_config("ownership_validation".to_string())
            .unwrap();
        assert_eq!(
            config.get("owner").and_then(|v| v.as_str()),
            Some(alice.to_string().as_str()),
    ...
    fn test_old_owner_becomes_regular_member() {
        ...
        let config = json!({"member_driven": true, "is_private": true});
        contract
            .execute(create_group_request("owner_stays".to_string(), config))
            .unwrap();
        ...
        let initial_config = contract
            .get_group_config("owner_stays".to_string())
            .unwrap();
        assert_eq!(
            initial_config.get("owner").and_then(|v| v.as_str()),
            Some(alice.to_string().as_str())
        ...
        assert!(
            contract.is_group_member("owner_stays".to_string(), alice.clone()),
        ...
        assert!(
            alice_proposal_result.is_ok(),
            "Alice should still be able to create proposals as regular member"
        );
        ...
        assert!(
            alice_vote_result.is_ok(),
            "Alice should still be able to vote as regular member"
        );
    ...
    fn test_cannot_transfer_ownership_to_self() {
        ...
        let config = json!({"member_driven": true, "is_private": true});
        contract
            .execute(create_group_request("self_transfer".to_string(), config))
            .unwrap();
        ...
        assert!(
            ...
            "Error should indicate self-transfer prevention, got: {}",
            error_msg
        );

        // Verify ownership unchanged
        let config = contract
            .get_group_config("self_transfer".to_string())
            .unwrap();
        assert_eq!(
            config.get("owner").and_then(|v| v.as_str()),
            Some(alice.to_string().as_str()),
    ...
    fn test_metadata_update_approval_multiple_fields() {
        ...
        let config = json!({"member_driven": true, "is_private": true});
        contract
            .execute(create_group_request("meta_update".to_string(), config))
            .unwrap();
        ...
        let updated_config = contract
            .get_group_config("meta_update".to_string())
            .unwrap();

        // Check that metadata fields were updated
        assert_eq!(
            updated_config.get("name"),
            Some(&json!("New Amazing Group")),
            "Name should be updated"
        );
        assert_eq!(
            updated_config.get("description"),
            Some(&json!("This group has been updated with new information")),
        ...
        if updated_config.get("tags").is_some() {
            assert_eq!(
                updated_config.get("tags"),
                Some(&json!(["updated", "governance", "community"])),
    ...
    fn test_member_driven_cannot_be_made_public_via_metadata_update() {
        ...
        assert!(
            vote_result.is_err(),
            "Vote should fail due to invariant enforcement"
        );
    ...
    fn test_metadata_update_rejection_preserves_old_data() {
        ...
        contract
            .execute(create_group_request("reject_meta".to_string(), config))
            .unwrap();
        ...
        // Get initial config
        let initial_config = contract
            .get_group_config("reject_meta".to_string())
            .unwrap();
        ...
        assert_eq!(
            final_config.get("name"),
            initial_name.as_ref(),
            "Name should remain unchanged after rejection"
        );
        assert_eq!(
            final_config.get("description"),
            initial_description.as_ref(),
            "Description should remain unchanged after rejection"
        );

        // Verify rejected values are NOT present
        assert_ne!(
            final_config.get("name"),
            Some(&json!("Rejected Name")),
            "Rejected name should not be applied"
        );
        assert_ne!(
            final_config.get("description"),
            Some(&json!("This should not be applied")),
            "Rejected description should not be applied"
        );
    ...
    fn test_metadata_update_validation() {
        ...
        let config = json!({"member_driven": true, "is_private": true});
        contract
            .execute(create_group_request("validation_meta".to_string(), config))
            .unwrap();
        ...
        assert!(
            result.is_ok(),
            "Valid metadata proposal should be created: {:?}",
            result.err()
        );
    ...
    fn test_ban_then_unban_full_cycle() {
        ...
        let config = json!({"member_driven": true, "is_private": true});
        contract
            .execute(create_group_request("full_cycle".to_string(), config))
            .unwrap();
        ...
        assert!(
            contract.is_group_member("full_cycle".to_string(), charlie.clone()),
        ...
        let mut bob_context = get_context_with_deposit(bob.clone(), test_deposits::ten_near());
        ...
        assert!(
            contract.is_blacklisted("full_cycle".to_string(), charlie.clone()),
        ...
        // Bob votes YES (alice already voted YES)
        let mut bob_context2 = get_context_with_deposit(bob.clone(), test_deposits::ten_near());
    ...
    fn test_ownership_transfer_then_back() {
        ...
        let config = json!({"member_driven": true, "is_private": true});
        contract
            .execute(create_group_request("transfer_chain".to_string(), config))
            .unwrap();
        ...
        let initial_config = contract
            .get_group_config("transfer_chain".to_string())
            .unwrap();
        assert_eq!(
            initial_config.get("owner").and_then(|v| v.as_str()),
            Some(alice.to_string().as_str()),
        ...
        let mut bob_context1 = get_context_with_deposit(bob.clone(), test_deposits::ten_near());
        ...
        let config_after_1 = contract
            .get_group_config("transfer_chain".to_string())
            .unwrap();
        assert_eq!(
            config_after_1.get("owner").and_then(|v| v.as_str()),
            Some(bob.to_string().as_str()),
            "Bob should be owner after first transfer"
        );
        assert!(
            contract.is_group_member("transfer_chain".to_string(), alice.clone()),
        ...
        // With 3 members, 2 YES votes = 66% participation, 100% approval -> auto-executes
        let mut alice_context2 = get_context_with_deposit(alice.clone(), test_deposits::ten_near());
        ...
        let config_after_2 = contract
            .get_group_config("transfer_chain".to_string())
            .unwrap();
        assert_eq!(
            config_after_2.get("owner").and_then(|v| v.as_str()),
            Some(charlie.to_string().as_str()),
            "Charlie should be owner after second transfer"
        );
        assert!(
            contract.is_group_member("transfer_chain".to_string(), bob.clone()),
            "Bob should still be a member"
        );
        ...
        // With 3 members, 2 YES votes = 66% participation, 100% approval -> auto-executes
        let mut bob_context3 = get_context_with_deposit(bob.clone(), test_deposits::ten_near());
        ...
        assert!(
            contract.is_group_member("transfer_chain".to_string(), charlie.clone()),
    ...
    fn test_permissions_survive_ownership_transfer() {
        ...
        contract
            .execute(create_group_request("perm_test".to_string(), config))
            .unwrap();
        ...
        assert!(
            charlie_has_moderate_before,
            "Charlie should have MODERATE permission before ownership transfer"
        );
        ...
        assert!(
            bob_has_manage,
            "Bob (new owner) should have MANAGE permissions automatically"
        );
        ...
        println!(
            "   - Charlie's MODERATE permission granted by Alice still works after Alice → Bob transfer"
        );
