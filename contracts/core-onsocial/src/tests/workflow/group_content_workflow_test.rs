...
mod group_content_integration_tests {
    ...
    fn test_member_creates_post_in_public_group() {
        ...
        contract
            .execute(join_group_request("tech_discussion".to_string()))
            .unwrap();
        ...
        let stored_content =
            retrieved.get(&format!("{}/groups/tech_discussion/posts/post1", member));

        assert!(
            stored_content.is_some(),
            "Post should exist at user-owned path"
        );
    ...
    fn test_member_creates_multiple_posts() {
        ...
        contract
            .execute(join_group_request("blog_group".to_string()))
            .unwrap();
        ...
        assert!(
            result.is_ok(),
            "Multiple posts should be created: {:?}",
            result.err()
        );
    ...
    fn test_different_content_types() {
        ...
        contract
            .execute(join_group_request("community".to_string()))
            .unwrap();
        ...
        assert!(
            result.is_ok(),
            "Different content types should work: {:?}",
            result.err()
        );
    ...
    fn test_non_member_cannot_create_content() {
        ...
        let context =
            get_context_with_deposit(non_member.clone(), 5_000_000_000_000_000_000_000_000);
        testing_env!(context.build());
    ...
    fn test_member_without_path_permission_blocked() {
        ...
        assert!(
            result.is_ok(),
            "Should be able to write to posts/: {:?}",
            result.err()
        );
    ...
    fn test_content_creator_pays_storage() {
        ...
        contract
            .execute(join_group_request("storage_test".to_string()))
            .unwrap();
    ...
    fn test_content_creation_emits_events() {
        ...
        contract
            .execute(join_group_request("event_test".to_string()))
            .unwrap();
    ...
    fn test_multi_member_content_collaboration() {
        ...
        testing_env!(context.build());
        contract
            .execute(join_group_request("collab".to_string()))
            .unwrap();
    ...
    fn test_content_creation_in_private_group() {
        ...
        let context =
            get_context_with_deposit(approved_member.clone(), 5_000_000_000_000_000_000_000_000);
        testing_env!(context.build());
        ...
        assert!(
            result.is_ok(),
            "Approved member should create content in private group: {:?}",
            result.err()
        );

        // Verify content exists (path includes group)
        let keys = vec![format!(
            "{}/groups/private_club/posts/secret",
            approved_member
        )];
        let retrieved = contract_get_values_map(&contract, keys, None);
        assert!(
            !retrieved.is_empty(),
            "Private group content should be retrievable"
        );
    ...
    fn test_content_creation_in_nonexistent_group() {
        ...
        assert!(
            error_msg.contains("Group does not exist") || error_msg.contains("Permission denied"),
    ...
    fn test_insufficient_storage_for_content() {
        ...
        contract
            .execute(join_group_request("storage_limit".to_string()))
            .unwrap();
        ...
        assert!(result.is_err(), "Should fail with insufficient storage");
    ...
    fn test_content_update_and_versioning() {
        ...
        contract
            .execute(join_group_request("versioning".to_string()))
            .unwrap();
        ...
        let content = retrieved
            .get(&format!("{}/groups/versioning/posts/article1", member))
    ...
    fn test_content_retrieval_by_different_users() {
        ...
        contract
            .execute(join_group_request("public_read".to_string()))
            .unwrap();
