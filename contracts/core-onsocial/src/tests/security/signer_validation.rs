...
mod signer_validation_core_tests {
    ...
    fn test_comprehensive_signer_security_model() {
        ...
        // ✅ TEST 1: Alice can write her own data directly
        let context = get_context_with_deposit(alice.clone(), calculate_test_deposit_for_operations(1, 200));
        testing_env!(context.build());
    ...
    fn test_uses_predeposited_balance_first() {
        ...
        // Step 3: Verify storage increased by the ADDITIONAL amount (pre-deposit + attached)
        let storage_final = contract.get_storage_balance(alice.clone()).unwrap();
        
        // Storage should be: predeposit + additional - used_bytes
        // The fact that it's > predeposit means additional was added
        assert!(
            storage_final.balance >= predeposit,
            "✅ Contract uses pre-deposited balance and adds attached deposit"
        );
    }
