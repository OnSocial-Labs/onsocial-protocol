...
--
-- Migrations run before *_schema_views.sql on fresh installs. Skip until
-- scarces event tables + dependent owner/listing views exist; fresh installs
-- get scarces_app_stats_hot from scarces_schema_views.sql.

DO $$
BEGIN
  IF to_regclass('public.scarces_events') IS NULL
     OR to_regclass('public.scarces_apps') IS NULL
     OR to_regclass('public.scarces_token_owners') IS NULL
     OR to_regclass('public.scarces_active_listings') IS NULL THEN
    RAISE NOTICE 'skip scarces_app_stats_hot migration (scarces deps not ready yet)';
    RETURN;
  END IF;
