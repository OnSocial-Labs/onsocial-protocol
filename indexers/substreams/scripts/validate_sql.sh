...
"${DOCKER_BIN}" run --rm \
  ...
  sh -lc '
    ...
    validate_guild_view_columns() {
      ...
      ")"
      expected_groups_columns="group_id,owner_id,group_name,is_public,creator_role,storage_allocation,block_height,block_timestamp,operation,group_description,group_banner_cid,is_member_driven,group_topics"
      ...
      ")"
      expected_members_columns="group_id,member_id,role,level,is_owner,is_admin,can_moderate,group_name,is_public,block_height,block_timestamp,group_description,group_banner_cid,is_member_driven,group_topics"
