...
fn make_event(event_type: &str, account_id: &str, payload: Payload) -> BoostEvent {
    BoostEvent {
...
fn test_write_boost_event_columns() {
    ...
    let event = make_event(
        ...
        Payload::BoostLock(BoostLock {
            ...
            months: 48,
            effective_boost: "112500000000000000000".to_string(),
        }),
    );
    ...
    assert_eq!(
        find_field(&changes, "boost_events", "account_id"),
        Some("alice.near")
    );
    assert_eq!(
    ...
    assert_eq!(
        find_field(&changes, "boost_events", "amount"),
        Some("75000000000000000000")
    );
    assert_eq!(
        find_field(&changes, "boost_events", "effective_boost"),
        Some("112500000000000000000")
    );
    assert_eq!(
        find_field(&changes, "boost_events", "block_height"),
        Some("100")
    );
}

#[test]
fn test_update_booster_state_lock_vs_unlock() {
    // BOOST_LOCK creates booster_state with locked amounts
    ...
    let lock_event = make_event(
        ...
        Payload::BoostLock(BoostLock {
            ...
            months: 12,
            effective_boost: "120".to_string(),
        }),
    );
    accumulate_booster_state(&mut accum, &lock_event);
    ...
    assert_eq!(count_table_rows(&changes, "booster_state"), 1);
    assert_eq!(
        find_field(&changes, "booster_state", "locked_amount"),
        Some("100")
    );
    assert_eq!(
        find_field(&changes, "booster_state", "effective_boost"),
        Some("120")
    );

    // BOOST_UNLOCK zeros out
    ...
    for (account_id, state) in &accum2 {
        let row = tables2.upsert_row("booster_state", account_id);
        row.set("account_id", account_id);
        if let Some(v) = &state.locked_amount {
            row.set("locked_amount", v);
        }
    ...
    assert_eq!(
        find_field(&changes2, "booster_state", "locked_amount"),
        Some("0")
    );
    assert_eq!(
        find_field(&changes2, "booster_state", "effective_boost"),
        Some("0")
    );
}
...
fn test_write_credit_purchase_table() {
    ...
    assert_eq!(count_table_rows(&changes, "boost_credit_purchases"), 1);
    assert_eq!(
        find_field(&changes, "boost_credit_purchases", "account_id"),
        Some("buyer.near")
    );
    assert_eq!(
        find_field(&changes, "boost_credit_purchases", "amount"),
        Some("1000")
    );
    assert_eq!(
        find_field(&changes, "boost_credit_purchases", "infra_share"),
        Some("600")
    );
    assert_eq!(
        find_field(&changes, "boost_credit_purchases", "rewards_share"),
        Some("400")
    );
}
...
fn test_non_state_event_skips_booster_state() {
    ...
    accumulate_booster_state(&mut accum, &event);

    assert_eq!(
        accum.len(),
        0,
        "INFRA_WITHDRAW should not create booster_state"
    );
}

#[test]
fn test_boost_extend_updates_state() {
    let mut accum = HashMap::new();
    let event = make_event(
        "BOOST_EXTEND",
        "alice.near",
        Payload::BoostExtend(BoostExtend {
            new_months: 24,
            new_effective_boost: "200".to_string(),
        }),
    );
    accumulate_booster_state(&mut accum, &event);
...
#[test]
fn test_rewards_claim_updates_state() {
    let mut accum = HashMap::new();
    let event = make_event(
        "REWARDS_CLAIM",
        "alice.near",
    ...
    let state = accum.get("alice.near").unwrap();
    assert_eq!(state.last_event_type, "REWARDS_CLAIM");
    assert_eq!(state.total_claimed.as_deref(), Some("5000"));
}
