#!/usr/bin/env bash
# Merge Google Secret Manager values into packages/onsocial-app/.env.local
# for local app dev aligned with testnet/mainnet gateway credentials.
#
# Usage:
#   NEAR_NETWORK=testnet ./scripts/sync-app-env-from-gsm.sh
#   NEAR_NETWORK=mainnet ./scripts/sync-app-env-from-gsm.sh
#
# Requires: gcloud auth with access to project onsocial-protocol
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
ENV_FILE="${APP_ENV_FILE:-$ROOT/packages/onsocial-app/.env.local}"
...
echo "Pulling app secrets from GSM (NEAR_NETWORK=$NETWORK)..."
TMP_GSM="$(mktemp)"
...
if [ -n "$LAVA_KEY" ]; then
  ...
  # Prefer OnSocial Lava RPC over ad-hoc public NEAR_RPC_URL overrides.
  remove_env_key "$ENV_FILE" "NEAR_RPC_URL"
fi
...
echo "✅ App .env.local aligned with GSM"
echo "   ONSOCIAL_API_KEY prefix: ${PREFIX}…"
if [ -n "$LAVA_KEY" ]; then
  echo "   LAVA_API_KEY: set (OnSocial RPC via /api/near/rpc BFF)"
else
  echo "   ⚠️  LAVA_API_KEY missing — server RPC will fall back to public FastNear"
fi
echo "   API URL: $PUBLIC_API"
echo ""
echo "Restart the app dev server: pnpm --filter @onsocial/app dev"
