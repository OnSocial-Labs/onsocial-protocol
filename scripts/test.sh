...
CONTRACT_LIST=("scarces-onsocial" "staking-onsocial" "boost-onsocial" "vesting-onsocial" "core-onsocial" "token-onsocial" "rewards-onsocial")
...
test_integration() {
    ...
    if [ -n "$module" ]; then
        case $module in
            scarces-onsocial|staking-onsocial|boost-onsocial|vesting-onsocial|core-onsocial|token-onsocial|rewards-onsocial)
                ...
                local features_flag=""
                [ "$module" = "scarces-onsocial" ] && features_flag="--features sandbox"
                cargo near build non-reproducible-wasm $features_flag || { echo -e "${ERROR}Release build failed for $module${RESET}"; INTEGRATION_RESULTS["$module"]="Failed"; ((INTEGRATION_FAILURES++)); return 1; }
    ...
    if [ -n "$module" ]; then
        case $module in
            ...
            staking-onsocial)
                ...
                ;;
            boost-onsocial)
                # Run near-workspaces integration tests for boost-onsocial
                rm -rf /tmp/.tmp* 2>/dev/null || true

                # Build mock-ft for FT integration tests
                echo "Building mock-ft contract for boost integration tests..."
                if [ -d "$BASE_DIR/mock-ft" ]; then
                    cd "$BASE_DIR/mock-ft" || { echo -e "${WARNING}mock-ft contract not found, FT tests will be skipped${RESET}"; }
                    cargo near build non-reproducible-wasm || { echo -e "${ERROR}Failed to build mock-ft${RESET}"; }
                fi
                ...
                local test_filter="boost_onsocial_tests"
                if [ -n "$test_name" ]; then
                    test_filter="$test_name"
                    echo "Running specific integration test: $test_name"
                fi
